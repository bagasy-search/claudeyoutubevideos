...
const OUT = "public/broll/aloearms";
const TMP = "public/broll/_aloearms_tmp";
...
const items = JSON.parse(fs.readFileSync("scripts/_aloearms_stock_src.json", "utf8").replace(/^\uFEFF/, ""));
const okMoments = [];
...
fs.writeFileSync("_aloearms_stock_moments.json", JSON.stringify(okMoments, null, 1));
fs.writeFileSync("scripts/_aloearms_stock_miss.json", JSON.stringify(missList, null, 1));
console.log(`\n=== aloearms stock: ${ok} OK · ${miss} MISS · ${dark} dark · ${badfps} badfps de ${items.length} ===`);
console.log(`moments -> _aloearms_stock_moments.json (${okMoments.length})`);
