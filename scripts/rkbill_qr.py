# -*- coding: utf-8 -*-
"""QR funcional del embudo de rkbill -> public/img/rkbill_qrcard.png
...
URL = "https://raykessler.vercel.app/"
...
def main() -> None:
    ...
    qr_img = build_qr()
    path = OUT / "rkbill_qrcard.png"
    qr_img.save(path, optimize=True)
    checks = {}
    for px in (104, 156, 208, 360, 560):
        dec = decode(qr_img.resize((px, px), Image.Resampling.LANCZOS))
        checks[str(px)] = dec == URL
        assert dec == URL, f"rkbill_qrcard no decodifica a {px}px: {dec!r}"
