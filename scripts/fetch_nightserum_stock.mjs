...
// Lee public/broll/dense_thinned_ns.json [{name, query, t}] → public/broll/nightserum/<name>.mp4 (aislado por slug).
import fs from "fs";
import { pexelsVideo, pixabayVideo, archiveVideo, usedRegistry } from "./stock_lib.mjs";

const items = JSON.parse(fs.readFileSync("public/broll/dense_thinned_ns.json", "utf8").replace(/^﻿/, ""));
...
fs.writeFileSync("public/broll/nightserum_miss.json", JSON.stringify(missList, null, 1));
console.log(`\n=== stock nightserum: ${ok} OK · ${miss} MISS de ${items.length} ===`);
