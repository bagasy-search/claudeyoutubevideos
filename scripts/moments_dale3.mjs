// moments_dale3.mjs — guion + wordms -> _v3/dale3_moments.json  [{t, txt, i, dur, sec}]
//
...
let vistos = 0;
for (const m of mom) {
  const mm = m.txt.match(/^Number (twenty[ -]\w+|\w+)\./i);
  if (mm) {
    const n = NUM.indexOf(mm[1].toLowerCase().replace("-", " ")) + 1;
    if (n > vistos) { vistos = n; sec = "S" + String(n).padStart(2, "0"); }
  } else if (sec === "HOOK" && /^My name is Dale Kessler/.test(m.txt)) sec = "INTRO";
  m.sec = sec;
}
// la cola despues del ultimo "What I photographed instead" del 25
const iCierre = mom.findIndex((m) => /^That is twenty five\./.test(m.txt));
