// fetch_rub_stock.mjs — descarga stock (Pexels→Pixabay→Archive) para la cama densa de rosemaryrub.
// Lee public/broll/dense_thinnedR.json [{name, query, t}] → public/broll/rosemaryrub/<name>.mp4 (aislado por slug).
import fs from "fs";
import { pexelsVideo, pixabayVideo, archiveVideo, usedRegistry } from "./stock_lib.mjs";

const items = JSON.parse(fs.readFileSync("public/broll/dense_thinnedR.json", "utf8").replace(/^﻿/, ""));
...
fs.writeFileSync("public/broll/rub_miss.json", JSON.stringify(missList, null, 1));
console.log(`\n=== stock rosemaryrub: ${ok} OK · ${miss} MISS de ${items.length} ===`);
