// dense_prep8.mjs — toma public/broll/dense_federer8.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinned8.json   [{name, at, query, t}]
//   public/broll/shots_dense8.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/federer8_broll.ts  (FED8_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_federer8.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned8.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_dense8.json", JSON.stringify(shots, null, 1));
...
fs.writeFileSync("src/_fed6/VideoEdit/federer8_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep8.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const FED8_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
