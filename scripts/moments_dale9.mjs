// moments_dale7.mjs — guion + wordms -> _v3/dale7_moments.json  [{t, txt, i, dur, sec}]
//
//   node scripts/moments_dale7.mjs [slug]
//
...
const SLUG = process.argv[2] || "dale9";
const MIN_S = 1.9;    // por debajo -> se pega al vecino
const MAX_S = 6.6;    // por encima -> se parte en la coma mas centrada
...
// dale9 NO es una lista: es la lectura guiada de UNA pagina. Se corta por las frases bisagra
// del guion, en orden. La primera que aparece manda de ahi en adelante.
const CORTES = [
  ["INTRO",      /^My name is Dale Kessler/],
  ["PAGINA",     /^So let us go find it/],
  ["PALABRA",    /^Now read it again/],
  ["ARITMETICA", /^The second thing about that line/],
  ["PORQUE",     /^Now, a fair question/],
  ["NOCHE",      /^So here is the evening/],
  ["CIERRE",     /^I want to finish where I started/],
];
let sec = "HOOK";
let k = 0;
for (const m of mom) {
