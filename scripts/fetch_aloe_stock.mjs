// fetch_water_stock.mjs — descarga stock (Pexels→Pixabay→Archive) para la cama densa de aloefiller.
// Lee public/broll/dense_thinnedA.json [{name, query, t}] → public/broll/aloefiller/<name>.mp4 (aislado por slug).
import fs from "fs";
import { pexelsVideo, pixabayVideo, archiveVideo, usedRegistry } from "./stock_lib.mjs";

const items = JSON.parse(fs.readFileSync("public/broll/dense_thinnedA.json", "utf8").replace(/^﻿/, ""));
const outDir = "public/broll/aloefiller";
...
console.log(`\n=== stock aloefiller: ${ok} OK · ${miss} MISS de ${items.length} ===`);
