// dense_prep19.mjs — toma public/broll/dense_federer19.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinned19.json   [{name, at, query, t}]
//   public/broll/shots_dense19.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/federer19_broll.ts  (FED19_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_federer19.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned19.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_dense19.json", JSON.stringify(shots, null, 1));

const VEND = (CW[CW.length - 1]?.s || 920) + 2;
const broll = kept.map((k, i) => ({
  name: k.name, src: `broll/federer19/${k.name}.mp4`,
  start: k.t, dur: +(((i + 1 < kept.length ? kept[i + 1].t : VEND) - k.t)).toFixed(2),
  query: k.query,
}));
fs.writeFileSync("src/_fed6/VideoEdit/federer19_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep19.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const FED19_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
