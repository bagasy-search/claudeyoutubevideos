...
def qr_card(qr: Image.Image) -> Image.Image:
    ...
    draw.text((82, 200), "QUE CONSUME", font=font(DISPLAY, 96), fill=PAPER)
    draw.text((82, 302), "TU CASA SOLA", font=font(DISPLAY, 96), fill=RED)
    copy = ("Los sesenta aparatos de una casa ya medidos uno por uno, las hojas para "
            "imprimir y pegar en el tablero, y como leer la factura renglon por renglon.")
...
def main() -> None:
    ...
    assets["qr_card"] = save_with_blur(qr_card(qr), "pinvacas_cta_qr")
    assets["pages"] = save_with_blur(pages_card(), "pinvacas_cta_pages")

    checks = {
        "full_card": decode(Image.open(OUT / "pinvacas_cta_qr.png")),
        "card_a_tamano_de_pantalla": decode_como_en_pantalla(OUT / "pinvacas_cta_qr.png"),
    }
    ...
    manifest = {
        ...
        "promesa_del_guion": "la hoja de los sesenta aparatos ya medidos, la tabla para imprimir y pegar en el tablero, y como leer la factura renglon por renglon",
        "cumple": "la landing publica los 60 aparatos medidos uno por uno, las hojas rapidas y tablas para imprimir, y el capitulo Como leer la factura sin perderte entre cargos",
        "qr_checks": checks,
        "assets": assets,
    }
    (OUT / "pinvacas_cta_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n",
                                                      encoding="utf-8")
