/**
 * anchor_quesos.mjs — alineación GLOBAL guion↔ASR (SequenceMatcher-style) para el video limonayunas.
 * Lee public/limonayunas_guion.txt + public/captions_limonayunas.json.
 * Resuelve el FRAME (30fps) de cada frase-ancla y escribe _v3/limonayunas_anchors.json.
 */
import fs from 'fs';
...
const guion = fs.readFileSync('public/limonayunas_guion.txt', 'utf8');
const caps = JSON.parse(fs.readFileSync('public/captions_limonayunas.json', 'utf8'));
...
fs.writeFileSync('_v3/limonayunas_anchors.json', JSON.stringify(out, null, 2));
...
// mapa plano label->frame para el Main
const flat = {};
for (const [k, v] of Object.entries(out)) flat[k] = v ? v.frame : null;
fs.writeFileSync('_v3/limonayunas_frames.json', JSON.stringify(flat, null, 1));
console.log('flat frames -> _v3/limonayunas_frames.json');
