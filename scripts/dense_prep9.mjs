// dense_prep9.mjs — toma public/broll/dense_federer9.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinned9.json   [{name, at, query, t}]
//   public/broll/shots_dense9.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/federer9_broll.ts  (FED9_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_federer9.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned9.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_dense9.json", JSON.stringify(shots, null, 1));
...
fs.writeFileSync("src/_fed6/VideoEdit/federer9_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep9.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const FED9_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
