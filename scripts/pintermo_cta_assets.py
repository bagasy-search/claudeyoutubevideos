# -*- coding: utf-8 -*-
"""CTA de `pintermo` (canal La Pinza de Claudio) -> public/img/pintermo/
...
URL = "https://claudiomendoza.vercel.app/?src=yt-pin5"
DOMINIO = "claudiomendoza.vercel.app"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "public" / "img" / "pintermo"
...
def qr_card(qr: Image.Image) -> Image.Image:
    ...
    copy = ("La tabla de consumos de una casa entera, aparato por aparato, con los vatios "
            "medidos y las horas reales de uso.")
...
def main() -> None:
    ...
    assets["qr_card"] = save_with_blur(qr_card(qr), "pintermo_cta_qr")
    assets["pages"] = save_with_blur(pages_card(), "pintermo_cta_pages")

    checks = {
        "full_card": decode(Image.open(OUT / "pintermo_cta_qr.png")),
        "card_a_tamano_de_pantalla": decode_como_en_pantalla(OUT / "pintermo_cta_qr.png"),
    }
    ...
    (OUT / "pintermo_cta_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n",
                                                      encoding="utf-8")
