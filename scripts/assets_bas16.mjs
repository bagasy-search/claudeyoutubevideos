// assets_bas16.mjs — arma _bastidarenal16_assets.txt (mismo patrón que bas11).
import fs from 'node:fs';
import path from 'node:path';
const found = new Set();
// 1) TODOS los assets del slug
for (const f of fs.readdirSync('public/img')) if (/^bas16_.*\.(jpg|png)$/.test(f)) found.add(`img/${f}`);
if (fs.existsSync('public/broll')) for (const f of fs.readdirSync('public/broll')) if (/^bas16_.*\.mp4$/.test(f)) found.add(`broll/${f}`);
// 2) fijos del montaje + reusados EXPLÍCITOS (no salen de staticFile literal)
['renal/avatar16.mp4', 'renal/bas_qr_bastida.png', 'renal/bastida_cutout.png',
...
const main = 'src/bastida/Main_bastidarenal16.tsx';
...
fs.writeFileSync('_bastidarenal16_assets.txt', ok.join('\n') + '\n');
console.log(`assets: ${ok.length} · _bastidarenal16_assets.txt`);
