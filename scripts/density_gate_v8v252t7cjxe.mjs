...
// 1) ubicar el build
const candidates = [
  `src/_fed6/VideoEdit/Main_${slug}.tsx`, `src/VideoEdit/Main_${slug}.tsx`,
  `src/VideoEdit/Main_${slug}_redo.tsx`,
];
const build = candidates.find((p) => existsSync(p)) || readdirSync("src/_fed6/VideoEdit").filter((f) => f.includes(slug) && /^Main_.*\.tsx$/.test(f)).map((f) => `src/_fed6/VideoEdit/${f}`)[0];
...
// Estilo _fed6: el track NO es JSX, es DATA (federer_<slug>_beats.ts + _broll.ts) y el Main
// instancia cada componente UNA vez dentro de un switch. Leyendo el Main a secas el gate contaba
// 14 usos donde hay cientos. `_synth_<slug>.tsx` (lo escribe _gate_prologo.mjs) es la vista
// sintética del track: un <Componente> por beat REAL y una ruta por cada asset REAL.
const synthPath = `_synth_${slug}.txt`;
const haySynth = existsSync(synthPath);
const src = [
  (hayCues || haySynth) ? sinComentarios(readFileSync(build, "utf8")) : readFileSync(build, "utf8"),
  hayCues ? readFileSync(cuesPath, "utf8") : "",
  haySynth ? readFileSync(synthPath, "utf8") : "",
].join("\n");
