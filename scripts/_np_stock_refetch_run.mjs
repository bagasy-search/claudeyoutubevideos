...
const items = JSON.parse(fs.readFileSync("scripts/_np_stock_refetch.json", "utf8").replace(/^\uFEFF/, ""));
const okMoments = [];
...
fs.writeFileSync("_np_stock_refetch_moments.json", JSON.stringify(okMoments, null, 1));
fs.writeFileSync("scripts/_np_stock_refetch_miss.json", JSON.stringify(missList, null, 1));
console.log(`\n=== nightprotein stock: ${ok} OK · ${miss} MISS · ${dark} dark · ${badfps} badfps de ${items.length} ===`);
console.log(`moments -> _np_stock_refetch_moments.json (${okMoments.length})`);
