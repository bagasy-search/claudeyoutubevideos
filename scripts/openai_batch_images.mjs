...
if (cmd === "submit") {
  ...
  const lines = pend.map((it) => {
    ...
    for (const r of refs) if (!refCache.has(r)) refCache.set(r, dataUri(r));
    return JSON.stringify({
      custom_id: it.name, method: "POST", url: "/v1/images/edits",
      body: { model: "gpt-image-2", prompt: it.prompt, size: SIZE, quality: QUALITY, n: 1,
        images: refs.map((r) => ({ image_url: refCache.get(r) })) },
    });
  });
  ...
  const b = await api("/batches", { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ input_file_id: file.id, endpoint: "/v1/images/edits", completion_window: "24h" }) });
