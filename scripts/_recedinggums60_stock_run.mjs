// _recedinggums60_stock_run.mjs — clon del runner nightprotein PROBADO.
// Baja stock Pexels (2 claves) → reencode CFR 30/1 1280x720 → descarta near-black.
// Sale a public/broll/ con nombres st_<name> (prefijo = anti-colisión duro del repo).
import fs from "fs";
...
const TMP = "public/broll/_rg60_tmp";
...
// Windows file-lock safe delete (ffmpeg/AV may still hold the mp4 for a beat).
const sleepSync = (ms) => { try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms); } catch {} };
function safeRm(p) {
  for (let a = 0; a < 8; a++) {
    try { fs.rmSync(p, { force: true, maxRetries: 5, retryDelay: 300 }); return; }
    catch { sleepSync(600); }
  }
}
...
const items = JSON.parse(fs.readFileSync("scripts/_recedinggums60_stock_src.json", "utf8").replace(/^\uFEFF/, ""));
const okMoments = [];
  ...
  for (const c of cands.slice(0, 5)) {
    if (await dl(c.link, tmp)) {
      if (reencode(tmp, dest)) {
        const y = meanY(dest);
        if (y < 25) { safeRm(dest); dark++; process.stdout.write(`[${i + 1}] DARK ${b.n} Y=${y.toFixed(0)} (try next)\n`); continue; }
        const fps = fpsOf(dest);
        if (fps !== "30/1") { safeRm(dest); badfps++; process.stdout.write(`[${i + 1}] BADFPS ${b.n} ${fps}\n`); continue; }
        used.add(c.key);
        okMoments.push({ p: b.p, t: "clip", n: b.n }); ok++;
...
fs.writeFileSync("_recedinggums60_stock_moments.json", JSON.stringify(okMoments, null, 1));
fs.writeFileSync("scripts/_recedinggums60_stock_miss.json", JSON.stringify(missList, null, 1));
try { fs.rmSync(TMP, { recursive: true, force: true }); } catch {}
console.log(`\n=== recedinggums60 stock: ${ok} OK · ${miss} MISS · ${dark} dark · ${badfps} badfps de ${items.length} ===`);
console.log(`moments -> _recedinggums60_stock_moments.json (${okMoments.length})`);
