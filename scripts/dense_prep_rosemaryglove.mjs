// dense_prep10.mjs — toma public/broll/dense_rosemaryglove.json ({at,query}) → resuelve cada
...
//   src/_fed6/VideoEdit/rosemaryglove_broll.ts  (ROSE_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_rosemaryglove.json", "utf8"));
...
const broll = kept.map((k, i) => ({
  name: k.name, src: `broll/rosemaryglove/${k.name}.mp4`,
  start: k.t, dur: +(((i + 1 < kept.length ? kept[i + 1].t : VEND) - k.t)).toFixed(2),
  query: k.query,
}));
fs.writeFileSync("src/_fed6/VideoEdit/rosemaryglove_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep10.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const ROSE_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
