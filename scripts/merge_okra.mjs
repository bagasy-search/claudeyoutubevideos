// merge_okra.mjs — une los 6 planes Haiku (plan_okra_1..6.json), ancla cada
// beat al ms de captions_okra.json (findMs sobre "at"), ordena, deduplica y
// DENSIFICA (rellena huecos de b-roll >4s con clips on-topic por sección de TIEMPO).
// Escribe public/plan_okra.json. Canal Dr. Federer — video ARÁNDANO/OJOS.
import fs from "fs";
const caps = JSON.parse(fs.readFileSync("public/captions_okra.json", "utf8"));
...
let raw = [];
for (let i = 1; i <= 6; i++) {
  const f = `public/plan_okra_${i}.json`;
...
fs.writeFileSync("public/plan_okra.json", JSON.stringify(withFill, null, 1));
