// dense_prep11.mjs — toma public/broll/dense_federer11.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinned11.json   [{name, at, query, t}]
//   public/broll/shots_dense11.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/federer11_broll.ts  (FED11_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
const caps = JSON.parse(fs.readFileSync("public/captions_federer11.json", "utf8"));
const norm = (s) => s.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z0-9 ]/g, " ").replace(/\s+/g, " ").trim();
const CW = caps.map((x) => ({ t: norm(x.text), s: (x.startMs || 0) / 1100 }));
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_federer11.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned11.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_dense11.json", JSON.stringify(shots, null, 1));
...
fs.writeFileSync("src/_fed6/VideoEdit/federer11_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep11.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const FED11_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
