// moments_dale5.mjs — guion + wordms -> _v3/dale5_moments.json  [{t, txt, i, dur, sec}]
//
//   node scripts/moments_dale5.mjs [slug]
//
...
const SLUG = process.argv[2] || "dale5";
...
// ── 4) seccion (para agrupar los prompts) ────────────────────────────────────────────────────────
// Este video NO es una lista: no hay "Number N." que marque secciones. Se corta por las frases
// bisagra del guion, en orden. La primera que aparece manda de ahi en adelante.
const CORTES = [
  ["INTRO",   /^My name is Dale Kessler/],
  ["DRAWER",  /^So let us start with the drawer/],
  ["WHYROOM", /^Now let us do the living room question/],
  ["NINETY",  /^All right\. The ninety seconds/],
  ["FOUND",   /^So what did I find in there/],
  ["POLICY",  /^And now the part of this that is actually my job/],
  ["DO",      /^So here is what I would do about all of this/],
  ["CIERRE",  /^I want to close with a file/],
];
let sec = "HOOK";
let k = 0;
for (const m of mom) {
  while (k < CORTES.length && CORTES[k][1].test(m.txt)) { sec = CORTES[k][0]; k++; }
  m.sec = sec;
}
if (k < CORTES.length) console.warn("⚠ no encontre las bisagras:", CORTES.slice(k).map((c) => c[0]).join(", "));
