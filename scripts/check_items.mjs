...
function contrato(comp) {
  ...
  const cand = [`src/${KIT}/${comp}.tsx`, `src/${KIT}/RayStage.tsx`,
  ...
  const out = { file, arrays: [...arrays], campos };
  cache.set(comp, out);
  return out;
}
...
for (const u of usos) {
  ...
  for (const [prop, valor] of Object.entries(u.props)) {
    ...
    valor.forEach((el, k) => {
      elementosMedidos++;
      // basta con que traiga AL MENOS el campo principal (el primero que el JSX lee)
