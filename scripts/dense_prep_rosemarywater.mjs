// dense_prep_rosemarywater.mjs — toma public/broll/dense_rosemarywater.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinnedW.json   [{name, at, query, t}]
//   public/broll/shots_denseW.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/rosemarywater_broll.ts  (WATER_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_rosemarywater.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinnedW.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_denseW.json", JSON.stringify(shots, null, 1));

const VEND = (CW[CW.length - 1]?.s || 920) + 2;
const broll = kept.map((k, i) => ({
  name: k.name, src: `broll/rosemarywater/${k.name}.mp4`,
  start: k.t, dur: +(((i + 1 < kept.length ? kept[i + 1].t : VEND) - k.t)).toFixed(2),
  query: k.query,
}));
fs.writeFileSync("src/_fed6/VideoEdit/rosemarywater_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep_rosemarywater.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const WATER_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
