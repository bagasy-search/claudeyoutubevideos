// fetch_handsage_stock.mjs — descarga stock (Pexels→Pixabay→Archive) para la cama densa de handsage.
// Lee public/broll/dense_thinnedH.json [{name, query, t}] → public/broll/handsage/<name>.mp4 (aislado por slug).
import fs from "fs";
import { pexelsVideo, pixabayVideo, archiveVideo, usedRegistry } from "./stock_lib.mjs";

const items = JSON.parse(fs.readFileSync("public/broll/dense_thinnedH.json", "utf8").replace(/^﻿/, ""));
...
fs.writeFileSync("public/broll/handsage_miss.json", JSON.stringify(missList, null, 1));
console.log(`\n=== stock handsage: ${ok} OK · ${miss} MISS de ${items.length} ===`);
