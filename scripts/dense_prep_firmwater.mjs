// dense_prep_firmwater.mjs — toma public/broll/dense_firmwater.json ({at,query}) → resuelve cada
// 'at' al ms del caption, afina a ~2.3s de separación, asigna nombres dNNN y escribe:
//   public/broll/dense_thinned_fw.json   [{name, at, query, t}]
//   public/broll/shots_dense_fw.json     [{name, query, type:"video", orientation:"landscape"}]  (para fetchstock)
//   src/_fed6/VideoEdit/firmwater_broll.ts  (FW_BROLL, track contiguo)
import fs from "fs";
const MINGAP = 2.2;
...
const dense = JSON.parse(fs.readFileSync("public/broll/dense_firmwater.json", "utf8"));
...
fs.writeFileSync("public/broll/dense_thinned_fw.json", JSON.stringify(kept, null, 1));
fs.writeFileSync("public/broll/shots_dense_fw.json", JSON.stringify(shots, null, 1));

const VEND = (CW[CW.length - 1]?.s || 1208) + 2;
const broll = kept.map((k, i) => ({
  name: k.name, src: `broll/firmwater/${k.name}.mp4`,
  start: k.t, dur: +(((i + 1 < kept.length ? kept[i + 1].t : VEND) - k.t)).toFixed(2),
  query: k.query,
}));
fs.writeFileSync("src/_fed6/VideoEdit/firmwater_broll.ts",
  `// AUTO-GENERADO por scripts/dense_prep_firmwater.mjs — b-roll denso (footage real cada ~2.3s).\n` +
  `export const FW_BROLL: { name: string; src: string; start: number; dur: number; query: string }[] = ${JSON.stringify(broll)};\n`);
