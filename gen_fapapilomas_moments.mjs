// gen_fapapilomas_moments.mjs — parte el GUION en momentos y (si ya existe el mapa) los ANCLA al ms.
// Clon de gen_fa70estudios_moments.mjs. 100 % Fish + RunPod por ventanas (UN /run).
//   node gen_fcsmanos10_moments.mjs   # sin _v3/fcscanas_wordms.json: momentos SIN ms · con él: + ms real
import fs from "node:fs";

const SLUG = "fapapilomas";
...
const ABRE = [
  ...
  ["Casi todas estas bolitas", "TIPOS"],
  ["Y la regla, que es lo más importante", "REGLA"],
  ["Ya. Con eso tienes lo principal.", "LOOPS"],
  ["Déjame presentarme rapidito", "INTRO"],
  ["Ahora sí, déjame contarte de alguien.", "ROSA"],
  ["Y ahí la dejo un momento", "MECANISMO"],
  ["Pero ese no es el error del que te hablé.", "ERROR"],
  ["Y ahora te quiero enseñar algo.", "LAMINA"],
  ["Y dicho sea de paso", "GUIA"],
  ["Bueno. Te prometí regresar con doña Rosa.", "ROSA2"],
  ["Por eso te lo repito.", "PATITO"],
  ["Y ahora vamos por partes", "ACRO"],
  ["Y ahora sí, la variante que te prometí.", "PARPADO"],
  ["Ahora, la queratosis seborreica.", "QUERA"],
  ["Bueno. Y ahora te quiero hablar de los remedios.", "REMEDIOS"],
  ["Y aquí me pasó algo que te quiero contar.", "CTA2"],
  ["Y ya que estamos, déjame contestarte", "MITOS"],
  ["Ahora, ¿cómo se quitan de verdad?", "QUITAR"],
  ["Ahora sí. Lo que sí puedes hacer en casa", "CASA"],
  ["Y cuarto, el que te prometí.", "FOTO"],
  ["Déjame repasarte, rapidito", "SENALES"],
  ["Y ya casi para terminar", "ROSA3"],
  ["Te dejo un aviso", "AVISO"],
  ["Y sobre la guía.", "GUIA3"],
