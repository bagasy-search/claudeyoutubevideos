...
const SLUG = "fedvet4", COMP = "Fedvet4", UP = "FEDVET4";
const FPS = 30;
...
const faltan = [];
// ⛔⛔ ESTE WALKER SOLO MIRABA `img/` y `broll/`: el QR del CTA vive en `med/fedvet4_qr.png`
//    (prop `qr` de RayCta, que ademas va en overlays[]) y NO entraba en _<slug>_assets.txt.
//    En el farm eso es un 404 en los 60 chunks — la misma mina que el .m4a. Ahora se acepta
//    CUALQUIER ruta relativa con extension de medio, y si no existe en public/ se reporta.
const MEDIA = /^[a-z0-9_\-]+(\/[a-z0-9_\-.]+)+\.(png|jpe?g|webp|mp4|m4a|wav|svg)$/i;
const scanProps = (v) => {
  if (typeof v === "string") {
    if (MEDIA.test(v)) { if (existe(v)) assets.add(v); else faltan.push(v); }
    return;
  }
...
for (const b of rows) {
  ...
  if (b.tipo === "clip") {
    ...
    scanProps(b.props);
    cues.push({ key, start, dur, el: `(d) => <${b.componente} durationInFrames={d} {...(${JSON.stringify(b.props || {})})} />` });
  }
}
...
for (const o of overlays) {
  ...
  ovCues.push({ key: `ov_${o.ms_in}`, start: f0 / FPS, dur: (f1 - f0) / FPS,
    el: `(d) => <${o.componente} durationInFrames={d} {...(${JSON.stringify(o.props || {})})} />` });
}
...
{
  const malos = [];
  // La compuerta de aspecto es para el B-ROLL, que se dibuja con objectFit:"cover" y se recorta.
  // Lo de `med/` son assets de INTERFAZ (el QR del CTA) y van con objectFit:"contain" dentro de una
  // caja chica: son cuadrados a proposito y quedan fuera.
  for (const rel of [...assets].filter((a) => /\.(png|jpe?g)$/i.test(a) && !/^med\//i.test(a) && !/_qrcard\.png$/i.test(a))) {
...
{
  const KIT = "fedvet";   // ⛔ el kit de ESTE canal es src/fedvet (clon repintado de src/dale).
