...
//   canal Federer Consejos Salud · "MANCHAS en las MANOS: 10 Noches" · tramo 1 avatar v3 del creador + tramo 2 Fish/RunPod
//
...
const QR = `img/${SLUG}_qrcard.png`, DOM = "archivos-federer.vercel.app";
...
const assetDe = (m) => {
  const s = SPEC[m.name];
  // agnes i2v: la foto animada vive en broll/<slug>/<name>.mp4 — se prefiere SIEMPRE sobre la foto quieta
  // (regla: TODA foto que se monte va animada). check_arbol_git / agnes_qc ya validaron estos clips.
  const anim = `broll/${SLUG}/${m.name}.mp4`;
  if (existe(anim)) return { tipo: "clip", src: anim };
  if (s?.t === "pres") {
...
for (let i = 0; i < MOM.length; i++) {
  ...
  if (c && !c.overlay) {
    ...
    // ⭐ EXCEPCIÓN: FedWhiteboard NO es una tarjeta, es una escena que se DIBUJA (sus elementos tienen
    //    `start` hasta 16,6 s en src/fcsjuanetes/scenes.ts). Con el techo de 11 s se cortaba a la mitad y
    //    la conclusión nunca aparecía. Techo propio de 20 s, siempre dentro del hueco sin avatar.
    const WB = c.comp === "FedWhiteboard";
    const TECHO = WB ? 20 : 11;
    const piso = WB ? 18 : Math.min(11, 2.8 + 0.28 * Math.max(0, palabras(c.props) - 3));
    let out = Math.min(t0 + Math.min(TECHO, Math.max(m.dur, piso)), proxAvatar(t0 + 0.1));
    ...
    // FedWhiteboard es pizarra full-bleed: no toma `bed` (su firma es {scene?, theme?}) y el build
    // lo emite como caso especial sin reenviar props. Metérselo deja el gate de contratos en rojo.
    const sinCama = c.comp === "FedWhiteboard";
    beats.push({ tipo: "componente", ms_in: Math.round(t0 * 1000), ms_out: Math.round(out * 1000), componente: c.comp, props: { ...c.props, ...(bed && !sinCama ? { bed } : {}) } });
    continue;
  }
...
// ── CTA overlay (QR real). Única mención explícita de la guía + URL en el guion.
for (const [frag, props] of [
  ["La encuentras en archivos-federer", { eyebrow: "ARCHIVOS-FEDERER.VERCEL.APP", title: "La Guía Completa de la Salud Después de los 60",
    sub: "Más de 150 remedios y rutinas caseras, explicadas con calma. $27.", domain: DOM, qr: QR, showQr: false }],
  ["ilustrados paso a paso en mi guía", { eyebrow: "ARCHIVOS-FEDERER.VERCEL.APP", title: "Los 4 ejercicios, ilustrados paso a paso",
    sub: "Todo lo del video está gratis aquí. La guía es para tenerlo a la mano.", domain: DOM, qr: QR, showQr: false }],
  ["cuesta veintisiete dólares", { eyebrow: "ARCHIVOS-FEDERER.VERCEL.APP", title: "La Guía Completa de la Salud Después de los 60",
    sub: "Más de 150 remedios caseros. $27. Complementa a tu médico, no lo reemplaza.", domain: DOM, qr: QR, showQr: true }],
]) {
  const k = buscar(frag);
...
// ── SIN LOOPS Y SIN CLIPS REPETIDOS (lo que exige scripts/agnes_qc_gate.mjs, y con razón)
// `Clip` envuelve el video en <Loop>: un plano más largo que su mp4 REINICIA el clip a la vista
// (los de agnes duran 4,03 s), y un mismo clip en dos planos se nota como material reciclado.
// Arreglo: el plano se queda con el clip por lo que el clip dura, y el resto (o el uso repetido)
// pasa a la FOTO fija de ese mismo momento, animada por <Foto punch> (Ken Burns).
// ⚠ Las fotos de este slug son .png (1088x608); el `assetDe` de arriba busca .jpg y por eso el
//   plan salía con `imagen 0` y TODO como clip: de ahí venían los 200 loops.
{
  const EPS = 2 / 30;
  const usados = new Set();
  const fotoDe = (src) => {
    const n = src.split("/").pop().replace(/\.mp4$/, "");
    for (const ext of [".png", ".jpg"]) if (existe(`img/${SLUG}/${n}${ext}`)) return `img/${SLUG}/${n}${ext}`;
    return null;
  };
  // ⭐ Para la COLA de un plano recortado NO sirve la foto original: el clip de agnes es la
  //    animación i2v DE ESA MISMA foto, así que cortar del final del clip a la foto vuelve al
  //    cuadro inicial — exactamente el salto del <Loop> que estamos sacando. La cola usa el
  //    ÚLTIMO cuadro del clip, así el movimiento se congela donde quedó y Ken Burns lo sigue.
  const colaDe = (src) => {
    const n = src.split("/").pop().replace(/\.mp4$/, "");
    const out = `${bedDir}/${n}_fin.jpg`;
    if (!existe(out)) {
      const d = durDe(src);
      spawnSync("ffmpeg", ["-v", "error", "-y", "-sseof", "-0.1", "-i", path.join("public", src), "-frames:v", "1", "-q:v", "3", path.join("public", out)]);
      if (!existe(out)) spawnSync("ffmpeg", ["-v", "error", "-y", "-ss", String(Math.max(0, d - 0.1)), "-i", path.join("public", src), "-frames:v", "1", "-q:v", "3", path.join("public", out)]);
    }
    return existe(out) ? out : null;
  };
  const out = [];
  let recortadosLoop = 0, aFoto = 0, sinFoto = 0;
  for (const b of beats) {
    if (b.tipo !== "clip") { out.push(b); continue; }
    const d = (b.ms_out - b.ms_in) / 1000;
    const cd = durDe(b.src) - (b.startFrom || 0);
    const foto = fotoDe(b.src);
    const repetido = usados.has(b.src);
    if (!repetido && d <= cd) { usados.add(b.src); out.push(b); continue; }
    if (!foto) { sinFoto++; usados.add(b.src); out.push(b); continue; }   // se reporta abajo
    if (repetido) { out.push({ tipo: "imagen", ms_in: b.ms_in, ms_out: b.ms_out, src: foto, name: b.name, punch: true }); aFoto++; continue; }
    // no repetido pero LARGO: el clip se queda con lo suyo y la foto cierra el resto
    usados.add(b.src);
    // la cola de foto nunca baja de 0,5 s: si el excedente es de 0,07 s, un beat de 2 cuadros
    // no se ve y encima deja el plano de clip 0,07 s por encima de su mp4 (el loop vuelve).
    const cola = Math.max(0.5, d - cd);
    const corte = b.ms_out - Math.round(cola * 1000);
    out.push({ ...b, ms_out: corte });
    out.push({ tipo: "imagen", ms_in: corte, ms_out: b.ms_out, src: colaDe(b.src) || foto, name: b.name });
    recortadosLoop++;
  }
  beats.length = 0; beats.push(...out);
  console.log(`sin loops: ${recortadosLoop} planos recortados al largo de su clip (+foto de cierre) · ${aFoto} usos repetidos pasados a foto · ${sinFoto} sin foto (quedan como estaban)`);
}
