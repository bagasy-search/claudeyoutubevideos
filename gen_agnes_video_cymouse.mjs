...
const COOLDOWN = 62_000;      // MEDIDO 22-ago-2026 en ESTE video: la API ahora responde
                              // "allows 1 requests per 1 minute(s)" POR CLAVE (antes eran 2).
                              // Con 31s la mitad de los envios se iban en 429: el item no se
                              // perdia (vuelve a la cola) pero se quemaba media hora de techo.
  ...
  try {
    ...
    if (!vid) {
      ...
      if (queueFull || rateLimited) {
        ...
        if ((qfull + rlim) % 20 === 0) console.log(`  … esperando cupo (cola-llena ${qfull}, rate-limit ${rlim}, en vuelo ${inflight.size}, ok ${ok})`);
        return;
      }
