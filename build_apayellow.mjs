// build_apayellow.mjs — MONTAJE de `apayellow` (The Amish Pest Almanac, EN/US).
//   "Destroying a Dangerous Yellow Jacket Nest With Soapy Water"
//
//   node _v3/apayellow_plan.mjs       # DIRECTOR (4 pasadas) + wordms -> _v3/apayellow_plan.json
//   node _v3/apayellow_swapstock.mjs  # sube el metraje real donde el sustantivo coincide
//   node build_apayellow.mjs          # plan -> cues + Main + index + _apayellow_assets.txt
//
// Lee   _v3/apayellow_plan.json  (beats base + overlays, anclados al ms del MASTER)
// Emite src/VideoEdit/{cues_apayellow.gen.tsx, avatar_apayellow.gen.ts, Main_apayellow.tsx}
//     + src/index_apayellow.tsx + _apayellow_assets.txt
//
// ⛔ LO QUE NO SE TOCA (cada linea costo un render):
//  · El AVATAR es el FONDO GARANTIZADO: base FULL SIEMPRE. Es PARCIAL (0..720,96s = 45,3% del
//    guion) -> BUCLE muteado de 33,5s para la cola. El audio de TODO el video es UN solo
//    <Audio> con el master (avatar real + cola de Fish clonada de el mismo).
//  · OffthreadVideo en TODOS lados (avatar + clips). NUNCA el elemento de video legacy.
//  · COMPUERTA DE FPS: todo clip y el avatar a 30/1 CFR o hay TIRON en todo el metraje.
//  · Los overlays (CornerLabel) van ENCIMA, no ocultan la base.
//  · El Stage del kit Amish es TRANSPARENTE: el avatar/bucle ES la cama de cada componente.
import fs from "node:fs";
import path from "node:path";
import { execFileSync } from "node:child_process";

const SLUG = "apayellow", COMP = "Apayellow", UP = "APAYELLOW";
const FPS = 30;
...
// frames REALES de un clip: si el slot dura mas que el clip, ApaClip lo loopea en vez de congelarse
const loopFCache = new Map();
const loopF = (rel) => {
  if (loopFCache.has(rel)) return loopFCache.get(rel);
  const d = durDe(rel);
  const n = Math.max(1, Math.round(d * FPS) - 1);
  loopFCache.set(rel, n); return n;
};
...
// ⛔ AVATAR PARCIAL: el creador grabó 0..347,925s. Después entra el BUCLE, así que hay FONDO
//    GARANTIZADO en todo el metraje (a diferencia de fedvet2, que era narrador puro).
const AVATAR_FILE = `${SLUG}_opt.mp4`;
const LOOP_FILE = `${SLUG}_loop.mp4`;
if (!existe(AVATAR_FILE)) { console.error(`⛔ falta public/${AVATAR_FILE}`); process.exit(1); }
if (!existe(LOOP_FILE)) { console.error(`⛔ falta public/${LOOP_FILE} (el bucle de la cola)`); process.exit(1); }
const AVATAR_S = durDe(AVATAR_FILE);
const LOOP_S = durDe(LOOP_FILE);
const TOTAL_S = Math.max(totalMs / 1000, WAV_S) + 0.5;
const TOTAL_FRAMES = Math.ceil(TOTAL_S * FPS);
const AVATAR_FRAMES = Math.round(AVATAR_S * FPS);   // tramo con el avatar REAL
const LOOP_FRAMES = Math.round(LOOP_S * FPS);       // largo del bucle de la cola
...
const faltan = [];
const scanProps = (v) => {
  // ⛔⛔ `med/` VA INCLUIDO. El QR del CTA vive en `med/<slug>_qr.png` y el patrón heredado sólo
  //    miraba `img/` y `broll/`: el QR no viajó en el tar y los 5 chunks que contienen las tarjetas
  //    de CTA murieron con `Error loading image with src: .../med/fedvet7_qr.png` (404). Misma
  //    familia que la mina de mdmold: un asset que vive DENTRO de props no lo ve un escaneo de beats.
  if (typeof v === "string") { if (/^(img|med)\/.+\.(png|jpe?g)$/i.test(v) || /^broll\/.+\.mp4$/i.test(v)) assets.add(v); return; }
...
for (const b of rows) {
  ...
  if (b.tipo === "clip") {
    ...
    cues.push({ key, start, dur, el: `(d) => <ApaClip src=${JSON.stringify(r)} loopFrames={${loopF(r)}} />` });
  } else if (b.tipo === "clipslow") {
    // clip de ~3 s reproducido a 0,5x -> 6 s en pantalla. Movimiento en todo el video sin repetir.
    const r = `broll/${b.clip}.mp4`;
    if (!existe(r)) faltan.push(r); else assets.add(r);
    cues.push({ key, start, dur, el: `(d) => <ApaClip src=${JSON.stringify(r)} rate={${b.rate || 0.5}} loopFrames={${loopF(r)}} />` });
...
{
  const malos = [];
  // ⛔ el AVATAR y el BUCLE entran a la compuerta: si no van a 30/1 CFR, tiembla TODO el metraje
  for (const rel of [...assets].filter((a) => a.endsWith(".mp4")).concat([AVATAR_FILE, LOOP_FILE])) {
  ...
  console.log(`fps ✓ ${[...assets].filter((a) => a.endsWith(".mp4")).length + 2} videos a ${FPS}/1 CFR (incluye avatar y bucle)`);
}
...
// ── COMPUERTA DE ASPECTO (16:9 ± tolerancia) sobre las imágenes usadas ──────────────────────────
{
  ...
  for (const rel of [...assets].filter((a) => /\.(png|jpe?g)$/i.test(a) && !/_qrcard\d*\.png$/i.test(a) && !/(^|\/)qr[_-]|_qr\.png$/i.test(a))) {
...
// Acá el avatar (real + bucle) ES el fondo garantizado, así que un hueco muestra al presentador,
// no pantalla plana. El piso duro es el de video-pipeline (≥90%), no el 98% del narrador puro.
if (COBER < 90) { console.error(`⛔ cobertura ${COBER.toFixed(1)}% — por debajo del 90% el avatar queda demasiado solo.`); process.exit(1); }
{ // ningún hueco largo: listar los ≥10s para poder mirarlos
  const huecos = []; let x = 0;
  while (x < TOTAL_FRAMES) {
    if (covered[x]) { x++; continue; }
    let y = x; while (y < TOTAL_FRAMES && !covered[y]) y++;
    if ((y - x) / FPS >= 10) huecos.push(`${(x / FPS).toFixed(1)}s (${((y - x) / FPS).toFixed(1)}s)`);
    x = y;
  }
  console.log(huecos.length ? `⚠️ huecos ≥10s de avatar solo: ${huecos.join(" · ")}` : `huecos ≥10s: 0 ✓`);
}
...
const fromStage = ["ApaClip", "ApaFoto"].concat([...usedComps].filter((c) => STAGE.has(c)));
...
export const Main${COMP}: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: "#1B1408" }}>
    {/* AVATAR = FONDO GARANTIZADO, base FULL siempre. Real 0..${AVATAR_FRAMES}f, bucle después.
    ...
    <Sequence from={0} durationInFrames={${AVATAR_FRAMES}} layout="none">
      <AbsoluteFill><ApaAvatar src="${AVATAR_FILE}" loopFrames={${AVATAR_FRAMES}} /></AbsoluteFill>
    </Sequence>
    <Sequence from={${AVATAR_FRAMES}} durationInFrames={Math.max(1, TOTAL_FRAMES_${UP} - ${AVATAR_FRAMES})} layout="none">
      <AbsoluteFill><ApaAvatar src="${LOOP_FILE}" loopFrames={${LOOP_FRAMES}} /></AbsoluteFill>
    </Sequence>
...
{
  const KIT = "apa";   // ⛔ el kit de ESTE canal es src/fedvet (clon repintado de src/dale).
  //    dale2 heredo "rksafe" del script padre y la compuerta escaneaba una carpeta que el video no usa.
...
conBlur.push(AVATAR_FILE, LOOP_FILE);   // el avatar parcial y el bucle van en el tar
