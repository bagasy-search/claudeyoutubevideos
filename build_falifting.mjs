// build_falifting.mjs — MONTAJE (plan → cues + Main + index + assets list) + COMPUERTAS.
//   node gen_falifting_plan.mjs && node build_falifting.mjs
import fs from "node:fs";
import path from "node:path";
import { execFileSync, spawnSync } from "node:child_process";

const SLUG = "falifting", COMP = "Falifting", UP = "FALIFTING", FPS = 30;
...
for (const b of rows) {
  ...
  if (b.tipo === "avatar") {
    ...
    { const fz = FOLEY[path.basename(b.src, ".mp4")]; if (fz && existe(fz.file)) { assets.add(fz.file); foleyCues.push({ from: b.f0, dur: Math.min(b.f1 - b.f0, Math.floor(durDe(fz.file) * FPS)), src: fz.file, vol: fz.vol }); } }
...
// ── COMPUERTAS de contenido
{
  ...
  if (qrCues.length < 2) fail(`CTAs con QR ${qrCues.length} < 2`);
  console.log(`foley: ${foleyCues.length} clips de agnes con su sonido`);
...
// ── fps (30/1) y aspecto (16:9)
{
  ...
  const imgs = [...assets].filter((a) => /\.(png|jpe?g)$/i.test(a) && !/_qr\.png$|_portada\.jpg$|qr_falifting|portada_piel/.test(a));
...
import { ${[...used].filter((u) => !NUEVOS.has(u)).sort().join(", ")} } from "../${SLUG}/Kit";
${[...used].some((u) => NUEVOS.has(u)) ? `import { ${[...used].filter((u) => NUEVOS.has(u)).sort().join(", ")} } from "../${SLUG}/KitNuevos";` : ""}
...
${foleyCues.map((c) => `    <Sequence from={${c.from}} durationInFrames={${c.dur}} layout="none"><Audio src={staticFile(${JSON.stringify(c.src)})} volume={(f) => ${c.vol} * Math.min(1, f / 4.5, (${c.dur} - f) / 4.5)} /></Sequence>`).join(String.fromCharCode(10))}
  </AbsoluteFill>
);
`);
...
{
  const files = [`src/VideoEdit/Main_${SLUG}.tsx`, `src/VideoEdit/cues_${SLUG}.gen.tsx`, `src/${SLUG}/Piezas.tsx`, `src/${SLUG}/Kit.tsx`, `src/${SLUG}/KitNuevos.tsx`];
...
const kit = fs.readFileSync(`src/${SLUG}/Kit.tsx`, "utf8") + fs.readFileSync(`src/${SLUG}/KitNuevos.tsx`, "utf8");
