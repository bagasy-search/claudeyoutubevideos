// build_mdfreezer.mjs — "Your Water Heater Isn't Dying — It's Full of This ($1,800 Saved)"
...
//   node build_mdfreezer.mjs
// Lee _v3/mdfreezer_plan.json (DIRECTOR), _v3/mdfreezer_luma.json, _v3/mdfreezer_avatar_clips.json.
import fs from "fs";
import { execFileSync } from "child_process";

const SLUG = "mdfreezer", COMP = "MdFreezer", FPS = 30;
...
for (const p of planos) {
  ...
  if (m.tipo === "componente") {
    ...
    if (OVERLAY.has(c)) {
      ...
      // cama de b-roll BAJO el overlay (si no, el slot queda sin cobertura -> hueco)
      let mb = null, mbL = -1;
      for (let q = Math.max(0, m.i - 8); q <= m.i + 8; q++) { const cand = nm(q), l = LUMA[cand]; if (l !== undefined && l > mbL && tieneImg(cand)) { mb = cand; mbL = l; } }
      if (mb) { cues.push({ key: `ovbed_${msIn}`, f0: p.f0, d, el: `(d) => <MdhFoto durationInFrames={d} src=${JSON.stringify(`img/${mb}.jpg`)} seed={${seed}} />` }); assets.add(`img/${mb}.jpg`); assets.add(`img/${mb}_blur.jpg`); }
      continue;
    }
