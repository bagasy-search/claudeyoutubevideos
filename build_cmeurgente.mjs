// build_cmeurgente.mjs — MONTAJE del video `cmeurgente` (Claudio Mendoza Constructor).
//   "¡URGENTE! No Compres Paneles Solares. Haz Esto y Ahorra Miles"
//
// Lee   _v3/cmeurgente_moments_ms.json  (micro-momentos YA anclados al ms por _v3/cmeg_anclar.py)
//     + _v3/cmeurgente_movs.json        (los 16 movimientos: anchor, actos y assets)
// Emite src/cmeurgente/{cues_cmeurgente.gen.tsx, Main_cmeurgente.tsx}
//     + src/index_cmeurgente.tsx + _cmeurgente_assets.txt
//
...
const SLUG = "cmeurgente", COMP = "Cmeurgente", UP = "CMEURGENTE";
const FPS = 30;
const AVATAR_S = 712.192;            // mp4 del avatar, medido con ffprobe
const AVATAR_END_MS = 711950;        // hasta acá el lipsync es REAL (última palabra de Whisper)
...
const MM = rd("_v3/cmeurgente_moments_ms.json");
const MOVS = rd("_v3/cmeurgente_movs.json");
...
for (const [k, v] of Object.entries(MOVS)) {
  // ⛔ El plan de ESTE video define el tramo con `t0`/`t1` en SEGUNDOS, y sus claves YA traen el
  // prefijo `Mov` (MovPresupuesto, no Presupuesto). El build clonado de `cmegenerador` leía `ms0` y
  // volvia a prefijar: `dur` salia NaN (la Sequence no cubria nada -> cobertura 25%) y el import
  // apuntaba a `MovMovPresupuesto`.
  const base = (MM.movimientos[k] ?? v.t0) * 1000;           // ms REAL del arranque
  const dur = v.t1 - v.t0;
  if (!Number.isFinite(base) || !Number.isFinite(dur) || dur <= 0) {
    console.error(`⛔ ${k}: tramo invalido (base ${base}, dur ${dur})`); process.exit(1);
  }
  actos.push({ id: `mv_${k}`, mov: k, acto: 0, ms: base, dur, ms0: base });
  ...
  for (const as of v.assets) {
    // ⛔ cada asset tiene su tipo: exigir foto Y clip de todos hacía fallar el build por assets
    // que nunca se pidieron. `foto_ref` es una foto; `lamina`/`qr` ya existen en disco.
    const rel = as.t === "clip"  ? `broll/${SLUG}/${as.n}.mp4`
              : as.t === "icono" ? `img/${SLUG}/${as.n}.png`
              : as.t === "qr"    ? `img/${SLUG}/${as.n}.png`
              : `img/${SLUG}/${as.n}.jpg`;
    if (!existe(rel)) faltan.push(rel); else add(rel);
  }
}
...
for (const e of MM.momentos) {
  ...
  // ⛔ EL ICONO SOBRE UN MOVIMIENTO SE APILA CONTRA SU HUD Y LO TAPA. Los movimientos ya cantan
  // su cifra con su propio titular, en la misma esquina: el overlay quedaba pisando el titular
  // (medido: "NOVECIENTOS SESENTA AL MES" encima de "TREINTA Y DOS POR DIA"). El icono SOLO
  // sobrevive si cae en un hueco, donde de verdad flota sobre material y no compite con nada.
  if (pisaMov(e.ms)) { descartados++; continue; }
  if (e.tipo === "CLIP") {
...
const base = cues.filter((c) => c.capa === "base").sort((a, b) => a.start - b.start || a.dur - b.dur);
...
// ── EL BUCLE DEL AVATAR ────────────────────────────────────────────────────────
// La cola (768 s) es MAS LARGA que el avatar (712 s): con una sola <Sequence> el <Video> se queda
// clavado en su ultimo cuadro los ultimos 56 s. Van N pasadas completas, y CADA costura tiene que
// caer debajo de un movimiento a pantalla completa (712,5 s -> MovLadronesA · 1424,7 s -> MovCuandoSi).
const AV_F = Math.round(AVATAR_S * FPS);
const LOOP_F0 = f(LOOP_START_MS);
const nPas = Math.ceil((TOTAL_FRAMES - LOOP_F0) / AV_F);
const LOOPS = Array.from({ length: nPas }, (_, i) => {
  const from = LOOP_F0 + i * AV_F;
  const dur = Math.min(AV_F, TOTAL_FRAMES - from);
  return [
...
{
  const costuras = Array.from({ length: nPas }, (_, i) => (LOOP_F0 + i * AV_F) / FPS);
  // La capa base va A PANTALLA COMPLETA (foto o clip a sangre), asi que cualquier cue tapa la
  // costura igual de bien que un movimiento. Antes solo se aceptaban los movimientos, y al bajar
  // de 17 a 6 el chequeo daba falso negativo aunque el cuadro estuviera ocupado.
  // Si una costura queda al aire, se ESTIRA el plano anterior hasta taparla con margen: el salto
  // del avatar volviendo al principio del video no puede verse, y un plano dura lo que haga falta
  // antes que dejarlo asomar.
  for (const c of costuras) {
    const f = Math.round(c * FPS);
    const cubre = base.some((x) => x.start + 24 <= f && x.start + x.dur - 24 >= f);
    if (cubre) continue;
    const prev = base.filter((x) => x.start + 24 <= f).sort((a, b) => b.start - a.start)[0];
    if (!prev) continue;
    const sig = base.filter((x) => x.start > prev.start).sort((a, b) => a.start - b.start)[0];
    const hasta = Math.min(f + 45, sig ? sig.start + sig.dur - 24 : TOTAL_FRAMES);
    prev.dur = hasta - prev.start;
    if (sig && sig.start < hasta) { sig.dur -= (hasta - sig.start); sig.start = hasta; }
    console.log(`   costura ${c.toFixed(1)}s: estiro el plano anterior para taparla`);
  }
  const tramos = base.map((c) => [c.start / FPS, (c.start + c.dur) / FPS]);
  for (const c of costuras) {
    const tapa = tramos.some(([a, b]) => c >= a + 0.7 && c <= b - 0.7);
    console.log(`costura del bucle en ${c.toFixed(1)}s -> ${tapa ? "TAPADA por un movimiento ✓" : "⛔ DESCUBIERTA"}`);
    if (!tapa) { console.error("⛔ una costura del bucle queda a la vista"); process.exit(1); }
  }
}
...
for (const n of fs.readdirSync(`public/img/${SLUG}`)) {
  ...
  const alfa = n.startsWith("cmeu_ic_") || n.startsWith("cmeu_qr");
