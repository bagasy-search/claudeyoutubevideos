// build_nrhooded.mjs — clon del motor de build_nrtinnitus (avatar RunPod por ventanas + stock + fotos del presentador + LaminaZoom/Lamina3D + camas). Slug/portada/QR/landing propios.
...
const SLUG = "nrhooded", COMP = "Nrhooded", UP = "NRHOODED", FPS = 30;
const COVER = "img/nrhooded_cover.jpg", QR = "qr_nrhooded.png", SITE = "docfederer.com/botox";
...
for (const b of rows) {
  ...
  if (b.tipo === "avatar") {
    ...
    scan(b.src); assets.add(COVER); assets.add(QR); assets.add("img/nrhooded/m048_blur.jpg");
    cues.push({ key, start, dur, el: `<Lamina3D table={"img/nrhooded/m048_blur.jpg"} src={${JSON.stringify(b.src)}} keys={${JSON.stringify(b.keys)}} ${b.cta !== undefined ? `cta={${b.cta}} cover={${JSON.stringify(COVER)}} qr={${JSON.stringify(QR)}} site={${JSON.stringify(SITE)}}` : ""} />` });
