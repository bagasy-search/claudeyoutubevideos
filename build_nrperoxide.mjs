// build_nrperoxide.mjs — clon de build_nrg-oil (cadena fcsmanos10) + tipo "lamina" (LaminaZoom con zoom a regiones y CTA portada+QR). Lee _v3/nrperoxide_plan.json y emite
//   src/_fed6/VideoEdit/Main_nrperoxide.tsx      (build real, con `startSec:` por cue → lo mide el gate)
//   src/_fed6/VideoEdit/avatar_nrperoxide.gen.ts (ventanas de avatar → tiempo VISIBLE del gate)
//   src/index_nrperoxide.tsx
...
const SLUG = "nrperoxide", COMP = "Nrperoxide", UP = "NRPEROXIDE", FPS = 30;
const COVER = "img/nrperoxide_cover.jpg", QR = "qr_nrperoxide.png", SITE = "docfederer.com/age-spots";
...
for (const b of rows) {
  ...
  if (b.tipo === "avatar") {
    ...
    if (b.table) scan(b.table);
    cues.push({ key, start, dur, el: `<Lamina3D src={${JSON.stringify(b.src)}} keys={${JSON.stringify(b.keys)}} intro={${b.intro ? "true" : "false"}} ${b.table ? `table={${JSON.stringify(b.table)}}` : ""} ${b.cta !== undefined ? `cta={${b.cta}} cover={${JSON.stringify(COVER)}} qr={${JSON.stringify(QR)}} site={${JSON.stringify(SITE)}}` : ""} />` });
...
if (sinCama.length) {
  ...
  console.error("   → corré: node scripts/gen_beds.mjs nrperoxide");
  process.exit(1);
}
