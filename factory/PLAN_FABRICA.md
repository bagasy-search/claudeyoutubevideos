...
- [x] **I1. Avatar arranca apenas termina Fish**, sin esperar ASR ni montaje (ventanas desde el manifest de bloques de Fish; el ASR sólo afina anclajes). Ahorro medido ~20 min. En la fábrica: 55_avatar hoy depende de 30_direct → separar "ventanas por plan" de "ventanas previas". — ✅ `dfd7790`: fase nueva `15_frases`; la dirección trabaja sobre los momentos del guion SIN esperar la voz. Tandas (verificadas sin ciclos): `00` → `10_voice + 15_frases` → `20_asr + 30_direct` → `40_images + 55_avatar` → `50` → `60`… El avatar sale apenas hay ASR y dirección, en paralelo con imágenes/agnes.
- [x] **I2. DECIDIDO (creador, 15-sep):** NO se paraleliza el avatar y **un endpoint propio nunca sale más barato**: InfiniteTalk público a US$0,25 por el avatar entero. La palanca es el ORDEN: avatar sólo de ventanas visibles, lanzado apenas termina el director (+ASR), en paralelo con b-roll/montaje/componentes; al terminar el montaje ya está (implementado en I1). Texto original: (a) 3–4 /run simultáneos: 38–97 min → 10–15 min, US$0,75–1,50 en vez de US$0,25, contradice "un solo /run"; (b) endpoint serverless PROPIO en RunPod (fork parcheado, steps 2, fp8, varios workers): rápido y probablemente más barato, requiere deploy desde la consola de RunPod (lo hace el creador).
- [ ] **I3. Assets fuera de GitHub Releases → R2/S3** (ya hay R2): el endpoint de releases es el que se satura con 5 renders; tar cacheado por contenido entre videos. Desbloquea el paralelismo real. — ⛔ BLOQUEADO (medido 15-sep): `CF_API_TOKEN` está activo pero **sin permiso de R2** (`/r2/buckets` → 10000 Authentication error) y las claves S3 de R2 no están en disco (`_ittest/onstart_r2_creds.sh` las recibe por env). **Acción del creador:** crear en Cloudflare un token R2 (Object Read & Write) y pegar `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` en `.env`.
- [~] **I4. Capacidad de render:** multicuenta (`reference_renderbatch_multicuenta`, token `.gh_token2` vencido) o runners self-hosted/GPU; nunca bajar a 12–15 chunks por cola. Poll por URL pública del release en vez de la API de Actions; un token por trabajador. — ✅ `releaseAssetPublic` (HEAD a la URL pública, 0 llamadas a la API) en 80_render y 90_deliver; lease `farm_slots`. ⛔ multicuenta: **acción del creador**, PAT nuevo (scopes repo+workflow) en `.gh_token2`.
- [x] **I5. Re-encode de entrega fuera del PC** (en el runner o con nvenc): 10–13 min → ~2 min. — ✅ medido: x264 con TODOS los hilos (Ryzen 7 6800H) 120 s de video **69 s → 32 s** y pasa check_entrega 7/7. NVENC queda automático cuando exista: hoy NO abre (driver 566.07, ffmpeg pide API 13.1 = driver ≥ 610) → **acción opcional del creador:** actualizar el driver NVIDIA; la fase lo prueba abriendo el encoder y cae a CPU si falla.
- [~] **I6. Preset por canal** (la biblioteca de clips/fotos queda FUERA por decisión del creador, 15-sep): — `styles/<canal>.json` con voz, cara, lugares, CTA; se suma `guia` (landing, qr, temas) que 30_direct pone en el prompt del director para cazar "está en la guía / no está" al planificar. Texto original: voz, cara, CTA, portada, QR, índice de la guía ya extraído (el caso "salvia/ricino no está en la guía" se caza al planificar), stock y fotos del presentador ya auditados por tema. Arranque 12–60 min → ~1 min; Pexels ~1 h → ~0.
- [x] **I7. Concurrencia:** ASR (si cae a OpenAI) de 5 a 20–30; Batch de imágenes del presentador sale apenas está el guion (no depende de la voz). — ✅ con I1 las imágenes salen tras la dirección, en paralelo con la voz; el ASR por defecto es Modal (no el de concurrencia 5).
...
|---|---|---|---|---|
| 2026-09-18 | **`fbrefrac` ENTREGADO** (video 7 de The Free Builder, tanda de 3 en paralelo) | rama `rkfob-render`, sin commitear (árbol de otra sesión) · ref de render `b2287c7` | **14 min 53 s · 26.793 cuadros · US$0,54** (gpt-image 0,29 + RunPod 0,25; Fish, Pexels y YouTube CC gratis). Avatar en **UN solo `/run`**: 32 ventanas, 207 s visibles (23,2 %), desfase **0 ms**, correlación 1,00. Montaje: 168 cues, 60 clips, 108 fotos, 15/15 componentes (7 kinds), **0 s de avatar tapado**, cobertura **100 %**, 0 saltos de PTS, 1 tramo negro (tolerado). Ritmo: min 0,10 / p25 3,43 / mediana 4,13 / p75 5,17 / máx 7,23 s. **Metraje real en pantalla 31 % del b-roll** (56 clips: 48 CC de YouTube + 8 Pexels). agnes sólo **5 %** de los planos. video_job **490 reusado**, `done:false`, sin subir a YouTube. **Lo que aportó este video:** (1) **Fish alucina bloques que pasan el flag de duración** — 5 rondas de regeneración con los parámetros por defecto no convergieron (b001/b002/b008/b009/b010 rotando); **`--temperature 0.55 --top-p 0.65` los sacó limpios**, confirmando el hallazgo del video 8 con un segundo caso. La señal es `bloquesGarbled` del ASR ('oyó el 0 % de sus palabras'), NO la duración: acá los bloques duraban lo correcto y el texto era galimatías. (2) **El motor reusa el `runId` del estado aunque el commit haya cambiado**: al matar un `80_render` a mitad, el `runId` quedó persistido y la re-corrida se enganchó a un run de OTRO commit que ya había terminado en verde. Se cazó comparando `headSha` del run contra la ref recién empujada; la cura fue borrar `factory/_state/<slug>/80_render.json` a mano. El sello por runId del mp4 sí funcionó y evitó entregar el archivo viejo. (3) **Los clips de 3 s no alcanzan**: con `colaFotoS 2,5` un clip de 3 s sólo cubre ~2,4 s de un momento de 5 s. Re-cortar el mismo metraje a **6 s** subió el metraje real en pantalla de **17 % a 31 %** sin bajar un clip más. (4) **Pexels: 24 pedidos → 12 clips, y de esos descarté 4 a ojo** (una playa, un ladrillero con la cara a la vista, pigmento azul, viruta de madera) + 4 más en el segundo intento. Tasa útil real **~33 %**. El hueco lo tapó **YouTube CC**: 10 fuentes bajadas, **5 descartadas enteras** por marca de agua persistente, subtítulos quemados o blanco y negro con gente (Robin Greenfield, Simon Pockley, Earth Uncovered, Crafted History, Sounds of Changes) y 5 aprobadas → 48 clips. Créditos CC-BY en la descripción. (5) **Auditoría de personas medida**: 56 clips revisados cuadro a cuadro (4 frames por clip de Pexels, 2 frames extra por cada clip CC alargado a 6 s) → **0 con cara visible o cuerpo entero**; los que muestran manos, brazos o piernas trabajando se dejaron, según el criterio del creador. (6) agnes rechazó **9 de 33** a ojo en dos rondas; el que reincidió derivando a primer plano de la cara del presentador se pasó a **foto quieta**. Rechazo `c:1` alto otra vez (deriva a retrato + cambio de identidad). | Que `80_render` compare el `headSha` del run contra la ref antes de reusar el `runId` · clips de stock cortados a la duración del momento, no a 3 s fijos |
| 2026-09-18 | **`fboxidoropa` ENTREGADO** (video 8 de The Free Builder, tanda de 3 en paralelo) | rama `rkfob-render`, sin commitear (árbol de otra sesión) | **13 min 29 s · 24.269 cuadros · US$0,50** (gpt-image 0,25 + RunPod 0,25; Fish y Pexels gratis). Avatar en **UN solo `/run`**: 37 ventanas, desfase **0 ms**, correlación 1,00. Montaje: 158 cues, 71 clips, 87 fotos, 11/11 componentes, **0 s de avatar tapado**, cobertura **100 %**, 0 saltos de PTS, pacing p25 3,17 / mediana 4,0 / p75 5,2 / máx 7,5. Metraje real **27 %** del b-roll (174,5 s de 648,9 s). video_job **491 reusado**, sin duplicar. **Lo que sólo apareció acá:** (1) **Fish LOOPEA bloques enteros** y el flag de duración no lo caza porque un bloque que falla no entra al manifest: b002 salió a 1,95× y b008 a 1,70× de su duración, el ASR los leyó como 'oyó el 1 % de sus palabras' y se fueron 3 rondas persiguiendo al ASR en vez de al TTS. Cura: **partir el bloque en mitades de ~580 chars** + `--temperature 0.6 --top-p 0.7` → los 11 bloques a ratio 0,97-1,12. Hoy es la compuerta `fishBloquesLooped`. (2) **El guard de `quietos` en 60_build se comía el metraje real**: desde que los planos de Pexels/CC se marcan `q:1` para sacarlos de agnes, el mismo `if` los mandaba a foto — **de 71 clips llegaban 30**. (3) **Dos mediciones nuevas nacieron mal** y las cazó el conteo a mano del agente, no una compuerta: `metrajeRealEnPantallaPct` adivinó la unidad de `dur` (746,9 s, que no era ni el video ni el b-roll) y después contó dos veces los overlays de componente. (4) **Los componentes del kit van en `k.props`, no sueltos en `k`**: 11 pedidos → 0 emitidos, sin error; el agente del video 9 cayó en lo mismo (compuerta nueva en `compose`). (5) **Pexels es 47 % inservible en este nicho**: de 51 clips bajados **descarté 24** (off-topic o con personas ajenas) y de 6 pedidos extra, **los 6**. El hueco lo tapó **YouTube con `videoLicense=creativeCommon`**: 11 clips auditados frame a frame, 1 descartado por marca de agua, créditos CC-BY en la descripción. (6) agnes rechazó **10 de 37** a ojo; los dos que reincidieron se pasaron a **foto quieta** en vez de gastar otra ronda. Rechazo `c:1` **50 %** vs `c:0` **21 %** (n=8, coincide en signo con fbaislar pero el modo de falla NO fue 'deriva a retrato': fue reemplazo de objeto/fondo y recorte del presentador fuera de cuadro). | Cerrar los 98 s de diferencia ya resueltos · `q:1` por defecto en `c:1` a confirmar con más muestra |
| 2026-09-15 | Plan creado | — | Diagnóstico §1 de 11 videos (11–15 sep) | Arrancar A1 en rama `factory` desde `main` |
| 2026-09-16/17 | **H3 REAL: 5 videos en paralelo, 1 entregado** · 16 arreglos del motor | rama `cme-fabrica` (`df36915`…`f906b7d`), 48 tests verdes | **Primera corrida de 5 videos a la vez (canal Claudio Mendoza).** `cmeodian` ENTREGADO: 22:04, 39.701 cuadros, 0 saltos de PTS, 0 negros, **0 s de avatar tapado**, cobertura 100 %, 3 CTA con QR, **US$0,80**. Los otros 4 quedaron con todo lo caro hecho (PNG, reels, voz, dirección) cuando un borrado accidental se llevó `public/` ([[reference_borrado_public_recuperacion]]). **Lo que SOLO aparece con 5 videos a la vez:** (1) `lease.mjs` otorgaba el DOBLE de capacidad — el latido reescribía su archivo sin mirar, así que un tenedor atrasado y barrido lo RESUCITABA encima del legítimo: medido 28/14 con dos PIDs generando; arreglado + control positivo que falla con el código viejo. Tras el arreglo: **0 rate-limits en 307 pedidos** (antes 45-53 por corrida). (2) Sin FIFO hay INANICIÓN: cmealter esperó 3 h 10 y perdió 5 sorteos con 1 clip de 307 (ítem B6). (3) Una fase `done` cuyas salidas se borraron se salteaba con "✓ fresca" sin correr su compuerta (ítem D6, `verify()`). (4) El exit code de `agnes_i2v` mataba la fase con 99,7 % hecho: **decide la compuerta, no el exit**. (5) Banda muerta en `55_avatar` entre el gate del reel (1,5 s) y el cortador de ventanas (0,087 s). (6) Assets que viajan en PROPS (el QR del CTA) no entraban en la lista del farm → 404 y 2 renders perdidos. (7) El pre-vuelo exigía `med/` a todos por grepear TODO `src`. (8) `fish_factory.py` en disco era la copia rkfob (sin `--voice`). (9) `align.py` le regalaba el tiempo de sus vecinos a un momento (7 de ~1.600). (10) `reset <slug> <fase>`, que no existía. | Pedir `RUNPOD_API_KEY` al creador (única pérdida del borrado) y cerrar los 4 · B6 (FIFO) y D6 (hash de prompt) con la cola vacía · merge `cme-fabrica` → `main` |
| 2026-09-15 (noche) | **H1 REAL ✅ · G1 cerrado** | `bc8e709`…`4272edb` | **tcestufa y tcfiltro RE-PRODUCIDOS Y ENTREGADOS por la fábrica** (`?v=3`, video_jobs 432 y 443 actualizados sin tarjeta nueva ni YouTube): avatar tapado 29,7 s → **0** y 73,1 s → **0**; cobertura 100 %; 0 saltos de PTS; 0 negros; bytes del release == local. Reloj desde el montaje: tcestufa 3.492 s de render + 352 s de entrega; tcfiltro 4.539 s + 477 s. Incidentes y sus arreglos: (1) `--from` rehacía fases anteriores y el import con HARDLINK pisó el máster de tcestufa → recuperado exacto del m4a del release, hoy `--from` confía y el import COPIA; (2) farm exige HEAD == ref → worktree mínimo por video con junctions; (3) otra sesión canceló 26/32 chunks → re-despacho automático reusando assets; (4) `saltosPts` contaba una línea vacía de ffprobe → falso salto. Limpieza de disco: D: de 1,1 GB → **539 GB** libres (scripts corridos por el creador). | Migrar montaje Federer/narrador · D4 · R2 (I3) · multicuenta (I4) |
