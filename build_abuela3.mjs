...
const SLUG = "abuela3", COMP = "Abuela3", UP = "ABUELA3";
const FPS = 30;
...
// ── KIT PREMIUM: los componentes del video APROBADO de este canal viven en
//    src/VideoEdit/kit/premium/ y se montan como OVERLAY sobre el metraje vivo,
//    envueltos en <PremiumOverlay> (escala 1:1, sin fondo propio).
// ⛔ NUNCA como cue base: taparian el b-roll, que es la identidad del canal.
const PREMIUM = new Set(
  fs.readdirSync("src/VideoEdit/kit/premium").filter((f) => /\.tsx?$/.test(f))
    .flatMap((f) => [...fs.readFileSync(`src/VideoEdit/kit/premium/${f}`, "utf8")
      .matchAll(/export const ([A-Z][A-Za-z0-9]*)/g)].map((m) => m[1])));
...
for (const o of overlays) {
  ...
  scanProps(o.props);
  const esPrem = PREMIUM.has(o.componente);
  const inner = `<${o.componente} durationInFrames={d} {...(${JSON.stringify(o.props || {})} as any)} />`;
  ovCues.push({ key: `ov_${o.ms_in}`, start: f0 / FPS, dur: (f1 - f0) / FPS,
    el: esPrem
      ? `(d) => <PremiumOverlay durationInFrames={d} zone={${JSON.stringify(o.zone || "topLeft")}}>${inner}</PremiumOverlay>`
      : `(d) => ${inner}` });
}
...
const imports = [
  ...
  `import { ${[...new Set(fromStage)].sort().join(", ")} } from "../abuela/RayStage";`,
  ...(fromFiles.some((c) => PREMIUM.has(c))
      ? [`import { PremiumOverlay } from "./scenes/PremiumOverlay";`,
         `import { ${fromFiles.filter((c) => PREMIUM.has(c)).sort().join(", ")} } from "./kit/premium";`]
      : []),
  ...fromFiles.filter((c) => !PREMIUM.has(c)).sort().map((c) => `import { ${c} } from "../abuela/${c}";`),
];
...
{
  const KIT = "abuela";   // ⛔ el kit de ESTE canal es src/abuela (clon de src/dale repintado a madera/terracota/crema).
