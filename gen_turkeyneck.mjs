// gen_turkeyneck.mjs — build · Canal "Dr. Federer — The Nightly Remedy" (EN)
...
const WIN = JSON.parse(fs.readFileSync("_v3/tn/tn_windows.json", "utf8")).windows;
const byPos = (arr) => arr.map((m) => ({ ...m, _i: SCRIPT.indexOf(m.p) })).sort((a, b) => a._i - b._i);
const MOMS = byPos([
  ...JSON.parse(fs.readFileSync("_v3/tn/spec_people.json", "utf8")),
  ...JSON.parse(fs.readFileSync("scripts/_turkeyneck_stock_src.json", "utf8")),
]);
const CMPS = byPos(JSON.parse(fs.readFileSync("_v3/tn/spec_comps.json", "utf8")));
...
const APERTURA = Math.max(3, charToMs(SCRIPT.indexOf("And you can start waking")) - 0.1);

// assets: stock real (clip) · presentador animado (clip agnes) · presentador foto
const assetOf = (n) => {
  if (has(`broll/tn/${n}.mp4`)) return { kind: "clip", src: `broll/tn/${n}.mp4` };
  if (has(`tn/clips/${n}.mp4`)) return { kind: "clip", src: `tn/clips/${n}.mp4` };
  if (has(`tn/img/${n}.jpg`)) return { kind: "foto", src: `tn/img/${n}.jpg` };
  return null;
};
const moments = [];
{
...
const altOf = (b) => {
  if (b.kind === "clip" && has(`tn/img/${b.n}.jpg`)) return { kind: "foto", src: `tn/img/${b.n}.jpg`, n: b.n };
  if (b.kind === "foto" && has(`tn/clips/${b.n}.mp4`)) return { kind: "clip", src: `tn/clips/${b.n}.mp4`, n: b.n };
  return null;
};
...
}
{
  ...
  COMPS.push({ kind: "laminacta", t: a.ms, fixedEnd: endMs, image: "tn/tn_lamina.jpg", cover: "tn/tn_cover.jpg", qr: "tn/tn_qr.png", domain: "docfederer.com/firm-skin" });
}
...
const stF = BASE.filter((b) => (b.src || "").startsWith("broll/tn/")).reduce((s, b) => s + b.dur, 0);
...
const hurga = (v) => { if (typeof v === "string") { if (/^(img|broll|med|tn)\//.test(v)) need.add(v); return; } if (Array.isArray(v)) v.forEach(hurga); else if (v && typeof v === "object") Object.values(v).forEach(hurga); };
