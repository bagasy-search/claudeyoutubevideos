// gen_castorglove.mjs — build · Canal "Dr. Federer — The Nightly Remedy" (EN)
// "TURKEY NECK: Tighten the Sagging Jawline While You Sleep (No Surgery)"
...
const WAV_S = parseFloat(probe(`public/cog/${SLUG}.wav`, ["-show_entries", "format=duration"]));
const VIDEO_END = +(WAV_S + 0.4).toFixed(3);

const SCRIPT = fs.readFileSync(`_v3/cog/${SLUG}_script.txt`, "utf8");
const WMS = JSON.parse(fs.readFileSync(`_v3/cog/${SLUG}_wordms.json`, "utf8"));
const WIN = JSON.parse(fs.readFileSync("_v3/cog/cog_windows.json", "utf8")).windows;
const byPos = (arr) => arr.map((m) => ({ ...m, _i: SCRIPT.indexOf(m.p) })).sort((a, b) => a._i - b._i);
const MOMS = byPos([
  ...JSON.parse(fs.readFileSync("_v3/cog/spec_people.json", "utf8")),
  ...JSON.parse(fs.readFileSync("_v3/cog/stock_src.json", "utf8")),
]);
const CMPS = byPos(JSON.parse(fs.readFileSync("_v3/cog/spec_comps.json", "utf8")));
...
const APERTURA = Math.max(3, charToMs(SCRIPT.indexOf("While you sleep.")) - 0.1);

// assets: stock real (clip) · presentador animado (clip agnes) · presentador foto
const assetOf = (n) => {
  if (has(`broll/cog/${n}.mp4`)) return { kind: "clip", src: `broll/cog/${n}.mp4` };
  if (has(`cog/clips/${n}.mp4`)) return { kind: "clip", src: `cog/clips/${n}.mp4` };
  if (has(`cog/img/${n}.jpg`)) return { kind: "foto", src: `cog/img/${n}.jpg` };
  return null;
};
const moments = [];
{
...
}
{
  ...
  COMPS.push({ kind: "laminacta", t: a.ms, fixedEnd: endMs, image: "cog/cog_lamina.jpg", cover: "cog/cog_cover.jpg", qr: "cog/cog_qr.png", domain: "docfederer.com/age-spots" });
}
...
const stF = BASE.filter((b) => (b.src || "").startsWith("broll/cog/")).reduce((s, b) => s + b.dur, 0);
...
// ── assets ──
const need = new Set([`cog/${SLUG}.m4a`, `cog/${SLUG}_avatar.mp4`]);
const hurga = (v) => { if (typeof v === "string") { if (/^(img|broll|med|cog)\//.test(v)) need.add(v); return; } if (Array.isArray(v)) v.forEach(hurga); else if (v && typeof v === "object") Object.values(v).forEach(hurga); };
