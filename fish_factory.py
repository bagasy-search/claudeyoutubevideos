...
def split_blocks(text: str, max_chars: int) -> list[str]:
    ...
    # segunda compuerta: lo que se manda a Fish tiene que ser el texto entero.
    # ⛔ Comparación INSENSIBLE A ESPACIOS: al re-unir bloques, un corte entre "?" y ","
    #    (p.ej. "camina?, ¿se...") inserta un espacio que el texto original no tiene. Eso NO es
    #    texto perdido ni duplicado — es reespaciado inocuo. Lo que la compuerta debe cazar es
    #    que se pierda o duplique un CARÁCTER real, así que se comparan las dos cadenas sin
    #    ningún espacio. (Medido en fcscuello: +4 espacios por 4 patrones "?,".)
    import re as _re
    a = _re.sub(r"\s+", " ", " ".join(blocks)).strip()
    if _re.sub(r"\s+", "", a) != _re.sub(r"\s+", "", text):
        print(f"⛔ los BLOQUES no reconstruyen el texto ({len(a)} vs {len(text)} chars, sin-espacios difiere)")
        sys.exit(3)
    return blocks
...
def main():
    ...
    ap.add_argument("--voice", help="id en fish_voices.json (alternativa a --refs)")
    ...
    if not a.refs and not a.voice:
        sys.exit("hace falta --refs o --voice")
    if a.voice:
        vp = Path(__file__).resolve().parent / "fish_voices.json"
        voices = json.loads(vp.read_text(encoding="utf-8"))
        if a.voice not in voices:
            sys.exit(f"voz '{a.voice}' no registrada en {vp} (hay: {', '.join(sorted(voices))})")
        v = voices[a.voice]
        refs_spec = v["refs"] if "refs" in v else [{"wav": v["wav"], "text": v["text"]}]
        base = vp.parent
        for r in refs_spec:
            if not Path(r["wav"]).is_absolute():
                r["wav"] = str(base / r["wav"])
    else:
