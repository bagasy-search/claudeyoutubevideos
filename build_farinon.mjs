// build_fapiel60.mjs — plan -> cues + Main + index + avatar_fapiel60.gen.ts + _fapiel60_assets.txt
...
//   1) node gen_fapiel60_plan.mjs && node build_fapiel60.mjs     # sale avatar_fapiel60.gen.ts
//   2) node avatar_fp8_ventanas.mjs fapiel60 --solo-full → farm → node avatar_fp8_ensamblar.mjs fapiel60
//   3) node build_fapiel60.mjs otra vez (ya con public/fapiel60_opt.mp4 → compuerta de fps del avatar)
//
...
const SLUG = "farinon", COMP = "Farinon", UP = "FARINON";
const FPS = 30;
const WAV_FILE = `${SLUG}.m4a`;
const AVATAR = `${SLUG}_opt.mp4`;   // ensamblado por avatar_fp8_ensamblar.mjs (negro fuera de las ventanas)
const GAP_MAX_S = 32;               // regla "~30 s": 5 tramos medidos de 30,0-31,0 s con el reel ya generándose en Vast               // regla del creador: nunca más de ~30 s sin ver al presentador
...
// arranques propios de clips que abren con fundido desde negro (medidos por _v3/farinon_stock_qa.py)
const START_OVR = fs.existsSync(`_v3/${SLUG}_stock_start.json`) ? JSON.parse(fs.readFileSync(`_v3/${SLUG}_stock_start.json`, "utf8")) : {};
const cues = [];
...
for (const b of rows) {
  ...
  if (b.tipo === "clip") {
    ...
    const sf = b.startFrom ?? (START_OVR[b.clip.split("/").pop()] ?? 0.6);
    cues.push({ key, start, dur, el: `(d) => <Clip src=${JSON.stringify(r)} seed={${b.f0}} startFrom={${sf}} />` });
  } else if (b.tipo === "clipslow") {
    const r = `broll/${b.clip}.mp4`;
    if (!existe(r)) faltan.push(r); else assets.add(r);
    cues.push({ key, start, dur, el: `(d) => <Clip src=${JSON.stringify(r)} rate={${b.rate || 0.5}} seed={${b.f0}} startFrom={0.6} />` });
    ...
    if (!existe(b.src)) faltan.push(b.src); else assets.add(b.src);
    usedComps.add("LaminaZoom");
    cues.push({ key, start, dur, el: `(d) => <LaminaZoom src=${JSON.stringify(b.src)} ${b.from ? `from={${JSON.stringify(b.from)}} ` : ""}to={${JSON.stringify(b.to)}} durationInFrames={d} />` });
...
const PRUEBA = process.env.BUILD_PRUEBA === "1";   // ⛔ SOLO para tsc temprano; el render real nunca lo usa
if (faltan.length && PRUEBA) { console.log(`⚠ PRUEBA: ${[...new Set(faltan)].length} assets faltan (no se aborta)`); faltan.length = 0; }
if (faltan.length) {
...
// ── COMPUERTA DE FPS (30/1 CFR en TODO clip + el avatar si ya existe) ────────────────────────────
{
  ...
  if (malos.length && PRUEBA) { console.log(`⚠ PRUEBA: ${malos.length} videos sin 30/1`); malos.length = 0; }
  if (malos.length) {
...
// ⛔ COMPUERTA DE CONEXIÓN: hueco máximo entre dos apariciones del avatar (medido sobre frames)
{
  ...
  const huecos = [];
  let prevZ = 0;
  // ⭐ excepción ÚNICA: la LÁMINA a pantalla completa (EL MOMENTO) puede dejar al presentador fuera hasta 50 s
  const lams = rows.filter((r) => r.tipo === "lamina").map((r) => [r.f0 / FPS, r.f1 / FPS]);
  const enLamina = (a, z) => lams.some(([la, lz]) => la < z && lz > a) && (z - a) <= 50;
  for (const t of full) { if (t.a - prevZ > GAP_MAX_S && !enLamina(prevZ, t.a)) huecos.push({ desde: prevZ, hasta: t.a }); prevZ = t.z; }
  console.log(`láminas a pantalla completa: ${lams.length} cues · ${lams.reduce((s, [a, z]) => s + z - a, 0).toFixed(1)} s en total`);
...
// ── imports del cues ────────────────────────────────────────────────────────────────────────────
const PIEZAS = new Set(["Clip", "Foto", "LaminaZoom"]);
const fromStage = ["Clip", "Foto"].concat([...usedComps].filter((c) => PIEZAS.has(c)));
const fromFiles = [...usedComps].filter((c) => !PIEZAS.has(c));
const imports = [
  `import React from "react";`,
  `import { ${[...new Set(fromStage)].sort().join(", ")} } from "../farinon/Piezas";`,
...
// ── COMPUERTA <Video> ───────────────────────────────────────────────────────────────────────────
{
  ...
  for (const dir of [`src/${KIT}`, "src/farinon"]) for (const fn of fs.readdirSync(dir)) if (/\.tsx?$/.test(fn)) files.push(`${dir}/${fn}`);
  const malos = [];
  let mirados = 0;
