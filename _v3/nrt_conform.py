# nrt_conform.py — normaliza los clips de stock de public/broll/nrtinnitus/{s_,x_}*.mp4:
...
# Original respaldado en D:/rtmp/nrt_work/stock_orig (nunca se borra). Imprime cuántos midió.
import os, re, subprocess, shutil, glob, sys
D = "public/broll/nrtinnitus"; BAK = "D:/rtmp/nrt_work/stock_orig"; os.makedirs(BAK, exist_ok=True)
...
for p in sorted(glob.glob(f"{D}/[sx]_m*.mp4")):
    ...
    if ss: trim += 1
    tmp = p + ".part.mp4"
    r = subprocess.run(["ffmpeg", "-v", "error", "-y", "-ss", str(ss), "-i", orig, "-t", "10", "-an", "-vf", "scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,fps=30,setsar=1,format=yuv420p", "-c:v", "libx264", "-crf", "20", "-preset", "veryfast", "-movflags", "+faststart", tmp])
