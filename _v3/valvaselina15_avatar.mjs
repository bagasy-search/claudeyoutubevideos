// UN /run RunPod InfiniteTalk con el reel de ventanas visibles. Ref = public/ref_valvaselina15.png TAL CUAL.
import fs from "fs";
...
const JF = "_work/valvaselina15/av/avatar_job.json";
const sleep = (ms) => new Promise(r=>setTimeout(r,ms));
const {U,K} = supaCreds();
const files = [["face.png","public/ref_valvaselina15.png","image/png"],["reel.wav","_work/valvaselina15/av/reel.wav","audio/wav"]];
let job = fs.existsSync(JF) ? JSON.parse(fs.readFileSync(JF,"utf8")) : {};
if (!job.id) {
  const urls = {};
  for (const [n,l,ct] of files) {
    const obj = `tmp_avatar/valvaselina15_${n}`;
  ...
  const body = { input: { prompt: "A warm female doctor in her sixties speaks naturally and kindly to the camera in her bright office, natural head movement, subtle expressions, realistic lighting", image: urls["face.png"], audio: urls["reel.wav"], size: "720p" }, policy: { executionTimeout: 7200000 } };
...
fs.writeFileSync("_work/valvaselina15/av/avatar_raw.mp4", Buffer.from(await mp4.arrayBuffer()));
job.cost = st.output.cost; job.done = true; fs.writeFileSync(JF, JSON.stringify(job,null,1));
for (const [n] of files) await fetch(`${U}/storage/v1/object/thumbnails/tmp_avatar/valvaselina15_${n}`,{method:"DELETE",headers:{apikey:K,Authorization:`Bearer ${K}`}}).catch(()=>{});
