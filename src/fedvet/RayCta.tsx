...
export const RayCta: React.FC<{
  ...
  title = "Un video nuevo cada semana",
  sub = "Perros mayores, explicado por un veterinario.",
  domain = "",
  eyebrow = "Federer Veterinario",
  qr = undefined,
  showQr = true,
}) => {
        ...
        <div style={{ position: "relative", background: `linear-gradient(160deg, ${rgba(V.ink1, 0.96)}, ${rgba(V.ink0, 0.97)})`, border: `2px solid ${V.brass}`, borderRadius: 14, boxShadow: `0 18px 60px rgba(0,0,0,0.78), 0 0 0 1px ${rgba(V.brass, 0.25)}, inset 0 0 40px ${rgba(V.brass, 0.06)}`, padding: "26px 28px", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 22, alignItems: "center" }}>
            ...
            <div style={{ flex: "1 1 auto", minWidth: 0 }}>
              ...
              {/* ⛔ El dominio vivia SOLO dentro del bloque del QR, asi que con showQr=false la
                  tarjeta salia sin la URL: invitaba a una guia sin decir donde esta (medido en
                  dale3 y dale5). Sin QR, la URL va como linea propia debajo del subtitulo. */}
              {!showQr && domain ? (
                <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ height: 2, width: 26, background: V.brass, borderRadius: 1 }} />
                  <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 26, letterSpacing: 0.6, color: V.brass, textShadow: "0 2px 10px rgba(0,0,0,0.85)" }}>
                    {domain}
                  </div>
                </div>
              ) : null}
