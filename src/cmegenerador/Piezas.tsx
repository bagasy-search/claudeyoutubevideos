...
export const Clip: React.FC<{ src: string }> = ({ src }) => (
  <AbsoluteFill style={{ backgroundColor: V.ink0 }}>
    {/* ⛔⛔ `OffthreadVideo`, NUNCA `<Video>`. Medido en el render de este video: `<Video>` usa el
        elemento <video> del navegador y en un render por chunks entrega el cuadro EQUIVOCADO —
        no repetido, DESORDENADO. La diferencia entre cuadros consecutivos de la cara pasaba de
        1,99 a 14,33 (el archivo fuente daba 3,5-4,2 parejo). Eso es el "se ve todo lageado" que
        el creador viene marcando en TODOS los videos del canal: el avatar y los clips iban con
        `<Video>` y sólo los movimientos (que usan OffthreadVideo) nunca tiritaron.
        `OffthreadVideo` saca el cuadro exacto con ffmpeg. Los dos van muteados, no se pierde nada. */}
