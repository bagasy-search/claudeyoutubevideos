...
export const MovTrescientos: React.FC<{ acto: number; gFrame: number }> = ({ acto, gFrame }) => {
  ...
  const carItems: { src: string; kind?: "video" | "photo"; label?: string }[] = [
    ...
    { src: "img/cmegenerador/cmeg_mv_tres6.png", kind: "photo", label: "LA LUZ DEL PASILLO" },
    { src: "img/cmegenerador/cmeg_mv_tres1.png", kind: "photo", label: "LA CUADRA A OSCURAS" },
  ];
      ...
      <Layers cam={camT}>
        ...
        {g < 300 && (
          <AbsoluteFill style={{ opacity: a1On ? ip(g, [0, 14, 258, 270], [0, 1, 1, 0.86]) : 0 }}>
            <PhotoPlane src="img/cmegenerador/cmeg_mv_tres1.png" kind="photo" z={-640}
              scale={1.42} dim={ip(g, [0, 52, 74], [0.30, 0.34, 0.74])} tint={V.amber} />
          </AbsoluteFill>
        )}
        {g >= 268 && g < 664 && (
          <AbsoluteFill style={{ opacity: a2WideOn }}>
            <PhotoPlane src="img/cmegenerador/cmeg_mv_tres6.png" kind="photo" z={-600}
              scale={1.26} dim={0.7} tint={V.volt} />
          </AbsoluteFill>
        )}
        {g >= 890 && (
          <AbsoluteFill style={{ opacity: ip(g, [890, 940, 1236, 1272], [0, 0.55, 0.55, 0.12]) }}>
            <PhotoPlane src="img/cmegenerador/cmeg_mv_tres6.png" kind="photo" z={-660}
