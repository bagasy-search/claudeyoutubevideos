...
// ║ AC ║ enterFrom                             ║ exitTo                                        ║
// ╠════╬═══════════════════════════════════════╬═══════════════════════════════════════════════╣
// ║ 1  ║ CÁM: viene de MovDosPreguntas, media  ║ CÁM: z≈-90→+26 y siguiendo su vector hacia     ║
// ║ g0 ║ distancia, retrocedida (z≈-90),       ║      adelante-izquierda. NO frena en la        ║
...
// ╠════╬═══════════════════════════════════════╬═══════════════════════════════════════════════╣
// ║ 2  ║ CÁM: z≈+26 avanzando, misma inercia.  ║ CÁM: z≈-34, ya retrocediendo y bajando (la     ║
...
// ╠════╬═══════════════════════════════════════╬═══════════════════════════════════════════════╣
// ║ 3  ║ CÁM: z≈-34 retrocediendo y bajando.   ║ CÁM: sigue BAJANDO y acelera (translateY       ║
...
// ╠════╬═══════════════════════════════════════╬═══════════════════════════════════════════════╣
// ║ 4  ║ CÁM: cayendo hacia la mesa, misma     ║ CÁM: z≈+94, quieta sobre la mesa con deriva    ║
...
const Barra: React.FC<{
  ...
  return (
    <div style={{ position: "absolute", left: `${x}%`, top: 0, opacity: clamp01(on) }}>
      ...
      <div style={{
        position: "absolute", left: 0, top: topPx - 74, width: w + 110, marginLeft: -(w + 110) / 2,
        textAlign: "center",
      }}>
        <div style={{
...
export const MovTresNumeros: React.FC<{ acto: number; gFrame: number }> = ({ acto, gFrame }) => {
  ...
  const camB = gcam(g, { z0: -90, z1: 80, panX: -38, panY: 22, ry: -4.4, rx: 1.5, dur: END });
  const camZ = ip(g,
    [0, 110, 300, 420, 560, 700, 900, 1010, 1180, 1330, 1380, 1470, 1620, 1800],
    [-150, -80, -10, 26, 2, 40, -34, 30, 72, 40, -24, 30, 64, 14]);
  ...
  const x1 = ip(g, k1, [47, 46, 45, 39, 38, 14, 14, 14.6, 17.5, 20.5, 20.5]);
  const y1 = ip(g, k1, [46, 46, 46, 46, 46, 22, 22, 43, 70, 76, 76]);
  const z1 = ip(g, k1, [40, 34, 30, 26, 24, -110, -110, -40, 20, 18, 18]);
  ...
  const h2 = ip(g, k2, [160, 648, 644, 636, 360, 360, 560, 520, 420, 140, 140, 162, 152, 152, 152]);
  const x2 = ip(g, k2, [64, 51, 51, 51, 51, 51, 44, 44, 36, 14, 14, 17.5, 24.5, 29.5, 29.5]);
  const y2 = ip(g, k2, [30, 44, 44, 44, 43.5, 43.5, 40, 40, 38, 37, 37, 50, 72, 76, 76]);
  const z2 = ip(g, k2, [4, 26, 28, 30, 16, 16, 40, 40, 36, -100, -100, -30, 20, 18, 18]);
  ...
  const vid2 = g < A2 ? 0 : ip(g, [420, 548, 568], [1, 1, 0]);
  const vid2b = g < A3 ? 0 : ip(g, [900, 1028, 1048], [1, 1, 0]);
  ...
  const x3 = ip(g, k3, [108, 79, 79, 14, 14, 46, 39, 39, 39, 39, 39]);
  const y3 = ip(g, k3, [64, 64, 64, 52, 52, 58, 70, 67, 67, 76, 76]);
  const z3 = ip(g, k3, [16, 16, 16, -110, -110, 60, 26, 60, 60, 18, 18]);
  ...
  const vid3 = g < A4 ? 0 : ip(g, [1380, 1508, 1528], [1, 1, 0]);
  ...
  const barBase = ip(g, [690, 1300, 1380], [88, 88, 106]);
  ...
  const hA = ipe(g, [700, 748], [0, 99], Easing.out(Easing.cubic)) * barK;
  const hB = ipe(g, [730, 778], [0, 216], Easing.out(Easing.cubic)) * barK;
  const hC = ipe(g, [760, 808], [0, 144], Easing.out(Easing.cubic)) * barK;
  ...
  const cifra3X = ip(g, [0, 54, 96], [50, 70, 82]);
  const cifra3Y = ip(g, [0, 54, 96], [44, 22, 9.2]);
  const cifra3S = ip(g, [0, 54, 96], [190, 112, 58]);
  ...
  const wNumX = ip(g, [188, 392, 430, 452], [64, 64, 26, 14.5]);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      ...
      <Layers cam={camT}>
        ...
        {!fondoTardio && (
          <PhotoPlane src="img/cmegenerador/cmeg_mv_tnum1.jpg" kind="photo" z={-640}
            scale={ip(g, [0, 900], [1.30, 1.20])} dim={ip(g, [0, 60, 420, 900], [0.46, 0.54, 0.68, 0.74])}
            tint={V.volt} />
        )}
        {fondoTardio && (
          <PhotoPlane src="img/cmegenerador/cmeg_mv_tnum2.jpg" kind="photo" z={-660}
        ...
        {g >= 1288 && (
          ...
          >
            <Mat photo="img/cmegenerador/cmeg_mv_tnum4.jpg" clip="broll/cmegenerador/cmeg_mv_tnum4.mp4"
              vid={g < A4 ? 0 : ip(g, [1380, 1500, 1524], [1, 1, 0])}
              w={1240} h={700}
        ...
        {g >= 452 && g < 1420 && (
          <Plane z={-115} style={{ opacity: ip(g, [452, 480, 1330, 1408], [0, 1, 1, 0]) }}>
            {[22, 37, 52].map((yy, i) => (
              <div key={i} style={{
                position: "absolute", left: "14%", top: `${yy}%`, width: 238, height: 146,
        ...
        <Plane z={0}>
          <Ventana x={x1} y={y1} w={w1} h={h1} z={z1} ry={ry1} rx={rx1}
            ...
            <Mat photo="img/cmegenerador/cmeg_mv_tnum1.jpg" clip="broll/cmegenerador/cmeg_mv_tnum1.mp4"
        ...
        {g >= 390 && (
          <Plane z={0}>
            <Ventana x={x2} y={y2} w={w2} h={h2} z={z2} ry={ry2} rx={rx2}
              ...
              <Mat photo="img/cmegenerador/cmeg_mv_tnum2.jpg" clip="broll/cmegenerador/cmeg_mv_tnum2.mp4"
        ...
        {g >= 266 && (
          <Plane z={0}>
            <Ventana x={x3} y={y3} w={w3} h={h3} z={z3} ry={ry3} rx={rx3}
              ...
              <Mat photo="img/cmegenerador/cmeg_mv_tnum3.jpg" clip="broll/cmegenerador/cmeg_mv_tnum3.mp4"
        ...
        {g >= 656 && g < 962 && (
          <Plane z={-10} style={{ opacity: acOn }}>
            <Ventana x={xA} y={43.5} w={360} h={300} z={-30} ry={9} radius={10} lit={0.82} litColor={V.sky}>
              <Mat photo="img/cmegenerador/cmeg_mv_tnum2.jpg" w={360} h={300} k={1.9}
            ...
            <Ventana x={xC} y={43.5} w={360} h={300} z={-30} ry={-9} radius={10} lit={0.82} litColor={V.copper}>
              <Mat photo="img/cmegenerador/cmeg_mv_tnum2.jpg" w={360} h={300} k={2.2}
        ...
        {lamOn > 0 && (
          <Plane z={30}>
            <Ventana x={80.5} y={lamY2} w={248} h={351} z={-14} ry={-13} rx={13}
              ...
              <Mat photo="img/cmegenerador/cmeg_lam_podio.jpg" w={248} h={351} k={1.04}
            ...
            <Ventana x={71} y={lamY} w={330} h={467} z={20} ry={-9} rx={14}
              ...
              <Mat photo="img/cmegenerador/cmeg_lam_60a_caros.jpg" w={330} h={467} k={1.04}
        ...
        {g >= 1250 && g < 1348 && (
          <Plane z={60}>
            <IconPng src="img/cmegenerador/cmeg_ic_rayo.png" x={77} y={57}
      ...
      <AbsoluteFill style={{ pointerEvents: "none", transform: hud }}>
        ...
        <div style={{ position: "absolute", left: "89%", top: "9%", transform: "translate(-50%,-50%)", opacity: ip(g, [70, 108], [0, 1]) }}>
          <div style={{
        ...
        {t1 > 0.01 && (
          <div style={{ position: "absolute", left: "6%", top: "74%", opacity: t1, transform: `translateY(${((1 - t1) * 22).toFixed(1)}px)` }}>
            <Bed w={640} pad={22}>
        ...
        {t2 > 0.01 && (
          <div style={{ position: "absolute", left: "6%", top: "7%", opacity: t2, transform: `translateY(${((1 - t2) * -22).toFixed(1)}px)` }}>
            <Bed w={660} pad={22}>
        ...
        {sumaTach > 0.01 && g < 1348 && (
          <div style={{
            position: "absolute", left: "55%", top: "86%", transform: "translate(-50%,-50%)",
