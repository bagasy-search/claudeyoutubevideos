// src/mdheater/Piezas.tsx — piezas de render de `mdheater` (canal Mike Dalton, EN).
// Clon de mdgrout/Piezas con exports PROPIOS (Mdh*) — ⛔ renombrar el export o el import nombrado
// llega undefined -> React #130 -> mueren los 60 chunks, y tsc no lo marca.
// Arquitectura POR VENTANAS: el avatar NO es piso continuo; son clips (MdhAvatar) por corte, y el
...
export const MdhFoto: React.FC<{ durationInFrames: number; src: string; seed: number; darken?: number }> = ({ src, seed, darken = 0.08 }) => {
...
export const MdhAvatar: React.FC<{ durationInFrames: number; src: string; seed: number }> = ({ seed, src }) => {
...
export const MdhBed: React.FC<{ durationInFrames: number; src: string; seed: number; children?: React.ReactNode }> = ({ durationInFrames, src, seed, children }) => (
  <AbsoluteFill>
    <MdhFoto durationInFrames={durationInFrames} src={src} seed={seed} darken={0.42} />
    {children}
  </AbsoluteFill>
);
