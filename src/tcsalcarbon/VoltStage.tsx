...
export const VoltAtmos: React.FC<{
...
}> = ({ tint = V.volt, tint2 = V.amber, keyFrom = 0.22, intensity = 1, floor = 0.20 }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      ...
      {/* ⛔ cama de abajo: llegaba al 55% desde la MITAD del cuadro y apagaba la foto en todos los
          movimientos. El rotulo ya trae su propia cama (`Bed`), asi que aca va apenas un apoyo. */}
      <AbsoluteFill style={{ background: `linear-gradient(180deg, rgba(0,0,0,0) 74%, rgba(0,0,0,${floor}) 100%)` }} />
...
export const PromiseGap: React.FC<{
  ...
  promise, measured, unit = "W", slats = 13,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `repeating-linear-gradient(-58deg, ${rgba(V.orange, 0.05 + 0.05 * gapPulse)} 0 7px, transparent 7px 20px)`,
        ...
        return (
          <React.Fragment key={i}>
            <div style={{
              ...
              background: rgba(V.orange, 0.12 + 0.16 * ph), borderRadius: 2,
            }} />
            <div style={{
              position: "absolute", left, bottom: 0, width: slatW, height: hM,
              background: rgba(V.volt, 0.34 + 0.22 * ph), borderRadius: 2,
              boxShadow: `0 0 ${Math.round(6 + 9 * ph)}px ${rgba(V.volt, 0.22)}`,
            }} />
          </React.Fragment>
        );
      })}
        ...
        <Sequence from={Math.floor(frame / CLIP_FRAMES) * CLIP_FRAMES} durationInFrames={CLIP_FRAMES} layout="none">
          <OffthreadVideo src={staticFile(src)} muted startFrom={startFrom}
            style={{
              ...
              // ⛔⛔ EL MOVIMIENTO VA POR CSS, NUNCA HORNEADO EN EL ARCHIVO. Todo `zoompan`/`fps`/
              // ida-y-vuelta de ffmpeg cuantiza a pixel entero: unos cuadros no se mueven y otros
              // saltan 2 px, y eso se ve como TIRON en todo el metraje (medido: 1.11 1.83 1.08 3.39
              // 1.07 ... contra la curva suave de un clip nativo). El transform de Remotion es
              // subpixel, asi que un clip QUIETO + este push se lee mas fluido que uno con el
              // movimiento horneado.
              transform: `scale(${(1.012 + (frame % CLIP_FRAMES) * 0.00042).toFixed(5)})`,
            }} />
        </Sequence>
      ) : (
