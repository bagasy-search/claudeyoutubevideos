...
export type CamKf = {time: number; fx: number; fy: number; z: number};
...
export const FedWhiteboardBase: React.FC<FedWhiteboardProps> = ({scene = SCENE_ROMERO, theme: themeKey = 'white'}) => {
  const frame = useCurrentFrame();
  ...
  return (
    <ThemeCtx.Provider value={theme}>
      <AbsoluteFill style={{background: theme.bg}}>
        ...
        {/* PiP ELIMINADO a propósito: en Federer Archivos el avatar va FULL o
            HIDDEN, nunca en recuadro. La pizarra ocupa la pantalla entera y el
            audio sigue viniendo del avatar persistente del Main. */}
...
/* ==================== PIZARRA DE ESTE VIDEO (sin PiP) ====================
 * Wrapper de un solo uso: la escena del MECANISMO NOCTURNO (la bomba de la
 * pantorrilla / el segundo corazón). Tema BLANCO a propósito: en este canal la
 * pizarra es el corte de respiración claro dentro del look dark-cinematic.
 * El audio sigue viniendo del avatar persistente del Main — acá no hay video.
 * ---------------------------------------------------------------------- */
import {SCENE_HORMIGUEO} from './FedWhiteboardScene_vucm3bvd4u3k';

export const FedWhiteboardVucm: React.FC<{totalF?: number}> = () => (
  <FedWhiteboardBase scene={SCENE_HORMIGUEO} theme="white" />
);
