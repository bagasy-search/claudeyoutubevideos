// Piezas.tsx — piezas de escena del video `valmaicena` (canal Doctora Valeria Alcázar).
...
export const Lamina: React.FC<{ src: string; zoom?: string; desde?: string }> = ({ src, zoom = "completa", desde }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#F4EBD9", overflow: "hidden" }}>
...
/** Clip de agnes hasta su ÚLTIMO cuadro real y, si el plano dura más, su último cuadro quieto, todo bajo
 *  la MISMA curva Ken-Burns (nunca se repite ni se estira el clip). */
export const ClipCola: React.FC<{ src: string; last: string; seed: number; frames: number }> = ({ src, last, seed, frames }) => {
  const kb = useKenBurns(seed, 1.04, 0.03, 0.08, 0.8);
  const st: React.CSSProperties = { position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" };
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      <AbsoluteFill style={{ ...kb }}>
        <Sequence from={0} durationInFrames={Math.max(1, frames - 1)} layout="none">
          <OffthreadVideo src={staticFile(src)} muted style={st} />
        </Sequence>
        <Sequence from={Math.max(1, frames - 1)} layout="none">
