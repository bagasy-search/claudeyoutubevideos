// Kit.tsx — kit PREMIUM del video `valmaicena` (canal Doctora Valeria Alcázar). Clon rebrandeado de famanchascara.
// Look: EDITORIAL VINTAGE CLARO — papel crema, tinta espresso, latón/oro, terracota; Playfair/Garamond/Caveat/Archivo.
...
import { FONT_DISPLAY, FONT_SERIF, FONT_SANS, FONT_HAND } from "../valeria/theme";
const F_INTER = FONT_SERIF;
const F_OSWALD = FONT_DISPLAY;
export const F_HAND = FONT_HAND;
...
export const Bed: React.FC<{ src?: string; seed?: number; blur?: number; wash?: number; startFrom?: number }> = ({ src, seed = 7, blur = 0, wash = 0, startFrom = 0 }) => {
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", background: C.paper }}>
      ...
      {wash > 0 && <AbsoluteFill style={{ background: `rgba(244,235,217,${wash})` }} />}
    </AbsoluteFill>
  );
};
...
const Card: React.FC<{ style?: React.CSSProperties; children: React.ReactNode; accent?: string }> = ({ style, children, accent = C.teal }) => (
  <div style={{
    background: C.paper2, borderRadius: 30, boxShadow: "0 30px 70px rgba(51,37,26,0.28), 0 4px 14px rgba(51,37,26,0.12)",
...
const Kicker: React.FC<{ children: React.ReactNode; color?: string }> = ({ children, color = C.tealDeep }) => (
  <div style={{ fontFamily: FONT_SANS, fontWeight: 800, fontSize: 26, letterSpacing: 4, textTransform: "uppercase", color }}>{children}</div>
);
const Title: React.FC<{ children: React.ReactNode; size?: number; color?: string; style?: React.CSSProperties }> = ({ children, size = 64, color = C.ink, style }) => (
  <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 800, fontSize: size, lineHeight: 1.08, color, letterSpacing: -0.5, ...style }}>{children}</div>
);
...
export const RecipeCard: React.FC<{ bed: string; photo: string; kicker: string; title: string; rows: { k: string; v: string }[]; hits?: number[]; durationInFrames?: number }> = ({ bed, photo, kicker, title, rows, hits = [] }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 110, top: 120, width: 640, height: 840, borderRadius: 30, overflow: "hidden", boxShadow: "0 40px 90px rgba(51,37,26,0.35)", transform: `rotate(-2.5deg) translateY(${(1 - card) * 60}px)`, opacity: card, border: "12px solid #fff" }}>
...
export const DayNight: React.FC<{ nightBed: string; dayBed: string; nightTitle: string; nightText: string; dayTitle: string; dayText: string; flipAt?: number; durationInFrames?: number }> = ({ nightBed, dayBed, nightTitle, nightText, dayTitle, dayText, flipAt = 2 }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out, background: C.paper }}>
      ...
      {Half({ bed: dayBed, left: false, p: b, title: dayTitle, text: dayText, icon: "", col: C.amber })}
...
export const QuoteCard: React.FC<{ bed: string; image: string; quote: string; attrib: string; durationInFrames?: number }> = ({ bed, image, quote, attrib }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 130, top: 170, width: 560, height: 740, borderRadius: 26, overflow: "hidden", border: "14px solid #fff", boxShadow: "0 40px 80px rgba(51,37,26,0.35)", transform: `rotate(2deg) scale(${0.9 + 0.1 * p})`, opacity: p }}>
      ...
      <div style={{ position: "absolute", left: 790, top: 230, width: 1000 }}>
        ...
        <div style={{ fontFamily: F_INTER, fontWeight: 800, fontSize: 64, lineHeight: 1.18, color: C.ink, background: "rgba(251,245,232,0.92)", padding: "30px 40px", borderRadius: 24, boxShadow: "0 20px 50px rgba(51,37,26,0.18)" }}>
          {words.map((w, i) => {
...
export const SkinDiagram: React.FC<{ title: string; labels: { text: string }[]; sunAt?: number; spotAt?: number; durationInFrames?: number }> = ({ title, labels, sunAt = 1.2, spotAt = 3 }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out, background: `linear-gradient(180deg, #EFE3CC 0%, ${C.paper} 100%)` }}>
  ...
  const Col = ({ img, d, at, x, col, tag }: { img: string; d: { title: string; sub: string }; at: number; x: number; col: string; tag: string }) => {
    ...
    return (
      <div style={{ position: "absolute", left: x, top: 130, width: 780, ...lift(p, 60) }}>
        <div style={{ height: 470, borderRadius: 26, overflow: "hidden", border: "12px solid #fff", boxShadow: "0 30px 70px rgba(51,37,26,0.3)" }}>
      ...
      {callouts.map((c, i) => {
        ...
        return (
          <div key={i} style={{ position: "absolute", left: c.side === "l" ? undefined : lx, right: c.side === "l" ? 1920 - lx : undefined, top: ly - 42, opacity: p, transform: `scale(${0.85 + 0.15 * p})` }}>
            <div style={{ background: C.paper2, borderLeft: `10px solid ${C.teal}`, borderRadius: 16, padding: "14px 26px", fontFamily: F_INTER, fontWeight: 800, fontSize: 40, color: C.ink, boxShadow: "0 16px 36px rgba(51,37,26,0.25)", whiteSpace: "nowrap" }}>{c.text}</div>
          </div>
        );
      })}
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, top: 90, textAlign: "center", ...lift(t) }}>
        <span style={{ background: C.paper2, fontFamily: F_INTER, fontWeight: 900, fontSize: 64, color: C.ink, padding: "14px 44px", borderRadius: 20, boxShadow: "0 16px 40px rgba(51,37,26,0.2)" }}>{title}</span>
      </div>
        ...
        {items.map((it, i) => {
          ...
          return (
            <div key={i} style={{ width: 380, opacity: p, transform: `translateY(${(1 - p) * 50}px)` }}>
              <div style={{ background: C.paper2, borderRadius: 26, overflow: "hidden", boxShadow: "0 24px 56px rgba(51,37,26,0.28)", position: "relative" }}>
...
export const MixRatio: React.FC<{ bed: string; title: string; a: { n: number; label: string }; b: { n: number; label: string }; note: string; icon?: "drop" | "spoon"; hits?: number[]; durationInFrames?: number }> = ({ bed, title, a, b, note, icon = "drop", hits = [] }) => {
  const f = useCurrentFrame();
  const out = useOut();
  const t = usePop(0.05);
    ...
    <div style={{ display: "flex", gap: 14, justifyContent: "center", height: 130 }}>
      {Array.from({ length: n }).map((_, i) => (
        <svg key={i} width={80} height={120} viewBox="-60 -100 120 200" style={{ transform: `translateY(${(1 - Math.min(1, p * 1.5 - i * 0.15)) * -60}px)`, opacity: Math.max(0, Math.min(1, p * 1.5 - i * 0.15)) }}>
          {icon === "spoon"
            ? <g transform="rotate(-30) scale(1.45)"><rect x={-9} y={-5} width={18} height={100} rx={9} fill="#9C7A4E" /><ellipse cx={0} cy={-48} rx={44} ry={56} fill="#B08B5C" /><ellipse cx={0} cy={-52} rx={34} ry={42} fill="#FFFDF6" /><ellipse cx={-10} cy={-62} rx={10} ry={14} fill="#fff" opacity={0.8} /></g>
...
export const ChapterCard: React.FC<{ bed: string; number: string; title: string; sub?: string; durationInFrames?: number }> = ({ bed, number, title, sub }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${wipe * 58}%`, background: "rgba(244,235,217,0.94)", boxShadow: "20px 0 60px rgba(51,37,26,0.2)" }} />
        ...
        {words.map((w, i) => {
          ...
          return (
            <span key={i} style={{
              ...
              background: w.hl ? C.amber : C.paper2, color: C.ink, boxShadow: "0 14px 34px rgba(51,37,26,0.3)",
...
export const NameTag: React.FC<{ name: string; role: string; durationInFrames?: number }> = ({ name, role }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      <div style={{ position: "absolute", left: 110, bottom: 120, display: "flex", alignItems: "stretch", overflow: "hidden", borderRadius: 18, boxShadow: "0 18px 44px rgba(51,37,26,0.3)", clipPath: `inset(0 ${(1 - w) * 100}% 0 0)` }}>
      ...
      {marks.map((m, i) => {
        ...
        return (
          <React.Fragment key={i}>
            <div style={{ position: "absolute", left: x - 34, top: 493, width: 68, height: 68, borderRadius: 34, background: on ? C.teal : C.paper2, border: `8px solid ${on ? C.paper2 : C.line}`, boxShadow: "0 8px 20px rgba(51,37,26,0.25)", transform: `scale(${0.8 + 0.3 * p})` }} />
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 140, top: 110, width: 600, height: 860, perspective: 1800 }}>
        <Img src={staticFile(cover)} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14, boxShadow: "30px 40px 80px rgba(51,37,26,0.4)", transform: `rotateY(${tilt * 2}deg) translateY(${(1 - p) * 80}px)`, opacity: p }} />
      </div>
      <div style={{ position: "absolute", left: 830, top: 130, width: 960, ...lift(p) }}>
        <Card accent={C.amber}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 40, marginTop: 34, opacity: q, transform: `scale(${0.9 + 0.1 * q})`, transformOrigin: "left center" }}>
            <div style={{ background: "#fff", padding: 20, borderRadius: 18, boxShadow: "0 10px 30px rgba(51,37,26,0.2)" }}>
              <Img src={staticFile(qr)} style={{ width: 360, height: 360, display: "block" }} />
            </div>
            <div>
              <div style={{ fontSize: 34, fontWeight: 800, color: C.ink2 }}>En el televisor:<br/>escanee el código</div>
              <div style={{ fontSize: 34, fontWeight: 800, color: C.ink2, marginTop: 10 }}>En el teléfono:<br/>enlace en la descripción</div>
            </div>
          </div>
          <div style={{ fontFamily: FONT_SANS, fontWeight: 800, fontSize: 46, color: C.tealDeep, marginTop: 22, whiteSpace: "nowrap", opacity: q }}>{domain}</div>
        </Card>
      </div>
      <Sfx at={0.1} src="sfx_whoosh_soft.mp3" vol={0.25} />
...
export const PhotoPair: React.FC<{ bed: string; left: string; right: string; leftLabel: string; rightLabel: string; note: string; flipAt?: number; durationInFrames?: number }> = ({ bed, left, right, leftLabel, rightLabel, note, flipAt = 1.5 }) => {
  ...
  const Pol = ({ src, label, p, rot, x }: { src: string; label: string; p: number; rot: number; x: number }) => (
    <div style={{ position: "absolute", left: x, top: 150, width: 700, background: "#fff", padding: "24px 24px 30px", boxShadow: "0 36px 80px rgba(51,37,26,0.35)", transform: `rotate(${rot}deg) translateY(${(1 - p) * 120}px)`, opacity: p }}>
...
export const WindowRays: React.FC<{ bed: string; title: string; blocked: string; passes: string; durationInFrames?: number }> = ({ bed, title, blocked, passes }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <svg width={1920} height={1080} style={{ position: "absolute", inset: 0 }}>
        <rect x={930} y={180} width={60} height={760} rx={10} fill="rgba(239,227,204,0.85)" stroke="#fff" strokeWidth={6} />
...
export const DoseFingers: React.FC<{ bed: string; image: string; title: string; sub: string; durationInFrames?: number }> = ({ bed, image, title, sub }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", left: 150, top: 150, width: 820, height: 780, borderRadius: 30, overflow: "hidden", border: "14px solid #fff", boxShadow: "0 40px 90px rgba(51,37,26,0.35)", transform: `scale(${0.9 + 0.1 * p})`, opacity: p }}>
...
export const TonoWipe: React.FC<{ before: string; after: string; leftLabel: string; rightLabel: string; tag: string; durationInFrames?: number }> = ({ before, after, leftLabel, rightLabel, tag }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <div style={{ position: "absolute", right: 60, top: 50 }}><span style={{ background: "rgba(51,37,26,0.75)", fontFamily: F_INTER, fontWeight: 800, fontSize: 28, padding: "8px 20px", borderRadius: 10, color: "#fff", letterSpacing: 3 }}>{tag}</span></div>
