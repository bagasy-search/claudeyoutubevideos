...
export const VoltAtmos: React.FC<{
...
}> = ({ tint = V.volt, tint2 = V.amber, keyFrom = 0.22, intensity = 1, floor = 0.34 }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      ...
      <AbsoluteFill style={{ opacity: 0.028, backgroundImage: "repeating-conic-gradient(rgba(255,255,255,.5) 0% 25%, rgba(0,0,0,.5) 0% 50%)", backgroundSize: "3px 3px", mixBlendMode: "overlay" }} />
    </AbsoluteFill>
  );
};
...
}> = ({ src, z = -400, scale = 1.18, dim = 0.24, kind = "photo", startFrom = 0, tint = V.volt }) => {
  const frame = useCurrentFrame();
