...
export const AvatarWin: React.FC<{src: string; from?: number}> = ({src, from = 0}) => {
  const frame = useCurrentFrame();
  const {durationInFrames} = useVideoConfig();
  ...
  return (
    <AbsoluteFill style={{background: '#1b1512', overflow: 'hidden'}}>
      <AbsoluteFill style={{transform: `scale(${z.toFixed(4)})`, transformOrigin: '50% 38%'}}>
        <OffthreadVideo src={staticFile(src)} muted startFrom={Math.round(from * 30)} style={FILL} />
      </AbsoluteFill>
    </AbsoluteFill>
  );
};

// plano generado (gpt-image → agnes): el clip corre sus cuadros reales y después queda su ÚLTIMO cuadro con la MISMA curva Ken-Burns.
export const Gen: React.FC<{src?: string | null; still: string; last?: string; seed: number; frames?: number}> = ({src, still, last, seed, frames}) => {
  const frame = useCurrentFrame();
  const t = useKenBurns(seed, 1.04, 0.04, 0.1, 0.9);
  const playing = src && frames && frames > 1 && frame < frames - 1;
  return (
    <AbsoluteFill style={{background: '#1b1512', overflow: 'hidden'}}>
      <AbsoluteFill style={t}>
        {playing ? <OffthreadVideo src={staticFile(src as string)} muted style={FILL} /> : <Img src={staticFile(src && last ? last : still)} style={FILL} />}
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
