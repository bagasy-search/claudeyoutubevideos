...
const Exploded:React.FC=()=>{const f=useCurrentFrame();const k=p(f,0,52);return <Stage>
 ...
 <div style={{position:'absolute',left:655,top:445,width:1170,height:570,perspective:1600}}><div style={{...fill,...keep3d,transform:`rotateX(-9deg) rotateY(${-25+k*8}deg) scale(.84)`}}>
  ...
  <div style={{position:'absolute',left:-125,top:140,transform:'translateZ(250px) rotateZ(0deg) scale(.42)',transformOrigin:'0 50%'}}><Screw id="assembly-screw"/></div>
  <svg style={{position:'absolute',left:30,top:155,transform:'translateZ(280px)',overflow:'visible'}} width="800" height="200"><path d="M0 135H410" fill="none" stroke={C.red} strokeWidth="3" strokeDasharray="8 8" strokeDashoffset={-f*.5}/><circle cx="410" cy="135" r="22" fill={C.ink} stroke={C.red} strokeWidth="3"/></svg>
 </div></div>
...
const Guide:React.FC=()=>{const f=useCurrentFrame();const k=p(f,0,24);return <div style={{position:'absolute',right:52,top:70,width:574,height:220,display:'flex',padding:16,gap:18,boxSizing:'border-box',borderRadius:16,border:'1px solid #c7a15b',background:'linear-gradient(125deg,#122120,#080f10)',boxShadow:'0 22px 55px #0008',opacity:k,transform:`translateY(${(1-k)*26}px)`}}>
 <div style={{flex:1,padding:'7px 0 0 7px'}}><div style={{fontFamily:BODY,fontSize:13,fontWeight:600,letterSpacing:2,color:C.gold}}>CHECK YOUR DOOR TODAY</div><div style={{fontFamily:DISPLAY,fontSize:32,lineHeight:1.12,color:C.paper,marginTop:12}}>The One<br/>Afternoon Door</div><div style={{marginTop:17,borderTop:'1px solid #ad8d4b66',paddingTop:11,fontFamily:BODY,fontSize:13,fontWeight:600,color:C.gold,letterSpacing:.7}}>SCAN TO SEE THE GUIDE →</div></div>
 <div style={{width:184,flexShrink:0,background:'white',borderRadius:8,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}><Img src={file('guide_qr.png')} style={{width:176,height:176,objectFit:'contain'}}/><span style={{fontFamily:BODY,fontSize:10,fontWeight:700,color:'#152021',marginTop:-3}}>raykessler.vercel.app</span></div>
 </div>};
