...
const Lock: React.FC<{ size: number; wobble: number }> = ({ size, wobble }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" style={{ transform: `rotate(${wobble.toFixed(2)}deg)`, filter: `drop-shadow(0 10px 22px ${rgba("#000", 0.6)})` }}>
    <path d="M32 46 V34 a18 18 0 0 1 36 0 V46" fill="none" stroke="#DCEBF7" strokeWidth={9} strokeLinecap="round" />
      ...
      <AbsoluteFill style={{ transform: `scale(${cam.scale.toFixed(4)})`, transformOrigin: "50% 55%" }}>
        ...
        <Plane depth={0.95} camX={cam.camX} camY={cam.camY} z={5}>
          <div style={{ position: "absolute", left: 0, right: 0, top: 96, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 70, color: V.white, marginTop: 12, textShadow: "0 6px 30px rgba(0,0,0,0.9)", opacity: ramp(frame, 10, 24), transform: `translateY(${((1 - ramp(frame, 10, 24)) * 16).toFixed(1)}px)` }}>{title}</div>
          </div>
        </Plane>
      </AbsoluteFill>
