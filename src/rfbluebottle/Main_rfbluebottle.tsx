...
import { FileOpen, BottleDossier, CaseChapter, FileRecap, FileCta } from "./FileKit";

// ── CANAL "THE ROWE FILES" (EN) · rfbluebottle — look expediente ─────────────────────────────
// Audio = máster Fish (voz `rowe`). Avatar = InfiniteTalk/RunPod SÓLO en las ventanas visibles
// (reel `rfbluebottle_avatar.mp4`, recortado con trimBefore). Todo video por OffthreadVideo.
// ⛔ Todas las props de texto e imagen van por props (defaults del kit en español / otra marca).
export const TOTAL_FRAMES_RFBLUEBOTTLE = TOTAL_FRAMES;
const AVATAR_REEL = "rfbluebottle_avatar.mp4";
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "presenter" ? <RowePresenter durationInFrames={d} name={TAG} mode={b.mode} img={b.img} bg={b.bg} kicker={b.kicker} role={b.role} cta="SUBSCRIBE" />
  ...
  : b.kind === "fileopen" ? <FileOpen durationInFrames={d} tab={b.tab} title={b.title} photo={b.photo} bed={b.bed} fields={b.fields} stamp={b.stamp} stampAt={b.stampAt} />
  : b.kind === "bottle" ? <BottleDossier durationInFrames={d} img={b.img} bed={b.bed} kicker={b.kicker} title={b.title} tags={b.tags} />
  : b.kind === "chapter" ? <CaseChapter durationInFrames={d} exhibit={b.exhibit} title={b.title} bed={b.bed} stamp={b.stamp} />
  : b.kind === "recap" ? <FileRecap durationInFrames={d} kicker={b.kicker} title={b.title} photo={b.photo} bed={b.bed} items={b.items} />
  : b.kind === "cta" ? <FileCta durationInFrames={d} covers={b.covers} qr={b.qr} domain={b.domain} bed={b.bed} kicker={b.kicker} title={b.title} lines={b.lines} />
  : null;

export const MainRfbluebottle: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: "#0E1D23" }}>
    <Audio src={staticFile("rfbluebottle.m4a")} />
