      ...
      <div style={{ position: "absolute", left: 120, top: 96, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 22 }}>
          {questions.map((q, i) => {
            ...
            return <div key={i} style={{ width: 54, height: 10, borderRadius: 5, background: k > 0.5 ? (q.tone === "danger" ? V.amber : V.brass) : rgba("#ffffff", 0.22), boxShadow: k > 0.5 ? `0 0 14px ${rgba(V.brass, 0.6)}` : "none" }} />;
          })}
        </div>
      </div>
          ...
          return (
            <div key={i} style={{
              ...
              <GlassPhoto w={W} h={H} blur={p.blur} glow={active ? acc : undefined} glowK={active ? ramp(frame, q.at + 6, q.at + 20) * 0.8 : 0}>
                ...
                <div style={{ position: "absolute", left: 0, right: 0, top: 330, bottom: 0, padding: "34px 44px", background: `linear-gradient(180deg, ${rgba("#1A120C", 0.4)} 0%, ${rgba("#1A120C", 0.98)} 14%)` }}>
                  ...
                  <div style={{ display: "flex", gap: 18, marginTop: 22, marginLeft: 70, opacity: active ? ramp(frame, q.at + 18, q.at + 30) : 0 }}>
                    {["SÍ", "NO"].map((w, k) => (
                      <div key={w} style={{
                        ...
                        border: `2px solid ${k === 0 ? acc : rgba("#fff", 0.35)}`, color: k === 0 ? V.white : V.bone,
              ...
              {done ? (
                <div style={{ position: "absolute", top: -40, right: -40, width: 120, height: 120, borderRadius: "50%", background: acc, display: "flex", alignItems: "center", justifyContent: "center", transform: `scale(${interpolate(-s, [0.5, 0.9], [0, 1], CL).toFixed(3)})`, boxShadow: `0 14px 30px ${rgba(acc, 0.6)}` }}>
                  <svg width={70} height={70} viewBox="0 0 40 40"><path d="M9 21 L17 29 L31 12" fill="none" stroke={V.ink0} strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" /></svg>
                </div>
              ) : null}
            </div>
          );
