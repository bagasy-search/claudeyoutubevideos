  ...
  <AbsoluteFill style={{ pointerEvents: "none" }}>
    {Array.from({ length: motes }).map((_, i) => {
      ...
      return <div key={i} style={{
        position: "absolute", left: `${x}%`, top: `${y}%`, width: s, height: s, borderRadius: "50%",
        background: rgba("#F3E3C0", 0.18 + rnd(i * 7) * 0.3), filter: `blur(${(rnd(i * 13) * 2).toFixed(1)}px)`,
    ...
    <AbsoluteFill style={{ background: `radial-gradient(120% 110% at 50% 46%, transparent 55%, ${rgba("#0A0704", 0.7)} 100%)` }} />
  </AbsoluteFill>
);
...
export const GlassPhoto: React.FC<{
  ...
  <div style={{
    ...
    border: `1px solid ${rgba("#EAD9B5", 0.42)}`,
    boxShadow: [
      `0 2px 0 ${rgba("#ffffff", 0.08)} inset`,
      `0 ${Math.round(h * 0.09)}px ${Math.round(h * 0.2)}px ${rgba("#0A0604", 0.62)}`,
      `0 10px 24px ${rgba("#0A0604", 0.45)}`,
