        ...
        <Plane depth={interpolate(back, [0, 1], [0.8, 0.45])} camX={cam.camX} camY={cam.camY} z={2}>
          <div style={{
            ...
            <GlassPhoto img={mythImg} w={MW} h={MH} blur={back * 2.6} desat={ramp(frame, hitAt + 4, hitAt + 16) * 0.9} dim={back * 0.5}>
              <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "80px 34px 30px", background: `linear-gradient(transparent, ${rgba("#050B14", 0.94)} 38%)` }}>
                <Chip text="MITO" color={V.danger} ink={V.white} />
            ...
            <div style={{ position: "absolute", inset: -40 }}>
              <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", overflow: "visible" }} viewBox={`0 0 ${MW + 80} ${MH + 80}`} preserveAspectRatio="none">
                <path d={`M10 ${Math.round(MH * 0.66)} L${MW + 70} 40`} fill="none" stroke={V.danger} strokeWidth={20} strokeLinecap="round" pathLength={1} strokeDasharray={1} strokeDashoffset={1 - slash}
                  style={{ filter: `drop-shadow(0 6px 14px rgba(0,0,0,0.5)) drop-shadow(0 0 16px ${rgba(V.danger, 0.6)})` }} />
              </svg>
            </div>
          </div>
        </Plane>
        ...
        <Plane depth={0.95} camX={cam.camX} camY={cam.camY} z={3}>
          <div style={{
            ...
            <GlassPhoto img={truthImg} w={MW + 60} h={MH + 60} glow={V.brass} glowK={ramp(frame, truthAt + 8, truthAt + 24)} push={Math.max(0, frame - truthAt) * 0.0003}>
              <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "90px 36px 32px", background: `linear-gradient(transparent, ${rgba("#050B14", 0.95)} 36%)` }}>
                <Chip text="VERDAD" color={V.brass} ink={V.ink0} />
