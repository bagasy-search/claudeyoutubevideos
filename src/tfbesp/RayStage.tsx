...
// ── PALETA / TIPOGRAFÍA ──────────────────────────────────────────────────────────────────────
export const V = {
  ink0: "#17110C",       // nogal/espresso del TALLER (fondo)
  ink1: "#21180F",
  ink2: "#2E2217",
  brass: "#C99A45",      // ORO del Constructor (acento principal)
  brassSoft: "#EAD3A2",  // oro suave — resaltados
  amber: "#D9A441",      // números
  danger: "#C0492F",     // ÓXIDO — sólo el error / la advertencia
  dangerSoft: "#E08A6E",
  white: "#F1E4C9",      // CREMA del papel de la guía
  bone: "#D8C8A8",
  steel: "#7A5A3A",
  ok: "#7FA85A",         // verde del "así sí"
};
export const F_DISPLAY = F_PLAYFAIR;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      ...
      {Array.from({ length: 16 }).map((_, i) => {
        ...
        return <div key={`m${i}`} style={{ position: "absolute", left: `${x}%`, top: `${y}%`, width: s, height: s, borderRadius: "50%", background: rgba("#F3E3C0", 0.25 + rnd(i * 5) * 0.3) }} />;
      })}
      ...
      <AbsoluteFill style={{ background: `radial-gradient(125% 110% at 50% 45%, transparent 55%, ${rgba("#0A0704", 0.62)} 100%)` }} />
    </AbsoluteFill>
  );
};
...
export const StatBug: React.FC<{
  ...
  value = "", unit = "", caption = "", series = "El Constructor Libre", tone = "brass",
}) => {
  const frame = useCurrentFrame();
...
export const Keyring: React.FC<{ size?: number }> = ({ size = 34 }) => (
  <svg width={size} height={size} viewBox="0 0 40 40" style={{ overflow: "visible" }}>
    <path d="M6 34 V6 h6 v22 h22 v6 z" fill="none" stroke={V.brass} strokeWidth="2.4" strokeLinejoin="round" />
    <path d="M12 12 h3 M12 18 h4 M12 24 h3 M18 28 v3 M24 28 v4 M30 28 v3" stroke={V.brass} strokeWidth="1.8" />
    <path d="M20 22 L34 8 l3 3 L23 25 l-5 1 z" fill="none" stroke={V.danger} strokeWidth="2.2" strokeLinejoin="round" />
  </svg>
);
