...
const THUMBS_DEF = ['img/ill/bas_ill_lemon_water.png', 'img/ill/bas_ill_hibiscus.png', 'img/ill/bas_ill_barley.png', 'img/ill/bas_ill_ginger.png', 'img/ill/bas_ill_greentea.png', 'img/ill/bas_ill_cranberry.png', 'img/ill/bas_ill_water_drop.png'];

export type GuideCTASceneProps = {kicker?: string; title?: string; subtitle?: string; thumbs?: string[]};
export const GuideCTAScene: React.FC<GuideCTASceneProps> = ({
  kicker = 'Gratis · en la descripción',
  title = 'La guía completa de las 7 bebidas',
  subtitle = 'Cantidades exactas · cómo prepararlas · a qué hora',
  thumbs = THUMBS_DEF,
}) => {
    ...
    <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', perspective: 1400}}>
      <div style={{transformStyle: 'preserve-3d', transform: `translateY(${interpolate(inP, [0, 1], [60, 0]) + bob}px) rotateX(${interpolate(inP, [0, 1], [12, 4])}deg) scale(${interpolate(inP, [0, 1], [0.9, 1])})`, opacity: inP}}>
        ...
        <div style={{position: 'relative', width: 900, height: 340, borderRadius: 20, padding: '38px 54px',
          ...
          <div style={{fontFamily: FONT_SANS, fontSize: 22, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: BAS.aquaDark}}>{kicker}</div>
          <div style={{fontFamily: FONT_DISPLAY, fontSize: 52, fontWeight: 700, color: BAS.brand, marginTop: 6, lineHeight: 1.05}}>{title}</div>
          <div style={{fontFamily: FONT_SANS, fontSize: 26, color: BAS.ink2, marginTop: 6}}>{subtitle}</div>
