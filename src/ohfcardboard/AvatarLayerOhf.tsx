// AvatarLayerOhf.tsx — copia LOCAL de scenes/AvatarLayer para el canal The Old House Fix.
// Existe por dos razones:
//  1) el compartido monta el avatar con <Video> de @remotion/media, que en el RENDER busca por
//     TIEMPO y devuelve cuadros equivocados -> tiron en todo el metraje. Acá va OffthreadVideo.
//  2) el repo es COMPARTIDO y hay otras sesiones rendeando: tocar el archivo comun les cambiaria
//     el video abajo de los pies. Por eso es una copia, no un parche al compartido.
import { OffthreadVideo } from "remotion";
import { AbsoluteFill, staticFile, useCurrentFrame, useVideoConfig, interpolate, Easing } from "remotion";
import { useAudioData, visualizeAudio } from "@remotion/media-utils";
import { COLORS } from "../VideoEdit/theme";
...
export const AvatarLayerOhf: React.FC<{
    ...
    return (
      <AbsoluteFill style={{ pointerEvents: "none" }}>
        <div style={{ position: "absolute", left: -9999, top: 0, width: 384, height: 512, overflow: "hidden" }}>
          <OffthreadVideo src={staticFile(src)} loop={loop} muted={muted} style={{ width: 683, height: 384 }} />
        </div>
      </AbsoluteFill>
    );
  }
