 ...
 */
export const ComentarioCard: React.FC<{
  lineas: string[];
  likes?: number;
  desde2?: number;      // cuadro (relativo) en el que aparece la 2ª línea
  chico?: boolean;      // versión reducida para el recuerdo del final
}> = ({ lineas, likes = 221, desde2 = 0, chico = false }) => {
  const f = useCurrentFrame();
  const { durationInFrames } = useVideoConfig();

  // entrada con rebote corto (plop), salida limpia
  const ent = interpolate(f, [0, 7, 13], [0, 1.035, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const op = interpolate(f, [0, 8], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const sal = interpolate(f, [durationInFrames - 10, durationInFrames], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const a = Math.min(op, sal);

  const op2 = interpolate(f, [desde2, desde2 + 8], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const y2 = interpolate(f, [desde2, desde2 + 8], [10, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const nLikes = Math.round(interpolate(f, [desde2 + 6, desde2 + 26], [0, likes], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }));

  const esc = chico ? 0.72 : 1;
  const PAPEL = "#F4F2EA", TINTA = "#15161A", ROJO = "#E03127";

  return (
    <AbsoluteFill style={{ pointerEvents: "none", justifyContent: "flex-end", alignItems: "center", paddingBottom: chico ? 120 : 96 }}>
      <div
        style={{
          width: 1180 * esc, background: PAPEL, borderRadius: 22 * esc,
          padding: `${34 * esc}px ${40 * esc}px`, display: "flex", gap: 26 * esc,
          boxShadow: "0 26px 70px rgba(0,0,0,.62)", opacity: a,
          transform: `scale(${(ent * esc).toFixed(4)}) translateY(${((1 - op) * 26).toFixed(1)}px)`,
          transformOrigin: "50% 100%",
        }}
      >
        {/* avatar anónimo: círculo liso, sin cara */}
        <div style={{ width: 76 * esc, height: 76 * esc, borderRadius: "50%", background: "#C9C6BC", flex: "0 0 auto" }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          {/* franja del nombre TACHADA: no se suplanta a nadie */}
          <div style={{ display: "flex", alignItems: "center", gap: 14 * esc, marginBottom: 16 * esc }}>
            <div style={{ width: 190 * esc, height: 17 * esc, borderRadius: 9 * esc, background: "#D2CEC3" }} />
            <div style={{ width: 96 * esc, height: 13 * esc, borderRadius: 7 * esc, background: "#E1DDD3" }} />
          </div>
          <div style={{ fontFamily: "system-ui, Segoe UI, Roboto, sans-serif", fontSize: 40 * esc, lineHeight: 1.34, color: TINTA }}>
            {lineas[0]}
            {lineas[1] ? (
              <span style={{ opacity: op2, display: "inline-block", transform: `translateY(${y2.toFixed(1)}px)` }}>
                {" " + lineas[1]}
              </span>
            ) : null}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 * esc, marginTop: 22 * esc, opacity: op2 }}>
            <svg width={30 * esc} height={30 * esc} viewBox="0 0 24 24" fill={ROJO}>
              <path d="M12 21s-7.5-4.9-9.6-9A5.4 5.4 0 0 1 12 6.2 5.4 5.4 0 0 1 21.6 12c-2.1 4.1-9.6 9-9.6 9z" />
            </svg>
            <span style={{ fontFamily: "system-ui, Segoe UI, Roboto, sans-serif", fontSize: 34 * esc, fontWeight: 700, color: TINTA }}>
              {nLikes}
            </span>
          </div>
        </div>
