// Piezas.tsx — valromero (clon de valvasmix): planos crudos (clip de stock, foto, ventana de avatar) con Ken-Burns AL AZAR.
// ⛔ OffthreadVideo siempre (nunca <Video>), clips en <Loop> con sus cuadros REALES, sin filtros de color.
import React from 'react';
import {AbsoluteFill, Img, Loop, OffthreadVideo, Sequence, interpolate, staticFile, useCurrentFrame, useVideoConfig} from 'remotion';
...
// clip de agnes (4,03 s) + resto = ÚLTIMO CUADRO con la MISMA curva Ken-Burns (el clip nunca se repite ni se estira)
export const AgShot: React.FC<{src: string; last: string; seed: number; clipDur: number}> = ({src, last, seed, clipDur}) => {
  const t = useKenBurns(seed, 1.03, 0.03, 0.07, 0.8);
  const {fps, durationInFrames} = useVideoConfig();
  const clipF = Math.min(durationInFrames, Math.floor(clipDur * fps) - 1);
  return (
    <AbsoluteFill style={{background: '#1b1512', overflow: 'hidden'}}>
      <AbsoluteFill style={t}>
        <Sequence durationInFrames={clipF} layout="none"><AbsoluteFill><OffthreadVideo src={staticFile(src)} muted style={FILL} /></AbsoluteFill></Sequence>
        {durationInFrames > clipF ? <Sequence from={clipF} layout="none"><AbsoluteFill><Img src={staticFile(last)} style={FILL} /></AbsoluteFill></Sequence> : null}
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
