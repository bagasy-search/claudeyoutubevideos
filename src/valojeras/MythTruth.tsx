...
import { V, F_DISPLAY, rgba, ramp, spr, Atmosphere, Finish, Plane, GlassPhoto, useCam, KickerRule } from "./ValDepth";
      ...
      <AbsoluteFill style={{ transform: `scale(${cam.scale.toFixed(4)})`, transformOrigin: "50% 55%" }}>
        ...
        <Plane depth={interpolate(back, [0, 1], [0.8, 0.45])} camX={cam.camX} camY={cam.camY} z={2}>
          <div style={{
            ...
            <GlassPhoto img={mythImg} w={MW} h={MH} blur={back * 2.6} desat={ramp(frame, hitAt + 4, hitAt + 16) * 0.9} dim={back * 0.5}>
              <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "80px 34px 30px", background: `linear-gradient(transparent, ${rgba(VAL.card, 0.97)} 38%)` }}>
                <Chip text="MITO" color={V.danger} ink={VAL.onAccent} />
        ...
        <Plane depth={0.95} camX={cam.camX} camY={cam.camY} z={3}>
          <div style={{
            ...
            <GlassPhoto img={truthImg} w={MW + 60} h={MH + 60} glow={V.brass} glowK={ramp(frame, truthAt + 8, truthAt + 24)} push={Math.max(0, frame - truthAt) * 0.0003}>
              <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "90px 36px 32px", background: `linear-gradient(transparent, ${rgba(VAL.card, 0.97)} 36%)` }}>
                <Chip text="LA VERDAD" color={VAL.sage} ink={VAL.onAccent} />
