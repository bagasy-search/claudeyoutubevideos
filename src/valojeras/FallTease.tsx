...
import { V, F_DISPLAY, rgba, CL, ramp, spr, Atmosphere, Finish, Plane, GlassPhoto, useCam, KickerRule } from "./ValDepth";

const Lock: React.FC<{ size: number; wobble: number }> = ({ size, wobble }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" style={{ transform: `rotate(${wobble.toFixed(2)}deg)`, filter: `drop-shadow(0 10px 22px ${rgba(VAL.ink, 0.25)})` }}>
    <path d="M32 46 V34 a18 18 0 0 1 36 0 V46" fill="none" stroke={VAL.goldDark} strokeWidth={9} strokeLinecap="round" />
    ...
    <circle cx="50" cy="62" r="6" fill={VAL.onAccent} />
    <rect x="47.5" y="64" width="5" height="12" rx="2.5" fill={VAL.onAccent} />
  </svg>
);
      ...
      <AbsoluteFill style={{ transform: `scale(${cam.scale.toFixed(4)})`, transformOrigin: "50% 55%" }}>
        ...
        <Plane depth={0.95} camX={cam.camX} camY={cam.camY} z={5}>
          <div style={{ position: "absolute", left: 0, right: 0, top: 96, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 70, color: V.white, marginTop: 12, textShadow: "none", opacity: ramp(frame, 10, 24), transform: `translateY(${((1 - ramp(frame, 10, 24)) * 16).toFixed(1)}px)` }}>{title}</div>
          </div>
        </Plane>
      </AbsoluteFill>
