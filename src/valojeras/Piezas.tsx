...
// Piezas de rowecastor (canal Dr. Emmett Rowe, EN).
...
export const Foto: React.FC<{ src: string; seed: number; cont?: { seed: number; off: number; n: number } }> = ({ src, seed, cont }) => {
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#F4EBD9" }}>
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#F4EBD9" }}>
      <AbsoluteFill style={{ transform: kb.transform, transformOrigin: kb.transformOrigin }}>
        {cuandoTermina ? (
          <>
            <Sequence durationInFrames={frames} layout="none"><OffthreadVideo src={staticFile(src)} muted style={cover} /></Sequence>
            <Sequence from={frames} layout="none"><Img src={staticFile(last as string)} style={cover} /></Sequence>
          </>
