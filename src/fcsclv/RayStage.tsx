// RayStage.tsx — ESCENARIO COMPARTIDO del canal "Federer Consejos Salud" (ES-MX, salud +60).
//
...
// ── PALETA / TIPOGRAFÍA ──────────────────────────────────────────────────────────────────────
export const V = {
  ink0: "#08110F",       // tinta oscura del canal Federer (fondo)
  ink1: "#0E1A18",
  ink2: "#152523",
  brass: "#12B3AE",      // TEAL — el acento clínico del canal
  brassSoft: "#8FE3DF",  // teal suave — títulos, resaltados
  amber: "#F2B544",      // ámbar — números y avisos (segundo acento)
  danger: "#D6453C",     // ROJO — SÓLO la advertencia / la señal de alerta
  dangerSoft: "#E4796F",
  white: "#F5F2EA",      // crema del canal
  bone: "#C7CFCC",
  steel: "#2E7D86",
...
export const Clip: React.FC<{ src: string; rate?: number }> = ({ src, rate = 1 }) => {
  const frame = useCurrentFrame();
  // ⛔⛔ SIN FADE DE ENTRADA. El `enter(frame, 6)` heredado deja el video en opacidad 0 en el frame 0
  //    sobre el fondo `V.ink0`: como los beats son contiguos (no se solapan), eso NO es un
  //    cross-fade — es UN FRAME NEGRO en CADA corte. Medido en fedvet2: 196 clips → ~180 destellos
  //    negros de 33 ms repartidos por todo el video, y `blackdetect` (d=0.5) no ve ninguno.
  //    `Foto` nunca tuvo fade y por eso los planos de imagen no parpadean. Corte duro, como ella.
  const a = 1;
...
export const StatBug: React.FC<{
  ...
  value = "", unit = "", caption = "", series = "Federer Consejos Salud", tone = "brass",
}) => {
  const frame = useCurrentFrame();
...
/** La marca de esquina del canal: la taza humeante (mismo nombre de export para no romper imports). */
export const Keyring: React.FC<{ size?: number }> = ({ size = 34 }) => (
  <svg width={size} height={size} viewBox="0 0 40 40" style={{ overflow: "visible" }}>
    <path d="M9 16 h18 v10 a7 7 0 0 1 -7 7 h-4 a7 7 0 0 1 -7 -7 z" fill="none" stroke={V.brass} strokeWidth="2.4" strokeLinejoin="round" />
    <path d="M27 19 h3.5 a4 4 0 0 1 0 8 H27" fill="none" stroke={V.brass} strokeWidth="2.2" strokeLinecap="round" />
    <path d="M16 12 c0 -2.6 2.6 -2.6 2.6 -5.2" fill="none" stroke={V.brass} strokeWidth="1.9" strokeLinecap="round" opacity="0.75" />
    <path d="M21.4 12 c0 -2.6 2.6 -2.6 2.6 -5.2" fill="none" stroke={V.brass} strokeWidth="1.9" strokeLinecap="round" opacity="0.5" />
    <rect x="9" y="30.4" width="18" height="2" rx="1" fill={V.brass} opacity="0.35" />
  </svg>
);
