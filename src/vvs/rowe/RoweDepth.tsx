    ...
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      {img ? (
        <Img src={src(img)} style={{
          ...
          filter: `blur(${blur}px) brightness(${(1.02 + dim * 0.05).toFixed(2)}) saturate(0.7) sepia(0.12)`, opacity: 0.55,
          transform: `scale(${z.toFixed(4)}) translateX(${(camX * -0.35).toFixed(2)}px)`,
        }} />
      ) : null}
      <AbsoluteFill style={{ background: `radial-gradient(90% 80% at 50% 45%, ${rgba(V.ink0, 0.35)} 0%, ${rgba(V.ink1, 0.86)} 100%)` }} />
      {Array.from({ length: bokeh }).map((_, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            background: `radial-gradient(circle, ${rgba(col, 0.12 + rnd(i * 9 + seed) * 0.1)} 0%, ${rgba(col, 0)} 68%)`,
      ...
      <AbsoluteFill style={{
        background: `linear-gradient(115deg, transparent ${(frame * 0.35) % 160 - 40}%, ${rgba("#ffffff", 0.22)} ${(frame * 0.35) % 160 - 25}%, transparent ${(frame * 0.35) % 160 - 10}%)`,
      }} />
    </AbsoluteFill>
  );
};
  ...
  <AbsoluteFill style={{ pointerEvents: "none" }}>
    {Array.from({ length: motes }).map((_, i) => {
      ...
      return <div key={i} style={{
        position: "absolute", left: `${x}%`, top: `${y}%`, width: s, height: s, borderRadius: "50%",
        background: rgba(V.brass, 0.14 + rnd(i * 7) * 0.22), filter: `blur(${(rnd(i * 13) * 2).toFixed(1)}px)`,
    ...
    <AbsoluteFill style={{ background: `radial-gradient(120% 110% at 50% 46%, transparent 55%, ${rgba(V.brassSoft, 0.32)} 100%)` }} />
  </AbsoluteFill>
);
...
export const GlassPhoto: React.FC<{
  ...
  <div style={{
    ...
    border: `6px solid ${V.card}`,
    boxShadow: [
      `0 2px 0 ${rgba("#ffffff", 0.08)} inset`,
      `0 ${Math.round(h * 0.09)}px ${Math.round(h * 0.2)}px ${rgba("#3A2A1A", 0.34)}`,
      `0 10px 24px ${rgba("#3A2A1A", 0.22)}`,
    ...
    {label ? (
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "56px 22px 20px", background: `linear-gradient(transparent, ${rgba(V.card, 0.96)} 45%)` }}>
        <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: labelSize, lineHeight: 1.0, color: V.white, letterSpacing: 0 }}>{label}</div>
...
export const KickerRule: React.FC<{ text: string; a: number; color?: string; center?: boolean }> = ({ text, a, color = V.brass, center }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 14, justifyContent: center ? "center" : "flex-start", opacity: a }}>
    ...
    <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 28, letterSpacing: 5, textTransform: "uppercase", color }}>{text}</div>
  </div>
);
