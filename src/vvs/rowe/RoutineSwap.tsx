      ...
      <AbsoluteFill style={{ transform: `scale(${cam.scale.toFixed(4)})`, transformOrigin: "50% 58%" }}>
        <Plane depth={0.3} camX={cam.camX} camY={cam.camY} z={1}>
          <div style={{ position: "absolute", left: 130, top: 92, opacity: headA, transform: `translateY(${((1 - headA) * 16).toFixed(1)}px)` }}>
            ...
            <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 70, color: V.white, marginTop: 8 }}>{title}</div>
          </div>
        </Plane>
        {items.map((it, i) => {
            ...
            <React.Fragment key={i}>
              <Plane depth={depthK} camX={cam.camX} camY={cam.camY} z={z}>
                <div style={{
                  ...
                  {showNew ? (
                    <div style={{ position: "absolute", top: -24, right: -24, width: 70, height: 70, borderRadius: "50%", background: V.ok, display: "flex", alignItems: "center", justifyContent: "center", transform: `scale(${check.toFixed(3)})`, boxShadow: `0 10px 26px ${rgba(V.brass, 0.6)}` }}>
                      <svg width={40} height={40} viewBox="0 0 40 40"><path d="M9 21 L17 29 L31 12" fill="none" stroke={V.onAccent} strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" /></svg>
                    </div>
                  ) : null}
                </div>
              </Plane>
        ...
        {chip ? (
          <Plane depth={1} camX={cam.camX} camY={cam.camY} z={60}>
            <div style={{ position: "absolute", left: "50%", top: 872, transform: `translate(-50%,0) scale(${spr(frame, fps, chipAt, 120, 0.7).toFixed(3)})`, opacity: spr(frame, fps, chipAt, 120, 0.7) }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 14, padding: "14px 36px", borderRadius: 999, background: mode === "old" ? V.danger : V.brass, boxShadow: `0 20px 44px ${rgba("#3A2A1A", 0.3)}` }}>
                <span style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 44, letterSpacing: 3, color: V.onAccent, textTransform: "uppercase" }}>{chip}</span>
              </div>
            </div>
          </Plane>
        ) : null}
