              ...
              {flags.map((f, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: `linear-gradient(135deg, ${rgba(V.card, 0.98)}, ${rgba(V.ink1, 0.98)})`,
                    border: `1px solid ${rgba(V.amber, 0.35)}`,
                    boxShadow: `0 26px 50px ${rgba("#3A2A1A", 0.22)}, inset 0 1px 0 ${rgba("#fff", 0.6)}`,
                    opacity: k * (1 - after * 0.35), transform: `translateX(${((1 - k) * 90).toFixed(1)}px) perspective(1400px) rotateY(${((1 - k) * -18).toFixed(2)}deg)`,
                  }}>
                    <div style={{ width: 64, height: 64, flex: "0 0 64px", borderRadius: "50%", background: V.danger, color: V.onAccent, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 34, boxShadow: "none" }}>{i + 1}</div>
        ...
        {stamp ? (
          <Plane depth={1} camX={cam.camX} camY={cam.camY} z={5}>
            <div style={{ position: "absolute", left: 1390, top: 880, transform: `translate(-50%,-50%) rotate(-4deg) scale(${interpolate(stampK, [0, 1], [2.2, 1]).toFixed(3)})`, opacity: Math.min(1, stampK * 1.5) }}>
              <div style={{ padding: "18px 44px", borderRadius: 16, background: V.danger, border: `3px solid ${V.onAccent}`, boxShadow: `0 20px 40px ${rgba("#3A2A1A", 0.35)}` }}>
                <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, fontSize: 60, letterSpacing: 4, color: V.onAccent, textTransform: "uppercase", whiteSpace: "nowrap" }}>{stamp}</div>
              </div>
            </div>
          </Plane>
        ) : null}
