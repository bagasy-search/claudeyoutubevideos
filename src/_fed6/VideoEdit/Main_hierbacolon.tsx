...
import { HIERBACOLON_BEATS, HIERBACOLON_BROLL, HIERBACOLON_COVER } from "./hierbacolon_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Más Salud, Más Vida" · LA HIERBA DE $1 QUE LIMPIA TU COLON ──
// Clon de Main_fcscanela (que trae el FIX ANTI-HUECO). Diferencias de ESTE video:
//  · el avatar es un BUCLE de 151.4s bajo 34.7min de narración → los cortes del bucle ya vienen
//    tapados por contenido desde el gen (HIERBACOLON_SEAMS), no hace falta nada acá.
...
const compBeats = HIERBACOLON_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = HIERBACOLON_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...HIERBACOLON_BEATS.map((b: any) => b.start + b.dur),
  HIERBACOLON_BROLL.length ? HIERBACOLON_BROLL[HIERBACOLON_BROLL.length - 1].start + HIERBACOLON_BROLL[HIERBACOLON_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_HIERBACOLON = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of HIERBACOLON_COVER) {
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "hierbacolon_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="57% 25%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "hierbacolon_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainHierbacolon: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (clips agnes) */}
      {HIERBACOLON_BROLL.map((b) => {
      ...
      <AvatarLayer src="hierbacolon_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.566, y: 0.292, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="¿Te acuestas con la panza dura y amaneces inflado? Un médico lo explica…" impact="LA HIERBA DE $1" accentColor={TEAL} font={F_INTER} fontSize={104} />
      </Sequence>
