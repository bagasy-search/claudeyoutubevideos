...
import { NAILLINES_BEATS, NAILLINES_BROLL, NAILLINES_COVER, AVATAR_END, VIDEO_END as VEND } from "./naillines_beats";
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "naillines_opt.mp4";
...
const compBeats = NAILLINES_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = NAILLINES_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_NAILLINES = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of NAILLINES_COVER) {
...
export const MainNaillines: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Audio src={staticFile("naillines.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v, animación leve de su propia foto) */}
      {NAILLINES_BROLL.map((b) => {
