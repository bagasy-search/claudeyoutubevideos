...
import { MASK_BEATS } from "./rosemarymask_beats";
import { MASK_BROLL } from "./rosemarymask_broll";
import { TALKSR } from "./rosemarymask_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · THE ROSEMARY NIGHT MASK (manchas cara) ──
...
const compBeats = MASK_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = MASK_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...MASK_BEATS.map((b: any) => b.start + b.dur), MASK_BROLL.length ? MASK_BROLL[MASK_BROLL.length - 1].start + MASK_BROLL[MASK_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_MASK = Math.round(VIDEO_END * 30);
...
MASK_BEATS.filter((b: any) => /^(dontgetup|whyform|nobody|reveal|intro|expect|science|compounds|whynight|patients|villain|howto|mistake|honesty|recap)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...MASK_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /rm_federer|federer_kitchen|recipe_book|pagina|guia/.test(b.src || "");
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "rosemarymask_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "rosemarymask_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainRosemaryMask: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="rosemarymask_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
