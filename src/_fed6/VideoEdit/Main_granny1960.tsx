...
import { GRANNY1960_BEATS, GRANNY1960_BROLL, GRANNY1960_COVER, AVATAR_END, VIDEO_END as VEND } from "./granny1960_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · 12 FORBIDDEN 1960s BEAUTY TRICKS ──
// ⛔ AVATAR PARCIAL: el creador grabó 0..630.44s (tricks 1-8) de un master de 1057.57s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "granny1960_opt.mp4";
const AVATAR_FRAMES = Math.round(630.44 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = GRANNY1960_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = GRANNY1960_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_GRANNY1960 = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of GRANNY1960_COVER) {
...
export const MainGranny1960: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..630.44) + cola de Fish. El avatar va MUTEADO.
          Es el MISMO wav que AvatarLayerLoopFcs usa para el borde → un solo asset en el tar. */}
      <Audio src={staticFile("granny1960.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v, animación leve de su propia foto) */}
      {GRANNY1960_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (gpt-image low: presentador, ingredientes, Ruth, LÁMINA cama) */}
      {rawTop.map((b: any) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="A jar in a 1962 bathroom cabinet cost a nickel — and did more for a woman's skin than the $80 cream on your shelf today."
          impact="12 FORBIDDEN 1960s BEAUTY TRICKS"
          accentColor={TEAL} font={F_INTER} fontSize={90} />
      </Sequence>
