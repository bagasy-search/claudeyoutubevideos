...
import { FCSCOLAGENO_BEATS, FCSCOLAGENO_BROLL, FCSCOLAGENO_COVER, AVATAR_END, VIDEO_END as VEND } from "./fcscolageno_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos Salud" · LA CUCHARADA DE $1 QUE DISPARA TU COLÁGENO ──
// ⛔ AVATAR PARCIAL: el mp4 del creador cubre 0..AVATAR_END del máster (ver fcscolageno_beats.ts).
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
// ⛔ NUNCA hardcodear el largo del avatar: al clonar el Main se arrastra el del video ANTERIOR y el
//    <Loop> pide frames que el mp4 no tiene (fondo congelado/negro en cada vuelta). Sale del build.
const AVATAR_FRAMES = Math.round(AVATAR_END * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = FCSCOLAGENO_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FCSCOLAGENO_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FCSCOLAGENO = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of FCSCOLAGENO_COVER) {
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fcscolageno_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % AVATAR_END) * 30)} objectPos="18% 30%" />
...
export const MainFcscolageno: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..13:53) + cola de Fish desde la costura. El avatar va MUTEADO. */}
      <Audio src={staticFile("fcscolageno.m4a")} />

      {/* CAPA 1 — CLIPS (agnes) */}
      {FCSCOLAGENO_BROLL.map((b) => {
      ...
      <AvatarLayerLoopFcs src="fcscolageno_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Cuesta un peso, está en tu cocina ahorita… y es el mineral con el que tu cuerpo COSE el colágeno."
          impact="LA CUCHARADA DE $1 QUE DISPARA TU COLÁGENO"
