...
import { ROSE_BEATS } from "./rosemaryglove_beats";
import { ROSE_BROLL } from "./rosemaryglove_broll";
import { TALKSR } from "./rosemaryglove_hooks";
...
const compBeats = ROSE_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = ROSE_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...ROSE_BEATS.map((b: any) => b.start + b.dur), ROSE_BROLL.length ? ROSE_BROLL[ROSE_BROLL.length - 1].start + ROSE_BROLL[ROSE_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_ROSE = Math.round(VIDEO_END * 30);
...
ROSE_BEATS.filter((b: any) => /^(reframe|inventory|hands_first|reveal|results|intro|mechanism|compounds|night|villain|routine|honesty|error|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...ROSE_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
...
export const MainRosemaryGlove: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="rosemaryglove_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
