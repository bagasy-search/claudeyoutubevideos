...
import { GB_BEATS } from "./greenlift_beats";
import { GB_BROLL } from "./greenlift_broll";
import { TALKS_GB } from "./greenlift_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · THE GREEN LIFT (gotu kola) ──────────
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
GB_BEATS.filter((b: any) => /^(hook1|mirror|whyfail|reveal|whynone|mistakes_intro|recipe_intro|lamina_intro|safety|start)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of contentRaw) {
    const forceHidden = /lamina|cmp_|facemap|collagen|dg_|libro|guia|kitchen|desk|tea|press|leaf|plant|teacup|dried|infusion/.test(c.src || "");
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "greenlift_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "greenlift_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainGreenlift: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="greenlift_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, splitZoom: 1.1 }} />
