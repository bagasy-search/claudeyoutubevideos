...
import { GB_BEATS } from "./greenbotox_beats";
import { GB_BROLL } from "./greenbotox_broll";
import { TALKS_GB } from "./greenbotox_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · GREEN BOTOX (okra) ──────────────────
// Avatar en 3 modos, CERO recuadro: FULL (habla / arranca sección) · HIDDEN (imagen/
// diagrama/lámina/página a pantalla completa — DOMINA) · SPLIT halfR (avatar mitad
// derecha + b-roll mitad izquierda). Lámina y presenter-in-context = SIEMPRE hidden.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const HERO_CAP = 3.6;
// cap POR BEAT: la LÁMINA no se topea (queda visible en continuo mientras la explica);
// los diagramas dg_ hasta 8s; guidecta 12s; el resto como federer.
const capOf = (b: any): number => {
  if (b.kind === "freezezoom") return /lamina/.test(b.image || "") ? Math.max(2, b.dur) : /dg_/.test(b.image || "") ? 8 : 4.5;
  const k = b.kind;
...
GB_BEATS.filter((b: any) => /^(hook1|mirror|creams|okra_reveal|why_none|ruin|recipe_intro|lamina_intro|safety|start)$/.test(b.key) && (b.id.endsWith("_0")))
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "greenbotox_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "greenbotox_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainGreenbotox: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="greenbotox_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, splitZoom: 1.1 }} />
