...
import { ALOE_BEATS } from "./aloefiller_beats";
import { ALOE_BROLL } from "./aloefiller_broll";
import { TALKSR } from "./aloefiller_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · THE ROSEMARY NIGHT ALOE (manchas cara) ──
...
// HOOK v3: freezezoom macro → pricewar → REPRINTSCAN → HOURDIAL → avatar full recién en "dontgetup".
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const compBeats = ALOE_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = ALOE_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...ALOE_BEATS.map((b: any) => b.start + b.dur), ALOE_BROLL.length ? ALOE_BROLL[ALOE_BROLL.length - 1].start + ALOE_BROLL[ALOE_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_ALOE = Math.round(VIDEO_END * 30);
...
ALOE_BEATS.filter((b: any) => /^(dontgetup|nobody|reveal|intro|patient|expect|science|calm|mistake|whynight|villain|honesty|desc2)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...ALOE_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /al_federer|al_hook|rm_federer|federer_kitchen|recipe_book|pagina|guia|lamina|reveal/.test(b.src || "");
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "aloefiller_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "aloefiller_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainAloeFiller: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS del presentador rm_federer_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="aloefiller_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.616, y: 0.4, splitZoom: 1.18 }} />
