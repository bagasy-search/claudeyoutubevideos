...
import { WATER_BEATS } from "./rosemarywater_beats";
import { WATER_BROLL } from "./rosemarywater_broll";
import { TALKSR } from "./rosemarywater_hooks";
...
// derecha + imagen mitad izquierda). Libro-guía y Federer-cocina = SIEMPRE hidden.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const VIDEO_END = Math.max(...WATER_BEATS.map((b: any) => b.start + b.dur), WATER_BROLL.length ? WATER_BROLL[WATER_BROLL.length - 1].start + WATER_BROLL[WATER_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_WATER = Math.round(VIDEO_END * 30);
...
WATER_BEATS.filter((b: any) => /^(reframe|inventory|reveal|results|intro|compounds|morning|villain|howto|honesty|error|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...WATER_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /libro|cocina|pagina|guia|recipe_book|federer_kitchen/.test(b.src || "");
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "rosemarywater_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "rosemarywater_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainRosemaryWater: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS rose_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="rosemarywater_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
