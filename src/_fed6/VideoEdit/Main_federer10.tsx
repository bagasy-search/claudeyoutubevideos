...
import { FED10_BEATS } from "./federer10_beats";
import { FED10_BROLL } from "./federer10_broll";
import { TALKS10 } from "./federer10_hooks";
...
const compBeats = FED10_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED10_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED10_BEATS.map((b: any) => b.start + b.dur), FED10_BROLL.length ? FED10_BROLL[FED10_BROLL.length - 1].start + FED10_BROLL[FED10_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED10 = Math.round(VIDEO_END * 30);
...
FED10_BEATS.filter((b: any) => /^(tease3|historia_reg|razon\d+|ramiro|dos_grupos|honesto|error|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FED10_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS10.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS10) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer10_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer10_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer10: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer10_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Si te salen moretones sin golpearte…" impact="TU CUERPO TE ESTÁ AVISANDO" accentColor="#12B3AE" font={F_INTER} fontSize={110} />
      </Sequence>
