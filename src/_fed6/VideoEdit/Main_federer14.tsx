...
import { FED14_BEATS } from "./federer14_beats";
import { FED14_BROLL } from "./federer14_broll";
import { TALKS14 } from "./federer14_hooks";
...
const capOf = (k: string): number =>
  k === "diagram" ? 10 : k === "board" ? 14 : k === "quote" ? 8 : k === "rule" ? 5
...
const compBeats = FED14_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED14_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED14_BEATS.map((b: any) => b.start + b.dur), FED14_BROLL.length ? FED14_BROLL[FED14_BROLL.length - 1].start + FED14_BROLL[FED14_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED14 = Math.round(VIDEO_END * 30);
...
FED14_BEATS.filter((b: any) => /^(medicina|tamano|antiox|honesto|receta|dos_formas|sol|error|detalle|injerto1|injerto2|elena_vuelve|clara|s1|s2|s3|primera_noche|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FED14_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS14.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS14) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer14_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer14_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer14: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer14_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.48, splitZoom: 1.12 }} />
