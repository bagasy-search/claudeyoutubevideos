...
import { COAG_BEATS } from "./coagulos_beats";
import { COAG_BROLL } from "./coagulos_broll";
import { TALKS_COAG } from "./coagulos_hooks";
...
const compBeats = COAG_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = COAG_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...COAG_BEATS.map((b: any) => b.start + b.dur), COAG_BROLL.length ? COAG_BROLL[COAG_BROLL.length - 1].start + COAG_BROLL[COAG_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_COAG = Math.round(VIDEO_END * 30);
...
const FULL_AT: number[] = []; // full lo maneja TALKS_COAG (cada talk = avatar full); sin refuerzos extra

function buildWindows(): AvatarWindow[] {
  type Pt = { start: number; mode: AvatarWindow["mode"]; pr: number };
  const pts: Pt[] = [];
  let flip = false;
  const content = [...COAG_BROLL.map((b: any) => ({ start: b.start, len: Math.min(b.dur + 3, 7.5), src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, len: HERO_CAP, src: b.src }))].sort((a, b) => a.start - b.start);
  for (let i = 0; i < content.length; i++) {
    const b = content[i];
    const mode: AvatarWindow["mode"] = "hidden"; // solo full↔hidden (avatar_iii: evitar crop del split)
    void flip;
    pts.push({ start: b.start, mode, pr: 0 });
    // ⛔ ANTI-FRAME-MUERTO: al TERMINAR el clip, si el próximo contenido tarda >0.6s,
    // devolver el avatar a FULL (nunca dejar el fondo navy vacío en el hueco).
  ...
  for (const b of compBeats) {
    ...
    pts.push({ start: b.start + d, mode: "full", pr: 1 }); // al terminar el componente → avatar FULL (no fondo muerto)
  }
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS_COAG.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS_COAG) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "coagulos_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "coagulos_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainCoagulos: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="coagulos_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.48, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} subtitle="Cuidá tus piernas y tu circulación — salud real después de los 60" />
      </Sequence>
    </AbsoluteFill>
  );
};
