...
import { FocusCardsV8 } from "./FocusCards_v8v252t7cjxe";
import { LoopLockV8 } from "./LoopLock_v8v252t7cjxe";
import { RelojRiesgoV8 } from "./RelojRiesgo_v8v252t7cjxe";
import { EscalonesV8 } from "./Escalones_v8v252t7cjxe";
import { F_INTER } from "./kit/premium/theme";
import { FEDZ_BEATS } from "./federer_v8v252t7cjxe_beats";
import { FEDZ_BROLL } from "./federer_v8v252t7cjxe_broll";
import { TALKSZ } from "./federer_v8v252t7cjxe_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";
// Variantes propias de este video (NO se tocan los compartidos, los usan otros videos):
//  · PizarraV8      — board sin que la viñeta numerada tape el primer caracter del item
//  · FraseCineticaV8 — frase clave con presencia real, no un subtitulo
import { PizarraV8 } from "./Pizarra_v8v252t7cjxe";
import { FraseCineticaV8 } from "./FraseCinetica_v8v252t7cjxe";

// ── CANAL "Federer Archivos" · LO QUE MILLONES HACEN CADA MAÑANA (levantarse de golpe) ──
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO halfR / recuadro (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "v8v252t7cjxe_opt.mp4";

const NEWFULL = new Set(["avatarpizarra", "avatarkeyword", "mitoverdad", "errorstinger", "guardaesto", "freezezoom", "focuscards", "looplock", "relojriesgo", "escalones"]);
const OVERLAY = new Set(["lowerthird", "frasecinetica"]);
const NOCAP = new Set(["avatarpizarra", "avatarkeyword", "focuscards", "escalones"]);
...
const capOf = (k: string): number =>
  ...
  : k === "lowerthird" ? 6 : k === "frasecinetica" ? 5 : k === "relojriesgo" ? 7
...
FEDZ_BEATS.filter((b: any) => /^(hook|story|principio|eleto|solucion|nelida|trampa1|trampa2|trampa3|guia|honesto|alarma|error|cierre|recap|teaser|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  // HOOK: avatar FULL hasta que entra el PRIMER visual real (frames 0-117). El AvatarScrimText
  // arranca a los 2.2s y es TRANSPARENTE (scrim sobre el avatar vivo), así que si el avatar se
  // escondiera antes de que haya b-roll/foto quedaría fondo pelado detrás del texto.
  const HOOK_FULL = 3.9;
  const HOOK_END = 7.6;
  ...
  // ── GAP-FILL anti-negro: donde NO hay contenido (b-roll ralo), el avatar va FULL, nunca fondo pelado.
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "frasecinetica" ? <FraseCineticaV8 durationInFrames={d} words={b.words} ats={b.ats} perWord={b.perWord} tone={b.tone} />
  : b.kind === "board" ? <PizarraV8 durationInFrames={d} eyebrow={b.eyebrow} title={b.title} items={b.items || []} side={b.side || "left"} />
  ...
  : b.kind === "relojriesgo" ? <RelojRiesgoV8 durationInFrames={d} title={b.title} stat={b.stat} statUnit={b.statUnit} note={b.note} source={b.source} tone={b.tone} />
  : b.kind === "escalones" ? <EscalonesV8 durationInFrames={d} title={b.title} items={b.items} />
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Lo que millones hacen cada mañana sin saberlo…" impact="NO ES LA EDAD. ES CÓMO TE LEVANTÁS" accentColor="#12B3AE" font={F_INTER} fontSize={100} />
      </Sequence>
