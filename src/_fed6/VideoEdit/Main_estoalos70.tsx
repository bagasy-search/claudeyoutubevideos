...
import { ESTOALOS70_BEATS, ESTOALOS70_BROLL, ESTOALOS70_COVER } from "./estoalos70_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Más Salud, Más Vida" · SI PUEDES HACER ESTO A LOS 70 ───────
// Clon de Main_fcscanela (que trae el FIX ANTI-HUECO). Diferencias de ESTE video:
//  · ACÁ el avatar NO es un bucle: el creador grabó los 23:07.8 completos, así que el lipsync
//    calza de punta a punta y ESTOALOS70_SEAMS viene vacío.
...
const compBeats = ESTOALOS70_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = ESTOALOS70_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...ESTOALOS70_BEATS.map((b: any) => b.start + b.dur),
  ESTOALOS70_BROLL.length ? ESTOALOS70_BROLL[ESTOALOS70_BROLL.length - 1].start + ESTOALOS70_BROLL[ESTOALOS70_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_ESTOALOS70 = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of ESTOALOS70_COVER) {
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "estoalos70_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="50% 26%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "estoalos70_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainEstoalos70: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (clips agnes) */}
      {ESTOALOS70_BROLL.map((b) => {
      ...
      <AvatarLayer src="estoalos70_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.51, y: 0.30, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="¿Puedes sentarte en el suelo y volver a levantarte sin apoyar una sola mano?" impact="LA PRUEBA DEL PISO" accentColor={TEAL} font={F_INTER} fontSize={104} />
      </Sequence>
