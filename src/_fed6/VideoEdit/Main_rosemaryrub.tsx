...
const compBeats = RUB_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = RUB_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...RUB_BEATS.map((b: any) => b.start + b.dur), RUB_BROLL.length ? RUB_BROLL[RUB_BROLL.length - 1].start + RUB_BROLL[RUB_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_RUB = Math.round(VIDEO_END * 30);
...
RUB_BEATS.filter((b: any) => /^(reframe|inventory|hands_first|reveal|results|intro|mechanism|compounds|night|villain|routine|honesty|error|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...RUB_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "rosemaryrub_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "rosemaryrub_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainRosemaryRub: React.FC = () => {
  const hookDur = 5.4;
  return (
