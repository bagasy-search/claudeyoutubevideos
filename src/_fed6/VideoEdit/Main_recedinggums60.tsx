...
import { RECEDINGGUMS60_BEATS, RECEDINGGUMS60_BROLL, RECEDINGGUMS60_COVER, AVATAR_END, VIDEO_END as VEND } from "./recedinggums60_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · CARDIOLOGIST WARNS ─────────
// ⛔ AVATAR PARCIAL: el creador grabó 0..448.43s de un master de 1019.31s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "recedinggums60_opt.mp4";
const AVATAR_FRAMES = Math.round(448.43 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = RECEDINGGUMS60_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = RECEDINGGUMS60_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_RECEDINGGUMS60 = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of RECEDINGGUMS60_COVER) {
...
export const MainRecedinggums60: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..11:12) + cola de Fish. El avatar va MUTEADO.
          Es el MISMO wav que AvatarLayerLoopFcs usa para el borde → un solo asset en el tar. */}
      <Audio src={staticFile("recedinggums60.wav")} />

      {/* CAPA 1 — CLIPS (stock literal del mecanismo/dolor) */}
      {RECEDINGGUMS60_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (gpt-image low: presentador vlog-casero, LÁMINA cama) */}
      {rawTop.map((b: any) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Those gums pulling back from your teeth aren't a brushing problem — they're an inflammation problem. And inflammation is won at night, while you sleep."
          impact="THE $2 NIGHT TRICK FOR RECEDING GUMS"
          accentColor={TEAL} font={F_INTER} fontSize={88} />
      </Sequence>
