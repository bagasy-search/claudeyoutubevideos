...
import { PROSTATA3_BEATS, PROSTATA3_BROLL, PROSTATA3_COVER, AVATAR_END, VIDEO_END as VEND } from "./prostata3_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Mas Salud, Mas Vida" · UROLOGO SUPLICA A LOS HOMBRES +60 ──
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 548.99s sobre un master de 1321.05s.
...
//    el chorro de agua, el clic de la caja. Volumen 0.07 debajo de la voz.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = Math.round(548.99 * 30);
const CLIP_VOL = 0.07;
...
const compBeats = PROSTATA3_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = PROSTATA3_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_PROSTATA3 = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of PROSTATA3_COVER) {
...
export const MainProstata3: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayerLoopFcs src="prostata3_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
        avatarFocus={{ x: 0.52, y: 0.30, splitZoom: 1.12 }} avatarFrames={AVATAR_FRAMES} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Tres cosas que un hombre de más de 60 hace todos los días, gratis, sin darse cuenta."
          impact="DEJA ESTOS 3 HÁBITOS ANTES DE QUE SEA TARDE"
