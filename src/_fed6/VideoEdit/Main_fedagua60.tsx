...
import { FEDAGUA60_BEATS, FEDAGUA60_BROLL, FEDAGUA60_COVER, AVATAR_END, VIDEO_END as VEND } from "./fedagua60_beats";
...
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 836.17s sobre un máster de 2845.28s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = Math.round(836.17 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const NEWFULL = new Set(["avatarpizarra", "mitoverdad", "errorstinger", "guardaesto", "freezezoom",
  ...
  "skinlayer", "bodymap", "checklisterrores", "splitcompare", "beforeafter", "barcompare",
  "reprintscan", "pizarraglicacion", "listaflotante", "comparaprof", "datoimpacto", "mitorevelado"]);
...
const capOf = (k: string): number =>
  ...
  : k === "process" || k === "splitlist" ? 9
  : k === "skinlayer" ? 10 : k === "bodymap" ? 11 : k === "checklisterrores" ? 8
  : k === "splitcompare" ? 9 : k === "beforeafter" ? 8 : k === "barcompare" ? 7
  : k === "reprintscan" ? 7 : k === "pizarraglicacion" ? 14 : k === "listaflotante" ? 9
  : k === "comparaprof" ? 9 : k === "datoimpacto" ? 7 : k === "mitorevelado" ? 8.5 : 6;

const compBeats = FEDAGUA60_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FEDAGUA60_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FEDAGUA60 = Math.round(VEND * 30);
...
const compDur = (b: any): number => {
  ...
  return Math.max(2, Math.min(b.dur, room));   // b.dur YA trae el piso de lectura (gen)
};
...
function buildWindows(): AvatarWindow[] {
  // ⛔⛔ POR UNIÓN DE INTERVALOS, NUNCA POR PRIORIDAD DE PUNTOS.
  //    La versión vieja empujaba un punto "full" al TERMINAR cada componente, y ese punto
  //    cancelaba el "hidden" de una foto que seguía en pantalla → el avatar volvía ENCIMA
  //    del b-roll. Medido en este video: 92 s de la zona Fish con el avatar a la vista y
  //    los labios fuera de sincronía. Acá el avatar está oculto en la UNIÓN de todo lo que
  //    tapa, y sólo vuelve a full donde no hay NADA.
  type Iv = { a: number; b: number; split: boolean };
  const ivs: Iv[] = [];
  let flip = false;
  for (const c of FEDAGUA60_COVER) {
    const puedeSplit = c.kind === "video" && c.start + c.cov < AVATAR_END;
    const split = puedeSplit && flip;
    if (puedeSplit) flip = !flip;
    ivs.push({ a: c.start, b: c.start + c.cov, split });
  }
  for (const b of compBeats) {
    // los OVERLAY (lowerthird / frasecinetica) van ENCIMA: NO ocultan al avatar
    if (OVERLAY.has(b.kind)) continue;
    ivs.push({ a: b.start, b: b.start + compDur(b), split: false });
  }
  ivs.sort((x, y) => x.a - y.a);

  // unión: un tramo tapado se corta sólo cuando hay un hueco real
  const merged: Iv[] = [];
  for (const iv of ivs) {
    const last = merged[merged.length - 1];
    if (last && iv.a <= last.b + 0.05) {
      last.b = Math.max(last.b, iv.b);
      last.split = last.split && iv.split;      // basta que UNO no sea split para tapar entero
    } else merged.push({ ...iv });
  }

  const out: AvatarWindow[] = [{ start: 0, mode: "full" }];
  for (const m of merged) {
    out.push({ start: +m.a.toFixed(2), mode: m.split ? "halfR" : "hidden" });
    out.push({ start: +m.b.toFixed(2), mode: "full" });
  }

  // HOOK: el texto va ENCIMA del avatar VIVO — AvatarScrimText solo lo OSCURECE, no dibuja
  // ningun avatar. Por eso el avatar tiene que estar VISIBLE de 0 a HOOK_END.
  // ⛔ Antes esto lo ponia en "hidden" y ademas resumia con una heuristica
  // (`resume.start >= 1.4 ? "hidden"`) que lo dejaba oculto hasta el SIGUIENTE cover.
  // Como el b-roll de apertura solo cubria 3,93 s de sus 14,2 s, quedaban ~10 s de
  // PANTALLA VACIA (medido: YAVG plano = 37 en t=8/11/13).
  const HOOK_END = 7.0;
  const modeAt = (t: number): AvatarWindow["mode"] => {
    const m = merged.find((v) => t >= v.a - 0.02 && t < v.b);
    return m ? (m.split ? "halfR" : "hidden") : "full";
  };
  const post = out.filter((w) => w.start >= HOOK_END);
  post.push({ start: 0, mode: "full" });                 // el avatar ABRE el video
  post.push({ start: HOOK_END, mode: modeAt(HOOK_END) }); // y despues manda la UNION
  post.sort((a, b) => a.start - b.start);
  const fin: AvatarWindow[] = [];
  for (const x of post) { if (!fin.length || fin[fin.length - 1].mode !== x.mode) fin.push(x); }
  return fin;
}
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fedagua60_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % 836.1) * 30)} objectPos="18% 30%" />
  ...
  : b.kind === "skinlayer" ? <SkinLayerBuild durationInFrames={d} eyebrow={b.eyebrow} title={b.title} stages={b.stages} bed={b.bed} />
  : b.kind === "bodymap" ? <BodyMapScene durationInFrames={d} title={b.title} stops={b.stops} bed={b.bed} />
  : b.kind === "checklisterrores" ? <ChecklistErrores durationInFrames={d} title={b.title} items={b.items} active={b.active ?? 0} side={b.side} />
  ...
  : b.kind === "beforeafter" ? <BeforeAfterPush durationInFrames={d} before={b.before} after={b.after} beforeLabel={b.beforeLabel} afterLabel={b.afterLabel} caption={b.caption} bed={b.bed} />
  : b.kind === "barcompare" ? <BarCompare durationInFrames={d} eyebrow={b.eyebrow} title={b.title} bars={b.bars} unit={b.unit} medico />
  : b.kind === "reprintscan" ? <ReprintScan durationInFrames={d} image={b.image} label={b.label} passes={b.passes} tone={b.tone} />
  : b.kind === "pizarraglicacion" ? <PizarraGlicacion durationInFrames={d} />
  : b.kind === "listaflotante" ? <ListaFlotante durationInFrames={d} title={b.title} image={b.image} items={b.items} tone={b.tone} />
  : b.kind === "comparaprof" ? <ComparaProfundidad durationInFrames={d} title={b.title} unit={b.unit} image={b.image} bars={b.bars} />
  ...
  : b.kind === "mitorevelado" ? <MitoRevelado durationInFrames={d} myth={b.myth} truth={b.truth} image={b.image} flipAt={b.flipAt} />
  : renderFederer2Comp(b, d, { medico: true });

export const MainFedagua60: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..15:05) + Fish desde la costura. El avatar va MUTEADO. */}
      <Audio src={staticFile("fedagua60.wav")} />

      {/* CAPA 1 — CLIPS (agnes) */}
      {FEDAGUA60_BROLL.map((b) => {
      ...
      <AvatarLayerLoopFcs src="fedagua60_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
        avatarFocus={{ x: 0.29, y: 0.3, splitZoom: 1.12 }} avatarFrames={AVATAR_FRAMES} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Cuesta menos que un café, está en la góndola de los postres… y es la MISMA proteína de la que está hecha tu piel."
          impact="AGREGÁ ESTO AL AGUA DESPUÉS DE LOS 60"
