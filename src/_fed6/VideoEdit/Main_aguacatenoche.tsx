...
import { AGUACATENOCHE_BEATS, AGUACATENOCHE_BROLL, AGUACATENOCHE_COVER, AVATAR_END, VIDEO_END as VEND } from "./aguacatenoche_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Más Salud, Más Vida" · COME ESTO CADA NOCHE (PIEL + ARTICULACIONES) ──
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 602.63s sobre un master de 1405.30s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = 18079; // largo REAL del mp4 conformado a 30 fps → el <Loop> lo repite
...
const compBeats = AGUACATENOCHE_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = AGUACATENOCHE_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_AGUACATENOCHE = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of AGUACATENOCHE_COVER) {
...
const CTA_AT = VEND - 13;

const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "aguacatenoche_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % 602.6) * 30)} objectPos="18% 30%" />
...
export const MainAguacatenoche: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..15:05) + Fish desde la costura. El avatar va MUTEADO. */}
      <Audio src={staticFile("aguacatenoche.wav")} />

      {/* CAPA 1 — CLIPS (agnes) */}
      {AGUACATENOCHE_BROLL.map((b) => {
      ...
      <AvatarLayerLoopAgu src="aguacatenoche_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
        avatarFocus={{ x: 0.52, y: 0.32, splitZoom: 1.10 }} avatarFrames={AVATAR_FRAMES} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Una fruta verde de veinticinco pesos hace en ocho semanas lo que un bote de colágeno de setecientos promete y jamás cumple."
          impact="COME ESTO CADA NOCHE"
