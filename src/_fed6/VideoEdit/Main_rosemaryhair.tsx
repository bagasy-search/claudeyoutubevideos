...
import { ScalpDive } from "./scenes/ScalpDive";
...
import { HAIR_BEATS } from "./rosemaryhair_beats";
import { HAIR_BROLL } from "./rosemaryhair_broll";
import { TALKSR } from "./rosemaryhair_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · ROSEMARY for HAIR ────────────────────
// Avatar en 3 modos, CERO recuadro: FULL · HIDDEN (imagen/paso/clip/componente domina) ·
// SPLIT halfR (avatar mitad derecha + imagen mitad izquierda).
const TEAL = "#12B3AE";
const BG = "#0E1D23";

const NEWFULL = new Set(["avatarpizarra", "avatarkeyword", "mitoverdad", "errorstinger", "guardaesto", "freezezoom", "pricewar", "scalpdive"]);
...
const capOf = (k: string): number =>
  ...
  : k === "scalpdive" ? 10 : k === "lowerthird" ? 6 : k === "frasecinetica" ? 5 : k === "pricewar" ? 10
  : k === "process" || k === "checklist" ? 9 : 6;

const compBeats = HAIR_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = HAIR_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...HAIR_BEATS.map((b: any) => b.start + b.dur), HAIR_BROLL.length ? HAIR_BROLL[HAIR_BROLL.length - 1].start + HAIR_BROLL[HAIR_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_HAIR = Math.round(VIDEO_END * 30);
...
// FULL breve del avatar: arranque de secciones grandes
const FULL_AT: number[] = [];
HAIR_BEATS.filter((b: any) => /^(reframe|inventory|intro|backup|compounds|morning|villain|howto|timeline|honesty|mistakepay|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...HAIR_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /libro|cocina|pagina|guia|recipe_book|federer_kitchen|scalp|infuse|massage/.test(b.src || "");
  ...
  // HOOK v3: el cold-open lo llevan los componentes (freezezoom sprig + pricewar +
  // scalpdive), el avatar entra full recién en "iknow" (vía TALKSR). Sin scrim.
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "rosemaryhair_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "rosemaryhair_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  ...
  : b.kind === "pricewar" ? <PriceWar durationInFrames={d} leftImage={b.leftImage} rightImage={b.rightImage} leftPrice={b.leftPrice} rightPrice={b.rightPrice} leftLabel={b.leftLabel} rightLabel={b.rightLabel} strike={b.strike} subtitle={b.subtitle} verdict={b.verdict} equalsAt={b.equalsAt} />
  : renderFederer2Comp(b, d, { medico: true });

export const MainRosemaryHair: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS presenter rh_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="rosemaryhair_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} kicker="Dr. Federer" title="Subscribe" subtitle="Every week — real, natural science for your hair and mature skin" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
