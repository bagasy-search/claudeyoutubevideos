...
import { SEVENREMEDIES_BEATS, SEVENREMEDIES_BROLL, SEVENREMEDIES_COVER, AVATAR_END, VIDEO_END as VEND } from "./sevenremedies_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · 7 HOME REMEDIES AFTER 60 ─
// ⛔ AVATAR PARCIAL: el creador grabó 0..262.24s de un master de ~1006.67s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "sevenremedies_opt.mp4";
const AVATAR_FRAMES = Math.round(262.24 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = SEVENREMEDIES_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = SEVENREMEDIES_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_SEVENREMEDIES = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of SEVENREMEDIES_COVER) {
...
export const MainSevenremedies: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..10:30) + cola de Fish. El avatar va MUTEADO.
          Es el MISMO wav que AvatarLayerLoopFcs usa para el borde → un solo asset en el tar. */}
      <Audio src={staticFile("sevenremedies.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v, animación leve de su propia foto) */}
      {SEVENREMEDIES_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (gpt-image low: presentador, alimentos, dolor, LÁMINA cama) */}
      {rawTop.map((b: any) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="I probably shouldn't say this out loud — most of what I was trained to give people over sixty barely helps. So here are the seven cheap things I actually recommend."
          impact="THE 7 REMEDIES I GIVE MY PATIENTS"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          ...
          subtitle="Simple, natural remedies for real health after 60 — while you sleep" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
