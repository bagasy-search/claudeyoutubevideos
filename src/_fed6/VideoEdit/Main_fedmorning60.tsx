...
import { FEDMORNING60_BEATS, FEDMORNING60_BROLL, FEDMORNING60_COVER, AVATAR_END, VIDEO_END as VEND } from "./fedmorning60_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · CARDIOLOGIST WARNS ─────────
// ⛔ AVATAR PARCIAL: el creador grabó 0..672.042s de un master de 1019.31s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "fedmorning60_opt.mp4";
const AVATAR_FRAMES = Math.round(672.042 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = FEDMORNING60_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FEDMORNING60_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FEDMORNING60 = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of FEDMORNING60_COVER) {
...
export const MainFedmorning60: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Audio src={staticFile("fedmorning60.wav")} />

      {/* CAPA 1 — CLIPS (stock literal del mecanismo/dolor) */}
      {FEDMORNING60_BROLL.map((b) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="The first ten minutes after you wake put more strain on your heart than climbing three flights of stairs — and after 60 it gets worse."
          impact="THE MOST DANGEROUS HOUR OF YOUR DAY"
