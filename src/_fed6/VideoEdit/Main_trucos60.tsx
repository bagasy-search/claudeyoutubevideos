...
import { T60_BEATS } from "./trucos60_beats";
import { T60_BROLL } from "./trucos60_broll";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Más Salud, Más Vida" · LOS 12 TRUCOS DE BELLEZA DE LOS AÑOS 60 ──
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "trucos60_opt.mp4";
...
const compBeats = T60_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = T60_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(
  ...T60_BEATS.map((b: any) => b.start + b.dur),
  T60_BROLL.length ? T60_BROLL[T60_BROLL.length - 1].start + T60_BROLL[T60_BROLL.length - 1].dur : 0,
  1434.005,
) + 1.0;
export const TOTAL_FRAMES_T60 = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  const contentIv: Iv[] = [
    ...T60_BROLL.map((b: any) => ({ s: b.start, e: b.start + b.dur, mode: "hidden" as const })),
...
export const MainTrucos60: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src={AVATAR} windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.52, y: 0.28, splitZoom: 1.12 }} />
