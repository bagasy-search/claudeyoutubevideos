...
import { FocusCardsV89 } from "./FocusCards_v89y5o7w2nz6";
import { LoopLockV89 } from "./LoopLock_v89y5o7w2nz6";
import { F_INTER } from "./kit/premium/theme";
import { FEDZ_BEATS } from "./federer_v89y5o7w2nz6_beats";
import { FEDZ_BROLL } from "./federer_v89y5o7w2nz6_broll";
import { TALKSZ } from "./federer_v89y5o7w2nz6_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Archivos" (ES) · NO BEBAS AGUA SOLA: GELATINA + VITAMINA C = COLÁGENO ────────
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO recuadro/PiP/split (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const INK = "#12222B";
const BG = "#0E1D23";
const AVA = "v89y5o7w2nz6_opt.mp4";
...
export const TOTAL_FRAMES_V89 = Math.round(VIDEO_END * 30);
...
const compDur = (b: any): number => {
  ...
  // si al componente le falta poco para llegar al siguiente, se ESTIRA (evita micro-huecos = negro)
  const d = Math.max(2, Math.min(b.dur, capOf(b.kind), room));
  return room > 0 && room - d < 1.2 ? Math.max(d, room) : d;
};

const FULL_AT: number[] = [];
FEDZ_BEATS.filter((b: any) => /^(hook|presenta|principio|ladrillo|vitc|olga|demo|prueba|m1|m2|m3|m4|m5|elpidio|enemigo|honesto|limites|error|recap|carnada|guia3|teaser|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const inGap = (t: number) => t >= 7.8 && gaps.some(([s, e]) => t >= Math.max(s, 7.8) - 1e-6 && t < e - 1e-6);
  const bounds = new Set<number>(out.map((w) => w.start));
  for (const [s, e] of gaps) { if (e <= 7.8) continue; bounds.add(+Math.max(s, 7.8).toFixed(2)); bounds.add(+e.toFixed(2)); }
  ...
  // PRESENCIA: una vuelta al avatar de 2 s es un corte más, no presencia. Todo tramo FULL
  // posterior al hook se estira a un mínimo de 4.6 s comiéndose el arranque del siguiente
  // tramo oculto (si ese tramo es lo bastante largo como para no quedar en nada).
  const MIN_FULL = 4.6;
  for (let i = 0; i < out2.length - 1; i++) {
    if (out2[i].mode !== "full" || out2[i].start < HOOK_END) continue;
    const end = out2[i + 1].start;
    if (end - out2[i].start >= MIN_FULL) continue;
    const nextEnd = i + 2 < out2.length ? out2[i + 2].start : VIDEO_END;
    const want = out2[i].start + MIN_FULL;
    if (nextEnd - want >= 2.0) out2[i + 1].start = +want.toFixed(2);
  }
  const out3: AvatarWindow[] = [];
  for (const x of out2) { if (!out3.length || (out3[out3.length - 1].mode !== x.mode && x.start > out3[out3.length - 1].start)) out3.push(x); }
  return out3;
}
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "focuscards" ? <FocusCardsV89 durationInFrames={d} items={b.items} title={b.title} />
  : b.kind === "looplock" ? <LoopLockV89 durationInFrames={d} title={b.title} sub={b.sub} />
  : renderFederer2Comp(b, d, { medico: true });

// ── Endcard ES (inline, aislado por slug) ────────────────────────────────────────────────
const EndcardV89: React.FC<{ durationInFrames: number }> = ({ durationInFrames: D }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: op, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", transform: `translateY(${interpolate(enter, [0, 1], [40, 0])}px) scale(${interpolate(enter, [0, 1], [0.94, 1])})`, padding: "56px 80px", borderRadius: 40, textAlign: "center", background: "linear-gradient(150deg, rgba(255,255,255,0.9), rgba(240,248,249,0.82))", backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", border: "1.5px solid rgba(255,255,255,0.9)", boxShadow: "0 40px 90px rgba(0,0,0,0.5)" }}>
        <div style={{ fontFamily: F_INTER, fontWeight: 800, fontSize: 26, letterSpacing: 5, color: TEAL, textTransform: "uppercase", opacity: interpolate(frame, [6, 20], [0, 1], cl) }}>Federer Archivos</div>
        <div style={{ fontFamily: F_INTER, fontWeight: 900, fontSize: 88, color: INK, marginTop: 12, letterSpacing: -1, opacity: interpolate(frame, [10, 24], [0, 1], cl) }}>Suscribite</div>
        <div style={{ fontFamily: F_INTER, fontWeight: 500, fontSize: 34, color: "rgba(20,34,43,0.7)", marginTop: 14, maxWidth: 820, opacity: interpolate(frame, [16, 30], [0, 1], cl) }}>
          Cada semana, un remedio de cocina explicado como se lo explico a mis pacientes
        </div>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 16, marginTop: 34, padding: "20px 44px", borderRadius: 999, background: `linear-gradient(150deg, ${TEAL}, #0c8f8b)`, boxShadow: `0 16px 40px ${TEAL}66`, transform: `scale(${pulse})`, opacity: interpolate(frame, [22, 36], [0, 1], cl) }}>
          ...
          <span style={{ fontFamily: F_INTER, fontWeight: 800, fontSize: 38, color: "#fff" }}>SUSCRIBITE</span>
        </div>
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Tomás tres litros de agua por día…" impact="Y LA PIEL SIGUE COMO PAPEL DE SEDA." accentColor="#12B3AE" font={F_INTER} fontSize={80} />
      </Sequence>

      {/* ENDCARD */}
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <EndcardV89 durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} />
      </Sequence>
    </AbsoluteFill>
  );
};
