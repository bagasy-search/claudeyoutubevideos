...
import { FEDZ_BEATS } from "./federer_v43tq6jgz3dw_beats";
import { FEDZ_BROLL } from "./federer_v43tq6jgz3dw_broll";
import { TALKSZ } from "./federer_v43tq6jgz3dw_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer" · slug v43tq6jgz3dw ───────────────────────────────────────────────────
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO recuadro/PiP/split (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const INK = "#12222B";
const BG = "#0E1D23";
const AVA = "v43tq6jgz3dw_opt.mp4";
...
export const TOTAL_FRAMES_V43 = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => /^(hook|story|principio|al[1-6]|objecion|limites|error|recap|cta|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
const EndcardV43: React.FC<{ durationInFrames: number }> = ({ durationInFrames: D }) => {
    ...
    <AbsoluteFill style={{ opacity: op, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", transform: `translateY(${interpolate(enter, [0, 1], [40, 0])}px) scale(${interpolate(enter, [0, 1], [0.94, 1])})`, padding: "56px 80px", borderRadius: 40, textAlign: "center", background: "linear-gradient(150deg, rgba(255,255,255,0.9), rgba(240,248,249,0.82))", backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", border: "1.5px solid rgba(255,255,255,0.9)", boxShadow: "0 40px 90px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ fontFamily: F_INTER, fontWeight: 500, fontSize: 34, color: "rgba(20,34,43,0.7)", marginTop: 14, maxWidth: 820, opacity: interpolate(frame, [16, 30], [0, 1], cl) }}>
          La guía completa, con las medidas exactas, está en archivos-federer.vercel.app — el enlace está en la descripción
        </div>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 16, marginTop: 34, padding: "20px 44px", borderRadius: 999, background: `linear-gradient(150deg, ${TEAL}, #0c8f8b)`, boxShadow: `0 16px 40px ${TEAL}66`, transform: `scale(${pulse})`, opacity: interpolate(frame, [22, 36], [0, 1], cl) }}>
          ...
          <span style={{ fontFamily: F_INTER, fontWeight: 800, fontSize: 38, color: "#fff" }}>SUSCRÍBETE</span>
        </div>
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Eso que ves cada mañana en el espejo…" impact="NO ES LO QUE TE DIJERON." accentColor="#12B3AE" font={F_INTER} fontSize={82} />
      </Sequence>

      {/* ENDCARD */}
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <EndcardV43 durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} />
      </Sequence>
    </AbsoluteFill>
  );
};
