...
import { FEDRODILLAS_BEATS, FEDRODILLAS_BROLL, FEDRODILLAS_COVER, AVATAR_END, VIDEO_END as VEND } from "./fedrodillas_beats";
import { ClipShotFr } from "./scenes/ClipShotFr";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Archivos" · CAMINA ASI 2 MINUTOS AL DIA Y TUS RODILLAS SANAN A LOS 80 ──
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 918.741s sobre un master de 2760.40s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = Math.round(918.741 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = FEDRODILLAS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FEDRODILLAS_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FEDRODILLAS = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of FEDRODILLAS_COVER) {
  ...
  post.push({ start: 0, mode: "full" }, { start: 1.4, mode: "full" });
  const resume = coll.filter((w) => w.start < HOOK_END).pop();
  post.push({ start: HOOK_END, mode: resume?.mode ?? "full" });
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fedrodillas_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % 918.7) * 30)} objectPos="18% 30%" />
...
export const MainFedrodillas: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayerLoopFcs src="fedrodillas_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Esa puntada en el primer paso de la mañana. El crujido al levantarte del sillón. Y vos pensás que es la edad."
          impact="LA EDAD NO GASTA UNA RODILLA"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          kicker="Federer Archivos" title="Suscribite"
