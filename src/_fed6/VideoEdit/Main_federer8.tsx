...
import { FED8_BEATS } from "./federer8_beats";
import { FED8_BROLL } from "./federer8_broll";
import { TALKS8 } from "./federer8_hooks";
...
const compBeats = FED8_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED8_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED8_BEATS.map((b: any) => b.start + b.dur), FED8_BROLL.length ? FED8_BROLL[FED8_BROLL.length - 1].start + FED8_BROLL[FED8_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED8 = Math.round(VIDEO_END * 30);
...
FED8_BEATS.filter((b: any) => /^(bronca|ricardo|mecanismo_intro|vamos_siete|ej[1-7]|test|tres_cosas|cta_intro|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FED8_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /libro|cocina|pagina|guia|fe8_fed_/.test(b.src || "");
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS8.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS8) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer8_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer8_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer8: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer8_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="7 ejercicios para reactivar tu testosterona…" impact="NO ES SOLO LA EDAD" accentColor="#12B3AE" font={F_INTER} fontSize={128} />
      </Sequence>
