...
import { CREATININEGREENS_BEATS, CREATININEGREENS_BROLL, CREATININEGREENS_COVER, AVATAR_END, VIDEO_END as VEND } from "./creatininegreens_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · CREATININE / PARSLEY ───────
// ⛔ AVATAR PARCIAL: avatar grabó 0..423.366s de un master de 1039.609s.
//    · avatar EN BUCLE y MUTEADO (AvatarLayerLoopFcs) + <Audio> del master aparte;
//    · en la ZONA FISH (>=AVATAR_END) los labios NO sincronizan → el avatar nunca full.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "creatininegreens_opt.mp4";
const AVATAR_FRAMES = Math.round(423.366 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = CREATININEGREENS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = CREATININEGREENS_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_CREATININEGREENS = Math.round(VEND * 30);
...
// ⛔ ANTI-HUECO: zona avatar = avatar es FONDO garantizado en huecos; zona fish = nunca full.
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of CREATININEGREENS_COVER) {
...
export const MainCreatininegreens: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..7:03) + cola de Fish. El avatar va MUTEADO.
          Mismo wav que AvatarLayerLoopFcs usa para el borde → un solo asset en el tar. */}
      <Audio src={staticFile("creatininegreens.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v leve: cocina/preparación) */}
      {CREATININEGREENS_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (gpt-image low: presentador vlog-casero + pasos + LÁMINA cama) */}
      {rawTop.map((b: any) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Your blood test came back and one number crept up — creatinine — and your doctor just said 'keep an eye on it.' Here's the part nobody explained."
          impact="THE #1 VEGETABLE THAT PROTECTS YOUR KIDNEYS AFTER 60"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          ...
          subtitle="Simple, natural remedies you can do tonight — while you sleep" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
