...
import { FARMANOS_BEATS, FARMANOS_BROLL, FARMANOS_COVER, AVATAR_END, VIDEO_END as VEND } from "./farmanos_beats";
...
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 643.24s sobre un master de 1359.23s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = Math.round(643.24 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = FARMANOS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FARMANOS_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FARMANOS = Math.round(VEND * 30);
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "farmanos_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % 643.24) * 30)} objectPos="18% 30%" />
...
export const MainFarmanos: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayerLoopFcs src="farmanos_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Miren el dorso de sus manos: esas manchas marrones no son de la edad."
          impact="EL ACEITE QUE LAS BORRA EN 3 SEMANAS"
