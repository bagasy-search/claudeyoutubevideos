...
import { DS_BEATS } from "./darkspots_beats";
import { DS_BROLL } from "./darkspots_broll";
import { TALKSR } from "./darkspots_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · THE MELANIN BRAKE (bearberry-arbutin / age spots) ──
...
const compBeats = DS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = DS_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...DS_BEATS.map((b: any) => b.start + b.dur), DS_BROLL.length ? DS_BROLL[DS_BROLL.length - 1].start + DS_BROLL[DS_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_DS = Math.round(VIDEO_END * 30);
...
DS_BEATS.filter((b: any) => /^(honesty|lookhand|whatspot|rehook|everybody|patient|reveal|anger|whynight|how|safety)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...DS_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /ds_federer|ds_hook|dg_|cmp_|lamina|reveal|libro/.test(b.src || "");
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "darkspots_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "darkspots_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainDarkSpots: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS del presentador ds_federer_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="darkspots_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, y: 0.30, splitZoom: 1.05 }} />
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} kicker="Dr. Federer" title="Subscribe" subtitle="Every week — real, gentle skin science for your face and mature skin" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
