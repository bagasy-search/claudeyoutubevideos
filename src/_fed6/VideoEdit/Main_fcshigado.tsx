...
// ── ESCENAS PREMIUM con capas/profundidad ──
...
import { FCSHIGADO_BEATS, FCSHIGADO_BROLL, FCSHIGADO_COVER, AVATAR_END, AVATAR_FRAMES, VIDEO_END as VEND } from "./fcshigado_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Consejos Salud" · 5 SEÑALES DE QUE TU HÍGADO ESTÁ FALLANDO ──
// ⛔ AVATAR PARCIAL: el mp4 del creador dura AVATAR_END sobre un master de VIDEO_END.
...
// ⛔ `OffthreadVideo` en TODO lo que dibuja video (AvatarLayerLoopFcs y RawShot ya lo usan):
//    `<Video>` busca por tiempo y sirve cuadros equivocados en el render = "todo lageado".
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const HERO_CAP = 3.6;
// ⚠ TIENE QUE SER IDÉNTICO a `capOfDur` de gen_fcshigado.mjs o la compuerta anti-hueco miente.
...
const compBeats = FCSHIGADO_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FCSHIGADO_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_FCSHIGADO = Math.round(VEND * 30);
...
// ⛔⛔ POR UNIÓN, no por puntos con prioridad: el avatar está oculto mientras HAYA cualquier
// contenido activo (si no, vuelve a full apenas termina UNO aunque otro siga debajo).
function buildWindows(): AvatarWindow[] {
  type Iv = { a: number; b: number; solo: boolean };
  const ivs: Iv[] = [];
  for (const c of FCSHIGADO_COVER) ivs.push({ a: c.start, b: +(c.start + c.cov).toFixed(2), solo: c.kind === "video" && c.start + c.cov < AVATAR_END });
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fcshigado_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % AVATAR_END) * 30)} objectPos="50% 22%" />
...
export const MainFcshigado: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..AVATAR_END) + cola Fish. El avatar va MUTEADO. */}
      <Audio src={staticFile("fcshigado.m4a")} />

      {/* CAPA 1 — CLIPS (agnes i2v sobre la propia foto klein) */}
      {FCSHIGADO_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (klein + colas de clip) */}
      {rawTop.map((b: any) => {
        // ⛔ la duración TIENE que ser el `cov` que usó la ventana del avatar. Recalcularlo acá
        // con otro tope deja colas de 0,3-0,9 s con el fondo a la vista (= "pantalla negra").
      ...
      <AvatarLayerLoopFcs src="fcshigado_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
        avatarFocus={{ x: 0.52, y: 0.25, splitZoom: 1.05 }} avatarFrames={AVATAR_FRAMES} />

      {/* CAPA 3.5 — CAMA DE FOTO bajo TODO componente no-overlay (regla 2.quater): los
          full-screen del kit dejan ~60px de margen y sin nada debajo se ve el fondo plano. */}
      {compBeats.filter((b: any) => !OVERLAY.has(b.kind) && b.bed).map((b: any) => {
        const d = Math.max(1, sec(compDur(b) + 0.4));
        return (
          <Sequence key={`bed_${b.id}`} from={sec(b.start)} durationInFrames={d} premountFor={20}>
            <RawShot durationInFrames={d} src={b.bed} hue="cold" />
          </Sequence>
        );
      })}
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Tu hígado no duele. Por eso avisa por la piel, por las manos y por el color."
          impact="5 SEÑALES DE QUE TU HÍGADO ESTÁ FALLANDO"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        {/* ⛔ los defaults del Endcard son de OTRO video ("después de los 40"): se pisan TODOS. */}
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          kicker="Dr. Federer" title="Suscríbete" cta="SUSCRIBIRME"
          subtitle="Cada semana, salud real y explicada para después de los 60" />
      </Sequence>
    </AbsoluteFill>
  );
};
