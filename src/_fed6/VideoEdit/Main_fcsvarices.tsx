...
import {
  FCSVARICES_BROLL, FCSVARICES_PHOTOS, FCSVARICES_BEATS,
  AVATAR_WINDOWS, VIDEO_END, AVATAR_END,
} from "./fcsvarices_beats";

// ── CANAL "Federer Consejos Salud" · EL TRUCO NOCTURNO DEL ROMERO / VÁRICES ──────────────
// PARTICULARIDAD DE ESTE VIDEO: el avatar existe SOLO hasta AVATAR_END (20:21). El audio del
// tramo 1 es el del PROPIO mp4 del avatar (no se re-sincroniza nada); a partir de la costura
// entra la cola de la locución como pista aparte. Después de AVATAR_END no hay avatar, así que
// la cobertura visual del tramo 2 la garantizan clips + fotos (gen deja 0 huecos, verificado).
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const SEAM = AVATAR_END + 0.30;   // 0.3s de respiro entre el avatar y la cola

const OVERLAY = new Set(["lowerthird", "frasecinetica", "errorstinger"]);

export const TOTAL_FRAMES_FCSVARICES = Math.round(VIDEO_END * 30);
...
export const MainFcsvarices: React.FC = () => {
  const hookDur = 5.6;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS (agnes / hero gpt-image). En el tramo sin avatar estiran hasta el
          próximo contenido: son las que garantizan que no se vea el fondo. */}
      {FCSVARICES_PHOTOS.map((p: any) => {
      ...
      <AvatarLayer src="fcsvarices_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, y: 0.28, splitZoom: 1.1 }} />
      ...
      {/* HOOK — sobre el avatar, primeros segundos */}
      <Sequence from={sec(1.2)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText
          durationInFrames={sec(hookDur)}
          setup="¿Te levantas al baño a las 3 de la mañana? Puede no ser la próstata…"
          impact="ES EL AGUA DE TUS PIERNAS"
          accentColor={TEAL}
          font={F_INTER}
          fontSize={98}
        />
      ...
      {/* CTA — la guía, con QR real a drfederer.com (verificado por decodificación) */}
