...
// ── CANAL "Dr. Federer | Holistic Health" · THE NIGHT SERUM (aloe + rosemary) ──────
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const compBeats = NS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = NS_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...NS_BEATS.map((b: any) => b.start + b.dur), NS_BROLL.length ? NS_BROLL[NS_BROLL.length - 1].start + NS_BROLL[NS_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_NS = Math.round(VIDEO_END * 30);
...
export const MainNightserum: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src={AVATAR} windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ ...AVATAR_FOCUS_NS, splitZoom: 1.1 }} />
