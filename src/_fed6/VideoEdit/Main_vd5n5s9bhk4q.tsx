...
import { FocusCardsVd5 } from "./FocusCards_vd5n5s9bhk4q";
import { LoopLockVd5 } from "./LoopLock_vd5n5s9bhk4q";
import { F_INTER } from "./kit/premium/theme";
import { FEDZ_BEATS } from "./federer_vd5n5s9bhk4q_beats";
import { FEDZ_BROLL } from "./federer_vd5n5s9bhk4q_broll";
import { TALKSZ } from "./federer_vd5n5s9bhk4q_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer Archivos" · EL GUANTE DE ROMERO PARA LOS BRAZOS ────────────
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO halfR / recuadro (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVA = "vd5n5s9bhk4q_opt.mp4";
...
export const TOTAL_FRAMES_VD5 = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => b.id && b.id.endsWith("_0") && b.kind === "talk").forEach((b: any) => FULL_AT.push(b.start));
...
function buildWindows(): AvatarWindow[] {
  ...
  const inGap = (t: number) => t >= 8.0 && gaps.some(([s, e]) => t >= Math.max(s, 8.0) - 1e-6 && t < e - 1e-6);
  const bounds = new Set<number>(out.map((w) => w.start));
  for (const [s, e] of gaps) { if (e <= 8.0) continue; bounds.add(+Math.max(s, 8.0).toFixed(2)); bounds.add(+e.toFixed(2)); }
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "focuscards" ? <FocusCardsVd5 durationInFrames={d} items={b.items} title={b.title} />
  : b.kind === "looplock" ? <LoopLockVd5 durationInFrames={d} title={b.title} sub={b.sub} />
  : renderFederer2Comp(b, d, { medico: true });

export const MainVd5: React.FC = () => {
  const hookStart = 2.4;
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Si la piel de tus brazos se arruga como papel de seda…" impact="NO ES LA EDAD. ES ROMERO Y UN GUANTE" accentColor="#12B3AE" font={F_INTER} fontSize={100} />
      </Sequence>
