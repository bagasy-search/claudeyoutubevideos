...
import { FED16_BEATS } from "./federer16_beats";
import { FED16_BROLL } from "./federer16_broll";
import { TALKS16 } from "./federer16_hooks";
...
const compBeats = FED16_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED16_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED16_BEATS.map((b: any) => b.start + b.dur), FED16_BROLL.length ? FED16_BROLL[FED16_BROLL.length - 1].start + FED16_BROLL[FED16_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED16 = Math.round(VIDEO_END * 30);
...
FED16_BEATS.filter((b: any) => /^(medicina|tamano|antiox|honesto|receta|dos_formas|sol|error|detalle|injerto1|injerto2|elena_vuelve|clara|s1|s2|s3|primera_noche|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS16.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS16) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer16_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer16_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer16: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer16_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.48, splitZoom: 1.12 }} />
