...
import { FocusCardsUro } from "./FocusCards_v3iuzgxce9vg";
import { LoopLockUro } from "./LoopLock_v3iuzgxce9vg";
import { F_INTER } from "./kit/premium/theme";
import { FEDZ_BEATS } from "./federer_v3iuzgxce9vg_beats";
import { FEDZ_BROLL } from "./federer_v3iuzgxce9vg_broll";
import { TALKSZ } from "./federer_v3iuzgxce9vg_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";
import { FedWhiteboardUro } from "../../FedWhiteboard_v3iuzgxce9vg";

// ── CANAL "Federer Archivos" · POR QUÉ TE LEVANTÁS DE NOCHE A ORINAR (+60) ────────────────
// Avatar: FULL · HIDDEN (visual full). ⛔ CERO recuadro/PiP/split (feedback creador). Look CLÍNICO teal.
const TEAL = "#12B3AE";
const INK = "#12222B";
const BG = "#0E1D23";
const AVA = "v3iuzgxce9vg_opt.mp4";

const NEWFULL = new Set(["whiteboard", "avatarpizarra", "avatarkeyword", "mitoverdad", "errorstinger", "guardaesto", "freezezoom", "focuscards", "looplock"]);
const OVERLAY = new Set(["lowerthird", "frasecinetica"]);
const NOCAP = new Set(["avatarpizarra", "avatarkeyword", "focuscards", "whiteboard"]);
...
export const TOTAL_FRAMES_URO = Math.round(VIDEO_END * 30);
...
FEDZ_BEATS.filter((b: any) => b.sectionStart).forEach((b: any) => FULL_AT.push(b.start));
...
const renderComp = (b: any, d: number) =>
  b.kind === "whiteboard" ? <FedWhiteboardUro scene={b.scene} theme="white" />
  ...
  : b.kind === "focuscards" ? <FocusCardsUro durationInFrames={d} items={b.items} title={b.title} />
  : b.kind === "looplock" ? <LoopLockUro durationInFrames={d} title={b.title} sub={b.sub} />
  : renderFederer2Comp(b, d, { medico: true });

// ── Endcard ES (inline, aislado — no toca el Endcard compartido) ──────────────
const EndcardUro: React.FC<{ durationInFrames: number }> = ({ durationInFrames: D }) => {
    ...
    <AbsoluteFill style={{ opacity: op, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", transform: `translateY(${interpolate(enter, [0, 1], [40, 0])}px) scale(${interpolate(enter, [0, 1], [0.94, 1])})`, padding: "56px 80px", borderRadius: 40, textAlign: "center", background: "linear-gradient(150deg, rgba(255,255,255,0.9), rgba(240,248,249,0.82))", backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", border: "1.5px solid rgba(255,255,255,0.9)", boxShadow: "0 40px 90px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ fontFamily: F_INTER, fontWeight: 500, fontSize: 34, color: "rgba(20,34,43,0.7)", marginTop: 14, maxWidth: 780, opacity: interpolate(frame, [16, 30], [0, 1], cl) }}>
          Cada semana, algo de la circulación que nadie te explicó
        </div>
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Sequence from={sec(hookStart)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="El agua que te levanta a las tres de la mañana…" impact="NO LA TOMASTE DE NOCHE." accentColor="#12B3AE" font={F_INTER} fontSize={82} />
      </Sequence>

      {/* ENDCARD */}
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} layout="none">
        <EndcardUro durationInFrames={sec(Math.max(2, VIDEO_END - CTA_AT))} />
      </Sequence>
    </AbsoluteFill>
  );
};
