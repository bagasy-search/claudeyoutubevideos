...
import { FW_BEATS } from "./firmwater_beats";
import { FW_BROLL } from "./firmwater_broll";
import { TALKSR } from "./firmwater_hooks";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Dr. Federer | Holistic Health" · THE COLLAGEN LOCK (hibiscus water, firm skin) ──
...
// HOOK: freezezoom macro → mitoverdad → pricewar → avatar full recién en "dontgetup".
// LÁMINA: 5 freezezoom sobre lamina_board (zoom por zona) → reveal card (guía+QR) → guardaesto.
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const compBeats = FW_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FW_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FW_BEATS.map((b: any) => b.start + b.dur), FW_BROLL.length ? FW_BROLL[FW_BROLL.length - 1].start + FW_BROLL[FW_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FW = Math.round(VIDEO_END * 30);
...
const compDur = (b: any): number => {
  ...
  // TILE CONTIGUO hasta él (tope 13s) → sin huecos negros entre zoom y zoom. Un freezezoom SUELTO queda
  // corto 4.5s y el avatar rellena después.
...
FW_BEATS.filter((b: any) => /^(dontgetup|nobody|reveal|intro|patient|expect|science|mistake|whynight|villain|honesty|desc2)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FW_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /fw_federer|fw_hook|federer_kitchen|recipe_book|pagina|guia|lamina|reveal/.test(b.src || "");
  ...
  for (const b of compBeats) {
    ...
    // ANTI-HUECO: tras cerrar un componente el avatar vuelve a FULL (nunca fondo muerto).
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "firmwater_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "firmwater_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFirmWater: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS del presentador fw_federer_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="firmwater_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, y: 0.42, splitZoom: 1.15 }} />
