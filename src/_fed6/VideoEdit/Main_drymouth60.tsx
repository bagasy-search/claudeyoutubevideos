...
import { DRYMOUTH60_BEATS, DRYMOUTH60_BROLL, DRYMOUTH60_COVER, AVATAR_END, VIDEO_END as VEND } from "./drymouth60_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · WHY YOUR EXPENSIVE CREAM ───
// ⛔ AVATAR PARCIAL: el creador grabó 0..467.69s de un master de 1023.26s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "drymouth60_opt.mp4";
const AVATAR_FRAMES = Math.round(467.69 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
const compBeats = DRYMOUTH60_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = DRYMOUTH60_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_DRYMOUTH60 = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of DRYMOUTH60_COVER) {
...
export const MainDrymouth60: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Audio src={staticFile("drymouth60.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v, animación leve de su propia foto) */}
      {DRYMOUTH60_BROLL.map((b) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Those little lines on your nails? You were told it's just age. It isn't — in a lot of people it's low iron."
          impact="IT'S NOT AGE — IT'S IRON"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          ...
          subtitle="Simple, natural health you can act on tonight — while you sleep" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
