...
import { TAZA9PM_BEATS, TAZA9PM_BROLL, TAZA9PM_COVER } from "./taza9pm_beats";
...
//    tapados por contenido desde el gen (TAZA9PM_SEAMS), no hace falta nada acá.
...
const compBeats = TAZA9PM_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = TAZA9PM_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...TAZA9PM_BEATS.map((b: any) => b.start + b.dur),
  TAZA9PM_BROLL.length ? TAZA9PM_BROLL[TAZA9PM_BROLL.length - 1].start + TAZA9PM_BROLL[TAZA9PM_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_TAZA9PM = Math.round(VIDEO_END * 30);
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "taza9pm_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="55% 25%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "taza9pm_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainTaza9pm: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="taza9pm_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.55, y: 0.3, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="¿Te despiertas a las 3 de la mañana y ya no puedes volver a dormir? Un médico lo explica…" impact="LA TAZA DE LAS 9 PM" accentColor={TEAL} font={F_INTER} fontSize={104} />
      </Sequence>
