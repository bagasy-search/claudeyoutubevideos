...
import { FCSJAPON_BEATS } from "./fcsjapon_beats";
import { FCSJAPON_BROLL } from "./fcsjapon_beats";
import { FCSJAPON_TALKS } from "./fcsjapon_beats";
...
const compBeats = FCSJAPON_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FCSJAPON_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FCSJAPON_BEATS.map((b: any) => b.start + b.dur), FCSJAPON_BROLL.length ? FCSJAPON_BROLL[FCSJAPON_BROLL.length - 1].start + FCSJAPON_BROLL[FCSJAPON_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FCSJAPON = Math.round(VIDEO_END * 30);
...
FCSJAPON_BEATS.filter((b: any) => /^(rem[1-7]|story_intro|story_leccion|cierre_idea|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  const content = [...FCSJAPON_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => FCSJAPON_TALKS.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of FCSJAPON_TALKS) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const CONTENT_STARTS = [...FCSJAPON_BROLL.map((b: any) => b.start), ...rawTop.map((b: any) => b.start)].sort((a, b) => a - b);
...
export const MainFcsjapon: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (stock .mp4) */}
      {FCSJAPON_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS fcsjapon_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
