...
import { GRBANANA_BEATS, GRBANANA_BROLL, GRBANANA_COVER } from "./grbanana_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Golden Remedies" (EN) · I RUBBED COFFEE ON MY FACE FOR 7 DAYS ───────
// Clon de Main_estoalos70 (= Main_fcscanela + FIX ANTI-HUECO + FIX OVERLAY). Diferencias de ESTE video:
//  · el avatar es un BUCLE: la creadora grabo 5:31.6 de los 27:23, asi que grbanana_opt.mp4 es el
//    lean repetido con el master de Fish muxeado. El lipsync SOLO calza hasta la 1a costura (331.6s).
...
const AVATAR_CYCLE = 331.6;   // largo del tramo real de avatar; despues es bucle
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const compBeats = GRBANANA_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = GRBANANA_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...GRBANANA_BEATS.map((b: any) => b.start + b.dur),
  GRBANANA_BROLL.length ? GRBANANA_BROLL[GRBANANA_BROLL.length - 1].start + GRBANANA_BROLL[GRBANANA_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_GRBANANA = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  const hayContenido = (t: number) =>
    GRBANANA_COVER.some((c: any) => t >= c.start - 0.05 && t < c.start + c.cov - 0.05) ||
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "grbanana_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="50% 26%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "grbanana_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainGrbanana: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (clips agnes) */}
      {GRBANANA_BROLL.map((b) => {
      ...
      <AvatarLayer src="grbanana_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.45, y: 0.30, splitZoom: 1.12 }} />
