...
import { FED11_BEATS } from "./federer11_beats";
import { FED11_BROLL } from "./federer11_broll";
import { TALKS11 } from "./federer11_hooks";
...
const compBeats = FED11_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED11_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED11_BEATS.map((b: any) => b.start + b.dur), FED11_BROLL.length ? FED11_BROLL[FED11_BROLL.length - 1].start + FED11_BROLL[FED11_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED11 = Math.round(VIDEO_END * 30);
...
FED11_BEATS.filter((b: any) => /^(hook|resigne|promesa|porque_cumplimos|advertir|recete_carmen|no_magia|carmen_vuelve|don_ernesto|consejos_intro|despedirme|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FED11_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS11.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS11) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer11_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer11_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer11: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer11_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.48, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="Si tus piernas se hinchan después de los 60…" impact="NO ES LA EDAD. ES UNA SEÑAL." accentColor="#12B3AE" font={F_INTER} fontSize={104} />
      </Sequence>
