...
// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · MOST POWERFUL PROTEIN LEGS 90 ─
// ⛔ AVATAR PARCIAL: el creador grabó 0..630.24s de un master de 1028.67s.
...
export const TOTAL_FRAMES_NIGHTPROTEIN = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  for (const c of NIGHTPROTEIN_COVER) {
...
export const MainNightprotein: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <Audio src={staticFile("nightprotein.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v, animación leve de su propia foto) */}
      {NIGHTPROTEIN_BROLL.map((b) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Your legs aren't getting old — they're getting quiet. And they only rebuild themselves in one window: while you sleep."
          impact="THE $1 NIGHT PROTEIN FOR YOUR LEGS"
