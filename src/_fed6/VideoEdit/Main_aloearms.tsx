...
import { ALOEARMS_BEATS, ALOEARMS_BROLL, ALOEARMS_COVER, AVATAR_END, VIDEO_END as VEND } from "./aloearms_beats";

// ── CANAL "Dr. Federer — The Nightly Remedy" (EN) · ALOE + This: crepey ARMS at 90 ─
// ⛔ AVATAR PARCIAL: el creador grabó 0..758.08s de un master de 1046.60s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR = "aloearms_opt.mp4";
const AVATAR_FRAMES = 22747; // largo REAL del opt.mp4 (758.23s * 30) → el <Loop> lo repite
...
const compBeats = ALOEARMS_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = ALOEARMS_BEATS.filter((b: any) => b.kind === "raw" && /^img\//.test(b.src || ""));
export const TOTAL_FRAMES_ALOEARMS = Math.round(VEND * 30);
...
const renderComp = (b: any, d: number) =>
  ...
  : b.kind === "mitoverdad" ? <MitoRevelado durationInFrames={d} myth={b.myth} truth={b.truth} image={b.image} flipAt={b.flipAt} mythLabel={b.mythLabel ?? "THE MYTH"} truthLabel={b.truthLabel ?? "THE TRUTH"} />
...
export const MainAloearms: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..12:38) + cola de Fish. El avatar va MUTEADO.
          Es el MISMO wav que AvatarLayerLoopFcs usa para el borde → un solo asset en el tar. */}
      <Audio src={staticFile("aloearms.wav")} />

      {/* CAPA 1 — CLIPS (agnes i2v animación leve + stock on-topic) */}
      {ALOEARMS_BROLL.map((b) => {
      ...
      {/* CAPA 2 — FOTOS (gpt-image low: presentador vlog casero, producto, dolor, LÁMINA cama) */}
      {rawTop.map((b: any) => {
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Crepey, papery arms — the kind no cream ever fixes. Because the crinkle isn't on the surface: it's the layer underneath, and it only rebuilds while you sleep."
          impact="ALOE + VITAMIN E FOR CREPEY ARMS — EVEN AT 90"
      ...
      <Sequence from={sec(CTA_AT)} durationInFrames={sec(Math.max(2, VEND - CTA_AT))} layout="none">
        <Endcard durationInFrames={sec(Math.max(2, VEND - CTA_AT))}
          ...
          subtitle="Simple, natural skin remedies after 60 — while you sleep" cta="SUBSCRIBE" />
      </Sequence>
    </AbsoluteFill>
  );
};
