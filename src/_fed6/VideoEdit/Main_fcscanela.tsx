...
import { FCSCANELA_TALKS } from "./fcscanela_beats";
...
FCSCANELA_BEATS.filter((b: any) => /^(rem[1-7]|story_intro|story_leccion|cierre_idea|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  const content = [...FCSCANELA_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => FCSCANELA_TALKS.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of FCSCANELA_TALKS) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const CONTENT_STARTS = [...FCSCANELA_BROLL.map((b: any) => b.start), ...rawTop.map((b: any) => b.start)].sort((a, b) => a - b);
