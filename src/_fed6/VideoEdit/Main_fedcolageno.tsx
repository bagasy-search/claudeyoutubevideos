...
import { FEDCOLAGENO_BEATS, FEDCOLAGENO_BROLL, FEDCOLAGENO_COVER } from "./fedcolageno_beats";
import { renderFederer2Comp, COMP2_KINDS } from "./FedererComponents2";

// ── CANAL "Federer - Más Salud, Más Vida" · LA TAZA DE LAS 9 PM ──────────────────
// Clon de Main_fcscanela (que trae el FIX ANTI-HUECO). Diferencias de ESTE video:
//  · el avatar es un BUCLE de 152s bajo 23min de narración → los cortes del bucle ya vienen
//    tapados por contenido desde el gen (FEDCOLAGENO_SEAMS), no hace falta nada acá.
//  · se cablean 4 componentes-ESCENA que el Main viejo no mapeaba: HourDial, PriceWar,
//    IngredientDuo y BenefitLockReveal (la "vara": escena con capas, no tarjeta plana).
const TEAL = "#12B3AE";
const BG = "#0E1D23";
...
const compBeats = FEDCOLAGENO_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FEDCOLAGENO_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FEDCOLAGENO_BEATS.map((b: any) => b.start + b.dur),
  FEDCOLAGENO_BROLL.length ? FEDCOLAGENO_BROLL[FEDCOLAGENO_BROLL.length - 1].start + FEDCOLAGENO_BROLL[FEDCOLAGENO_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FEDCOLAGENO = Math.round(VIDEO_END * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of FEDCOLAGENO_COVER) {
    const forceHidden = c.kind === "photo";     // las fotos HERO del Dr. van SIEMPRE a pantalla completa
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fedcolageno_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="50% 22%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "fedcolageno_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFedcolageno: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (clips agnes) */}
      {FEDCOLAGENO_BROLL.map((b) => {
      ...
      <AvatarLayer src="fedcolageno_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.5, y: 0.3, splitZoom: 1.12 }} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="La crema de 400 pesos no puede entrar en tu piel. Un médico te explica por qué, con física de secundaria…" impact="EL ALIMENTO DE $1" accentColor={TEAL} font={F_INTER} fontSize={104} />
      </Sequence>
