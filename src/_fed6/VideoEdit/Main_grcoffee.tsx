...
import { GRCOFFEE_BEATS, GRCOFFEE_BROLL, GRCOFFEE_COVER } from "./grcoffee_beats";
...
//  · el avatar es un BUCLE: la creadora grabo 5:31.6 de los 27:23, asi que grcoffee_opt.mp4 es el
...
const compBeats = GRCOFFEE_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = GRCOFFEE_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...GRCOFFEE_BEATS.map((b: any) => b.start + b.dur),
  GRCOFFEE_BROLL.length ? GRCOFFEE_BROLL[GRCOFFEE_BROLL.length - 1].start + GRCOFFEE_BROLL[GRCOFFEE_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_GRCOFFEE = Math.round(VIDEO_END * 30);
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "grcoffee_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="50% 26%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "grcoffee_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainGrcoffee: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* CAPA 1 — B-ROLL DENSO continuo (clips agnes) */}
      {GRCOFFEE_BROLL.map((b) => {
      ...
      <AvatarLayer src="grcoffee_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.45, y: 0.31, splitZoom: 1.12 }} />
