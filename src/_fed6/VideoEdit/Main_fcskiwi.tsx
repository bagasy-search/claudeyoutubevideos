...
import { FCSKIWI_BEATS } from "./fcskiwi_beats";
import { FCSKIWI_BROLL } from "./fcskiwi_beats";
import { FCSKIWI_TALKS } from "./fcskiwi_beats";
...
const compBeats = FCSKIWI_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FCSKIWI_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FCSKIWI_BEATS.map((b: any) => b.start + b.dur), FCSKIWI_BROLL.length ? FCSKIWI_BROLL[FCSKIWI_BROLL.length - 1].start + FCSKIWI_BROLL[FCSKIWI_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FCSKIWI = Math.round(VIDEO_END * 30);
...
FCSKIWI_BEATS.filter((b: any) => /^(rem[1-7]|story_intro|story_leccion|cierre_idea|close)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  const content = [...FCSKIWI_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  ...
  let last = "full";
  const talkAt = (s: number) => FCSKIWI_TALKS.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of FCSKIWI_TALKS) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fcskiwi_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} objectPos="10% 45%" />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "fcskiwi_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
const CONTENT_STARTS = [...FCSKIWI_BROLL.map((b: any) => b.start), ...rawTop.map((b: any) => b.start)].sort((a, b) => a - b);
...
export const MainFcskiwi: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* CAPA 2 — FOTOS fcskiwi_*.png TOPEADAS (~3.6s) */}
      {rawTop.map((b: any) => {
      ...
      <AvatarLayer src="fcskiwi_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} avatarFocus={{ x: 0.29, y: 0.3, splitZoom: 1.12 }} />
