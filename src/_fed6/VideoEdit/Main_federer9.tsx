...
import { FED9_BEATS } from "./federer9_beats";
import { FED9_BROLL } from "./federer9_broll";
import { TALKS9 } from "./federer9_hooks";
...
const compBeats = FED9_BEATS.filter((b: any) => isComp(b.kind));
const rawTop = FED9_BEATS.filter((b: any) => b.kind === "raw" && /^(img|vid)\//.test(b.src || ""));
const VIDEO_END = Math.max(...FED9_BEATS.map((b: any) => b.start + b.dur), FED9_BROLL.length ? FED9_BROLL[FED9_BROLL.length - 1].start + FED9_BROLL[FED9_BROLL.length - 1].dur : 0) + 1.2;
export const TOTAL_FRAMES_FED9 = Math.round(VIDEO_END * 30);
...
FED9_BEATS.filter((b: any) => /^(error_teaser|enemigo_real|cosa\d+|ejercicio_magico|tramo_incomodo|recap|cierre)$/.test(b.key) && (b.id.endsWith("_0")))
...
function buildWindows(): AvatarWindow[] {
  ...
  const pts: Pt[] = [];
  let flip = false;
  const content = [...FED9_BROLL.map((b: any) => ({ start: b.start, src: b.src })), ...rawTop.map((b: any) => ({ start: b.start, src: b.src }))].sort((a, b) => a.start - b.start);
  for (const b of content) {
    const forceHidden = /libro|cocina|pagina|guia|fe9_fed_/.test(b.src || "");
  ...
  let last = "full";
  const talkAt = (s: number) => TALKS9.some((t) => s >= t.start - 0.05 && s < t.start + t.dur);
  for (const p of pts) {
    const mode: AvatarWindow["mode"] = p.pr < 3 && talkAt(p.start) ? "full" : p.mode;
    if (mode !== last) { w.push({ start: p.start, mode }); last = mode; }
  }
  for (const t of TALKS9) { w.push({ start: t.start, mode: "full" }); w.push({ start: +(t.start + t.dur).toFixed(2), mode: "hidden" }); }
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "federer9_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
  : b.kind === "avatarkeyword" ? <AvatarKeyword durationInFrames={d} items={b.items} avatar={b.clip || "federer9_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round(b.start * 30)} />
...
export const MainFederer9: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      <AvatarLayer src="federer9_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL} />
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)} setup="10 cosas cotidianas que bajan tu testosterona…" impact="SIN QUE TE DES CUENTA" accentColor="#12B3AE" font={F_INTER} fontSize={120} />
      </Sequence>
