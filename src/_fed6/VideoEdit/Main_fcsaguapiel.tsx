...
import { FCSAGUAPIEL_BEATS, FCSAGUAPIEL_BROLL, FCSAGUAPIEL_COVER, AVATAR_END, VIDEO_END as VEND } from "./fcsaguapiel_beats";
...
// ⛔ AVATAR PARCIAL: el mp4 del creador dura 905.877s sobre un master de 3024.15s.
...
const TEAL = "#12B3AE";
const BG = "#0E1D23";
const AVATAR_FRAMES = Math.round(905.877 * 30); // largo REAL del mp4 → el <Loop> lo repite
...
export const TOTAL_FRAMES_FCSAGUAPIEL = Math.round(VEND * 30);
...
function buildWindows(): AvatarWindow[] {
  ...
  let flip = false;
  for (const c of FCSAGUAPIEL_COVER) {
...
const renderComp = (b: any, d: number) =>
  b.kind === "avatarpizarra" ? <AvatarPizarra durationInFrames={d} items={b.items} avatar={b.clip || "fcsaguapiel_opt.mp4"} avatarFrom={b.clip ? 0 : Math.round((b.start % 905.8) * 30)} objectPos="18% 30%" />
...
export const MainFcsaguapiel: React.FC = () => {
  const hookDur = 5.4;
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* AUDIO MASTER — avatar real (0..15:05) + Fish desde la costura. El avatar va MUTEADO. */}
      <Audio src={staticFile("fcsaguapiel.wav")} />

      {/* CAPA 1 — CLIPS (agnes) */}
      {FCSAGUAPIEL_BROLL.map((b) => {
      ...
      <AvatarLayerLoopFcs src="fcsaguapiel_opt.mp4" windows={AVATAR_WINDOWS} accent={TEAL}
      ...
      <Sequence from={sec(1.4)} durationInFrames={sec(hookDur)} layout="none">
        <AvatarScrimText durationInFrames={sec(hookDur)}
          setup="Cuesta ocho pesos, está en la tienda de la esquina… y es la MISMA proteína de la que está hecha tu piel."
          impact="AGREGA ESTO AL AGUA PARA REAFIRMAR LA PIEL"
