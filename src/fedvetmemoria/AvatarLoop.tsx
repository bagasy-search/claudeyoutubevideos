...
// ⛔ Clonado a la carpeta del video A PROPÓSITO: el repo es COMPARTIDO y el original
// (src/_fed6/VideoEdit/scenes/AvatarLayerLoopFcs.tsx) ya se refactorizó bajo los pies de un
// render. Acá no depende de nadie. Se saca el import de `../theme` porque COLORS sólo servía
// de default de `accent`, que este video pasa explícito.
const COLORS = { accent: "#0F4A42" };
...
export const AvatarLoopVet: React.FC<{
  ...
  avatarFrames: number; // largo REAL del mp4 del avatar en frames -> se reproduce EN BUCLE
}> = ({ src, windows, accent = COLORS.accent, wav, avatarFocus, avatarFrames }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const t = frame; // frames

  // ⛔ SIN useAudioData a propósito: el master de este video dura 52 min (wav de 264 MB) y se
  // cargaría entero en CADA chunk del farm. El borde audio-reactivo sólo se ve en los modos con
  // chrome (corner/left/right) y este video usa únicamente full / hidden / halfR (chrome = 0).
  void wav;
  const amp = 0;
