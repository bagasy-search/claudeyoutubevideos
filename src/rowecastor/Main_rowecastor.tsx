...
// SFX por componente (volumen bajo, entra con el componente)
const SFX: Record<string, [string, number]> = {
  errorstinger: ["sfx/stinger_hit.mp3", 0.4], datoimpacto: ["sfx/number_slam.mp3", 0.35], carrusel: ["sfx/sfx_whoosh_soft.mp3", 0.35],
  mitoverdad: ["sfx/sfx_trans2.mp3", 0.3], checklist: ["sfx/layer_drop.mp3", 0.3], lowerthird: ["sfx/kicker_type.mp3", 0.25],
  frasecinetica: ["sfx/sfx_text_thud.mp3", 0.3], freezezoom: ["sfx/cam_zoom_punch.mp3", 0.3], callout: ["sfx/marker_drive.mp3", 0.3],
  lineatiempo: ["sfx/line_draw.mp3", 0.3],
};

// ── CANAL "Dr. Emmett Rowe" (EN) · rowecastor ─────────────────────────────────
// Audio = máster Fish (voz `rowe`). Avatar = InfiniteTalk/RunPod SÓLO en las ventanas visibles
// (reel `rowecastor_avatar.mp4`, recortado con trimBefore). Todo video por OffthreadVideo.
export const TOTAL_FRAMES_ROWECASTOR = TOTAL_FRAMES;
const AVATAR_REEL = "rowecastor_avatar.mp4";
...
export const MainRowecastor: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: "#0E1D23" }}>
    <Audio src={staticFile("rowecastor.m4a")} />
    ...
    {COMPS.map((c: any, i: number) => (
      <Sequence key={`c${i}`} from={c.from} durationInFrames={c.dur} layout="none">
        {renderComp(c, c.dur)}
        {SFX[c.kind] ? <Audio src={staticFile(SFX[c.kind][0])} volume={SFX[c.kind][1]} /> : null}
      </Sequence>
    ))}
  </AbsoluteFill>
);
