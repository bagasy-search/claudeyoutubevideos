...
import { CUES, OVERLAYS } from "./cues_mohonegro.gen";
import { AVATAR_WINDOWS, TOTAL_MOHONEGRO } from "./avatar_mohonegro.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en mohonegro_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_MOHONEGRO = Math.round(TOTAL_MOHONEGRO * 30);

export const MainMohonegro: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="mohonegro_opt.mp4" muted windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("mohonegro.wav")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
