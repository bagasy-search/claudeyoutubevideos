...
import { CUES, OVERLAYS } from "./cues_fbmoscas.gen";
import { AVATAR_WINDOWS, TOTAL_FBMOSCAS } from "./avatar_fbmoscas.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en fbmoscas_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_FBMOSCAS = Math.round(TOTAL_FBMOSCAS * 30);

export const MainFbmoscas: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="fbmoscas_opt.mp4" wav="fbmoscas.m4a" muted windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("fbmoscas.m4a")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
