...
import { CUES, OVERLAYS } from "./cues_salitre3.gen";
import { AVATAR_WINDOWS, TOTAL_SALITRE3 } from "./avatar_salitre3.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en salitre3_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_SALITRE3 = Math.round(TOTAL_SALITRE3 * 30);

export const MainSalitre3: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="salitre3_opt.mp4" muted windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("salitre3.wav")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
