...
import { CUES, OVERLAYS } from "./cues_sotanorico.gen";
import { AVATAR_WINDOWS, TOTAL_SOTANORICO } from "./avatar_sotanorico.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en sotanorico_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_SOTANORICO = Math.round(TOTAL_SOTANORICO * 30);

export const MainSotanorico: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="sotanorico_opt.mp4" muted loop windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("sotanorico.wav")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
