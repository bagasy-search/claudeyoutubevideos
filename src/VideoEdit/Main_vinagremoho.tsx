...
import { CUES, OVERLAYS } from "./cues_vinagremoho.gen";
import { AVATAR_WINDOWS, TOTAL_VINAGREMOHO } from "./avatar_vinagremoho.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en vinagremoho_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_VINAGREMOHO = Math.round(TOTAL_VINAGREMOHO * 30);

export const MainVinagremoho: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="vinagremoho_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
