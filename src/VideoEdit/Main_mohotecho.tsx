...
import { CUES, OVERLAYS } from "./cues_mohotecho.gen";
import { AVATAR_WINDOWS, TOTAL_MOHOTECHO } from "./avatar_mohotecho.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en mohotecho_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_MOHOTECHO = Math.round(TOTAL_MOHOTECHO * 30);

export const MainMohotecho: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="mohotecho_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
