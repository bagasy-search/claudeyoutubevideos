...
import { CUES, OVERLAYS } from "./cues_blackout25.gen";
import { AVATAR_WINDOWS, TOTAL_BLACKOUT25 } from "./avatar_blackout25.gen";

export const TOTAL_FRAMES_BLACKOUT25 = Math.round(TOTAL_BLACKOUT25 * 30);
...
// Arrancan DESPUÉS de que termina la CtaCard (977.2+6 y 1396.5+6): encadenados,
// primero la tarjeta explica, después queda el QR para escanear.
const QR_WINDOWS: [number, number][] = [
  [984.0, 1003.0],   // tras la CtaCard #1 ("into a plain little guide, it's down below")
  [1403.5, 1423.0],  // tras la CtaCard #2 (cierre, "every one of these in one place")
];

export const MainBlackout25: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap handheld={0} grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="blackout25_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
