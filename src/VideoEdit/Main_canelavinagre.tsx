...
import { CUES, OVERLAYS } from "./cues_canelavinagre.gen";
import { AVATAR_WINDOWS, TOTAL_CANELAVINAGRE } from "./avatar_canelavinagre.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en canelavinagre_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_CANELAVINAGRE = Math.round(TOTAL_CANELAVINAGRE * 30);

export const MainCanelavinagre: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="canelavinagre_opt.mp4" wav="canelavinagre_amp.wav" muted windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("canelavinagre.m4a")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
