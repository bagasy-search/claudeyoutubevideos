...
import { CUES, OVERLAYS } from "./cues_romerovinagre.gen";
import { AVATAR_WINDOWS, TOTAL_ROMEROVINAGRE } from "./avatar_romerovinagre.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en romerovinagre_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_ROMEROVINAGRE = Math.round(TOTAL_ROMEROVINAGRE * 30);

export const MainRomerovinagre: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayer src="romerovinagre_opt.mp4" windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
