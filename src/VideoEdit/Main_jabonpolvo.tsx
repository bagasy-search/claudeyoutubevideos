...
import { CUES, OVERLAYS } from "./cues_jabonpolvo.gen";
import { AVATAR_WINDOWS, TOTAL_JABONPOLVO } from "./avatar_jabonpolvo.gen";
...
//
// AVATAR PARCIAL + BUCLE, horneado en jabonpolvo_opt.mp4 (21:19) junto al máster de audio:
...
export const TOTAL_FRAMES_JABONPOLVO = Math.round(TOTAL_JABONPOLVO * 30);

export const MainJabonpolvo: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
      <CinematicWrap grain={0} vignette={0}>
        <AbsoluteFill style={{ backgroundColor: COLORS.bg0 }}>
          ...
          <AvatarLayerSal src="jabonpolvo_opt.mp4" wav="jabonpolvo.m4a" muted windows={AVATAR_WINDOWS} accent={COLORS.accent} />
          <Audio src={staticFile("jabonpolvo.m4a")} />
          {OVERLAYS.map((cue) => (
            <Sequence key={cue.key} from={sec(cue.start)} durationInFrames={sec(cue.dur)}>
              {cue.el(sec(cue.dur))}
            </Sequence>
