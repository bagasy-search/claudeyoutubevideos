// RayStage.tsx — ESCENARIO COMPARTIDO de THE ROWE FILES (EN-US, piel +70, look expediente).
//
...
export const V = {
  ...
  ink1: "#101B2C",
  ink2: "#18263A",
  brass: "#D9A94F",      // MANILA — el acento del expediente (carpeta, clip, pestaña)
  brassSoft: "#F1DDAE",  // papel manila claro — títulos, resaltados
...
export const Clip: React.FC<{ src: string; rate?: number; seed?: number; frames?: number; last?: string }> = ({ src, rate = 1, seed = 7, frames = 0, last }) => {
  const frame = useCurrentFrame();
  const { durationInFrames } = useVideoConfig();
  const n = Math.max(2, durationInFrames);
  // sin fade de entrada; Ken-Burns suave AL AZAR sobre TODO el plano. ⛔ el clip nunca se estira ni se repite:
  // si el plano dura más que el archivo, se sostiene su ÚLTIMO cuadro (jpg del build) con la MISMA curva.
  ...
  const st: React.CSSProperties = { width: "100%", height: "100%", objectFit: "cover", transformOrigin: `${ox.toFixed(1)}% ${oy.toFixed(1)}%`, transform: `scale(${z.toFixed(4)})` };
  const hold = !!last && frames > 1 && frame >= frames;
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      {hold ? <Img src={staticFile(last as string)} style={st} /> : <OffthreadVideo src={staticFile(src)} muted playbackRate={rate} style={st} />}
    </AbsoluteFill>
  );
};
...
export const StatBug: React.FC<{
  ...
  value = "", unit = "", caption = "", series = "THE ROWE FILES", tone = "brass",
}) => {
  const frame = useCurrentFrame();
...
/** La marca de esquina de THE ROWE FILES: carpeta de expediente con pestaña (mismo nombre de export). */
export const Keyring: React.FC<{ size?: number }> = ({ size = 34 }) => (
  <svg width={size} height={size} viewBox="0 0 40 40" style={{ overflow: "visible" }}>
    <path d="M4 12 h11 l3 -4 h18 v26 H4 z" fill="none" stroke={V.brass} strokeWidth="2.4" strokeLinejoin="round" />
    <path d="M4 16 h32" fill="none" stroke={V.brass} strokeWidth="2.2" />
    <path d="M12 24 h16 M12 29 h10" fill="none" stroke={V.brass} strokeWidth="2.2" strokeLinecap="round" />
  </svg>
);
