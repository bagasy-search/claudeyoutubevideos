...
export const FileTag: React.FC<{
  ...
  title: string;
  desc?: string;
  tag?: string;              // 3er textito (marca), default "THE ROWE FILES"
  tone?: "teal" | "warn";
  kicker?: string;           // etiqueta del bloque de acento (ej. "ADVERTENCIA", "DATO")
}> = ({ durationInFrames, title, desc, tag = "THE ROWE FILES", tone = "teal", kicker = "THE ROWE FILES" }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const accent = tone === "warn" ? "#D2463B" : "#D9A94F";
  const accentDark = tone === "warn" ? "#A3322A" : "#9C7428";
