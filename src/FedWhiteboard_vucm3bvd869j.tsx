/* Copia por-slug de FedWhiteboard (pizarra Vox) SIN el PiP de esquina.
 * El canal Federer Archivos prohibe el recuadro: o avatar full o visual full.
 * No se toca el archivo compartido src/FedWhiteboard.tsx. */
/**
 * ============================================================================
...
export type Scene = {avatarSrc?: string; elements: BoardEl[]; cameras?: CamKf[]; muted?: boolean};
...
/* ── ESCENA "POR QUÉ EL AJO" (hormigueo · vucm3bvd869j) ────────────────────────
 * Mecanismo paso a paso, escrito a mano: diente entero (dos piezas separadas) →
 * machacar → esperar 10 min → alicina → compuestos de azufre → la arteria se
 * relaja → llega mas sangre a la punta del nervio. Y el error: a la sartén ya.
 * Anclas (s) del audio: alicina 1118.7 · reaccion 1120.1 · sarten 1130.3.
 * -------------------------------------------------------------------------- */
const AJO = (n: string) => staticFile(`img/vucm3bvd869j/${n}.jpg`);

export const SCENE_AJO_VUCM: Scene = {
  cameras: [
    {time: 0.0, fx: 30, fy: 12, z: 1.14},
    {time: 2.2, fx: 18, fy: 44, z: 1.42},
    {time: 7.0, fx: 40, fy: 44, z: 1.38},
    {time: 12.0, fx: 62, fy: 44, z: 1.42},
    {time: 17.0, fx: 82, fy: 46, z: 1.44},
    {time: 21.5, fx: 50, fy: 48, z: 1.0},
  ],
  elements: [
    {t: 'title', x: 4, y: 6, text: 'Por que el ajo abre la arteria', start: 0.4},

    {t: 'card', x: 16, y: 44, w: 15, src: AJO('rm_cabeza_ajo'), label: 'Diente entero', caption: 'La pila y el cable,\nguardados aparte', start: 2.4, circle: true},
    {t: 'arrow', from: [25, 40], to: [31, 40], start: 6.2, curve: -0.25},
    {t: 'card', x: 38, y: 44, w: 15, src: AJO('rm_ajo_machacado'), label: 'Machacado', caption: 'Las dos piezas\nse encuentran', start: 7.0},
    {t: 'note', x: 38, y: 70, w: 22, text: '10 minutos al aire', start: 9.6, highlight: true, align: 'center'},
    {t: 'arrow', from: [47, 40], to: [54, 40], start: 11.4, curve: -0.25},
    {t: 'card', x: 61, y: 44, w: 15, src: AJO('rm_ajo_reposo'), label: 'Alicina', caption: 'Recien ahi\nse forma', start: 12.0},
    {t: 'arrow', from: [70, 40], to: [76, 40], start: 16.2, curve: -0.25},
    {t: 'card', x: 83, y: 44, w: 15, src: AJO('ci_manguera_doblada'), label: 'Arteria relajada', caption: 'Mas luz de paso,\nmas sangre al pie', start: 17.0},

    {t: 'pair', x: 18, y: 78, w: 22, src: AJO('rm_ajo_sarten'), text: 'A la sarten ya: no hay alicina', start: 19.4},
    {t: 'lasso', x: 18, y: 78, w: 26, h: 14, start: 20.6, rot: -2},
  ],
};
...
// @ts-ignore-unused (PiP desactivado en este video)
...
export type FedWhiteboardNPProps = {scene?: Scene; theme?: ThemeKey};

export const FedWhiteboardNP: React.FC<FedWhiteboardNPProps> = ({scene = SCENE_ROMERO, theme: themeKey = 'white'}) => {
  const frame = useCurrentFrame();
  ...
  return (
    <ThemeCtx.Provider value={theme}>
      <AbsoluteFill style={{background: theme.bg}}>
        ...
        {/* PiP DESACTIVADO en este video: el canal prohibe el recuadro en esquina. */}
...
export default FedWhiteboardNP;
