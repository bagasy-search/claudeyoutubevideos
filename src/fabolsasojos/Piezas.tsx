// Piezas.tsx — piezas de escena del video `fabolsasojos` (canal Federer Archivos). Clon de src/fa70estudios/Piezas.tsx.
...
/** CAMA bajo los componentes del kit _fed6 que no traen `bed` propio (regla 2.quater: nunca fondo plano). */
export const Cama: React.FC<{ src?: string; seed: number }> = ({ src, seed }) => {
  if (!src) return null;
  return (
    <AbsoluteFill>
      <Foto src={src} seed={seed} />
      <AbsoluteFill style={{ backgroundColor: "rgba(8,17,15,0.35)" }} />
    </AbsoluteFill>
  );
};

/** CLIP SIN REPETIR (regla agnes_qc): el clip corre UNA vez hasta su último cuadro y el resto del plano es ese
 *  último cuadro (jpg extraído), bajo la MISMA curva Ken-Burns (el transform vive en el contenedor). */
export const ClipHold: React.FC<{ src: string; last: string; seed: number; frames: number; startFrom?: number }> = ({ src, last, seed, frames, startFrom = 0 }) => {
  const frame = useCurrentFrame();
  const kb = useKenBurns(seed, 1.03, 0.025, 0.06, 0.6);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      <AbsoluteFill style={{ ...kb }}>
        {frame < frames
          ? <OffthreadVideo src={staticFile(src)} muted playbackRate={1} startFrom={Math.round(startFrom * 30)} style={st} />
          : <Img src={staticFile(last)} style={st} />}
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
