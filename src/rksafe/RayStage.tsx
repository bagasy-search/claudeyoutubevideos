...
// ── PALETA / TIPOGRAFÍA ──────────────────────────────────────────────────────────────────────
export const V = {
  ink0: "#0A0A0C",       // negro del canal (fondo)
  ink1: "#131316",
  ink2: "#1E1E22",
  brass: "#C8912F",      // latón — el acento del canal (la llave, el keyring)
  brassSoft: "#E4B75C",  // ámbar suave — títulos, resaltados
  amber: "#E4B75C",
  danger: "#D8352A",     // ROJO — SÓLO el punto débil / la alerta
  dangerSoft: "#F0564B",
  white: "#F4F1E9",
  bone: "#CFC8B8",
  steel: "#8C8C94",      // el metal de la caja fuerte
...
export const PhotoBed: React.FC<{ src?: string; dim?: number }> = ({ src, dim = 0.62 }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      <Img src={staticFile(src)} style={{ width: "100%", height: "100%", objectFit: "cover", filter: `brightness(${Math.max(0.62, 1 - dim).toFixed(2)})`, transform: `scale(${z.toFixed(4)})` }} />
...
export const Clip: React.FC<{ src: string }> = ({ src }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      <OffthreadVideo src={staticFile(src)} muted style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 1, transform: `scale(${z.toFixed(4)}) translateX(${x.toFixed(3)}%)` }} />
    </AbsoluteFill>
  );
};
...
/** El keyring de latón: la marca de esquina del canal (SVG, sin assets). */
export const Keyring: React.FC<{ size?: number }> = ({ size = 34 }) => (
  <svg width={size} height={size} viewBox="0 0 40 40" style={{ overflow: "visible" }}>
    <circle cx="15" cy="15" r="9" fill="none" stroke={V.brass} strokeWidth="3" />
    <rect x="22" y="19" width="4.5" height="16" rx="1.4" fill={V.brass} transform="rotate(-40 24 27)" />
    <rect x="26" y="26" width="7" height="3.2" rx="1" fill={V.brass} transform="rotate(-40 29 27)" />
    <rect x="29" y="30" width="6" height="3.2" rx="1" fill={V.brass} transform="rotate(-40 32 31)" />
  </svg>
);
