...
export const VoltAtmos: React.FC<{
  ...
  // ⛔ la respiracion de la luz clave estaba pulsando el cuadro ENTERO un 5,5 % cada 2,9 s.
  // No se puede señalar mirando un cuadro, pero es la mitad de la sensacion de "no descansa".
  const breathe = 0.955;
  return (
        ...
        {Array.from({ length: 8 }, (_, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: rgba(V.white, 0.06 + rnd(i * 6.6) * 0.08),
            }} />
          );
        })}
      </AbsoluteFill>
...
export const MediaCard: React.FC<{
  ...
  const frame = useCurrentFrame();
  // deriva bajada de 2,4 px a 0,9 px y la rotacion de 0,5 a 0,15 grados: la tarjeta sigue
  // VIVA (no es un PNG pegado) pero deja de flotar, que es lo que cansaba.
  const drift = Math.sin(frame / 41 + x) * 0.9;
  const driftR = Math.sin(frame / 67 + y) * 0.15;
  // ⛔ barrido especular APAGADO: habia 80 en el video y es brillo por el brillo mismo.
  const sheen = -1;
  void sheenAt;
  return (
    <div
...
// ⛔⛔ DOS MINAS ARREGLADAS ACA, LAS DOS CAZADAS ESCRIBIENDO ESTE VIDEO. Se arreglan en el
// Stage y no en los 46 lugares donde se llama, porque si no hay que acordarse 46 veces.
//
// 1. NO CUBRIA EL CUADRO. La cama vive adentro de un `<Plane z={-520}>` y ademas se aplicaba
//    su propio `translateZ(-400)`: los dos se SUMAN (-920) y con `perspective(1500px)` eso
//    encoge la cama al 62 %. Con `scale` 1.18-1.24 quedaba en 0.73-0.77 del cuadro, o sea que
//    por los bordes se veia el fondo plano — justo el defecto que la cama existe para tapar.
//    Ahora la cama NO aplica z propio (la profundidad se la da el Plane que la contiene) y su
//    escala tiene un PISO calculado contra la profundidad real de la cama en este video.
// 2. `kind="video"` SE CONGELABA. Igual que `MediaCard` antes de su fix: los clips duran
//    CLIP_FRAMES y `OffthreadVideo` se clava en su ultimo cuadro. Un fondo congelado se nota
//    mas que uno quieto, porque el resto de la escena sigue moviendose.
const CAMA_Z = -520;                                    // convencion del video: <Plane z={-520}>
const CAMA_MIN = (1500 - CAMA_Z) / 1500 * 1.06;         // ~1.43 — el 6 % es margen de la deriva
...
}> = ({ src, z = 0, scale = 1.18, dim = 0.44, kind = "photo", startFrom = 0, tint = V.volt }) => {
  void z;                                               // la profundidad la pone el <Plane> de afuera
  const frame = useCurrentFrame();
  const px = Math.sin(frame / 121) * 8;
  const s = Math.max(scale, CAMA_MIN);
  return (
    <AbsoluteFill style={{ transform: `translateX(${px.toFixed(1)}px) scale(${s.toFixed(3)})`, overflow: "hidden" }}>
      {kind === "video"
        ? (
          <Sequence from={Math.floor(frame / CLIP_FRAMES) * CLIP_FRAMES} durationInFrames={CLIP_FRAMES} layout="none">
...
// ⛔ EL DESTELLO ESTA APAGADO A PROPOSITO. Habia 42 en el video y es el efecto mas agresivo
// del kit: un fogonazo que no aporta ninguna informacion. Sin el, la frontera queda como CORTE
// SECO, que es lo que hace una edicion limpia. Se deja la firma para no tocar los 17 movimientos.
export const SeamFlash: React.FC<{ at: number; color?: string; dur?: number }> = () => null;
