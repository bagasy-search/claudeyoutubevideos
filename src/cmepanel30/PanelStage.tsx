// VoltStage.tsx — ESCENARIO COMPARTIDO del video `cmepanel30`
...
export const MediaCard: React.FC<{
  src: string;                      // ej: broll/cmepanel30/<nombre>.mp4  |  img/cmepanel30/<nombre>.png
  kind?: "video" | "photo";
  w?: number; h?: number;
...
export const SeamOcclude: React.FC<{ at: number; dur?: number; color?: string; angle?: number }> = ({
  ...
  // ⛔ BUG ARREGLADO (cmepanel30, ago-2026): iba de left -170% a +170% con width 320%, o sea que en
  // p=0 la losa ya cubria de -170% a +150% -> TAPABA LA PANTALLA ENTERA DESDE EL PRIMER FRAME y se
  // leia como un flash a losa, no como algo que cruza. Ahora arranca FUERA DE CUADRO a la derecha
  // (left 100%) y sale entera por la izquierda (left -320%), con ~50% del `dur` de cobertura total.
  return (
    <AbsoluteFill style={{ overflow: "hidden", pointerEvents: "none" }}>
      <div style={{
        position: "absolute", top: "-60%", left: `${lerp(100, -320, p).toFixed(1)}%`,
...
// ════════════════════════════════════════════════════════════════════════════════════════════
// LEYES PROPIAS DE `cmepanel30` — las comparten los 12 movimientos. Ver `_v3/cmepanel30_VARA.md`.
// ════════════════════════════════════════════════════════════════════════════════════════════

/** LA HORA DEL DÍA POR SECCIÓN. El sol es un personaje: su ángulo y su temperatura marcan en qué
 *  momento del mes estamos. Un movimiento entra con la hora de su seccion y sale con la de la
 *  siguiente — asi la luz EVOLUCIONA de S1 a S12 sin saltar nunca. */
export const HORA = {
  brasa:    { ang: -18, temp: "torch" as Lum, amb: 0.16 }, // S1 arranque: negro con una brasa
  limpio:   { ang: 22,  temp: "white" as Lum, amb: 0.62 }, // S1 salida / S2: dia limpio
  taller:   { ang: 34,  temp: "amber" as Lum, amb: 0.44 }, // S2-S3: mesa de taller calida
  mediodia: { ang: 88,  temp: "white" as Lum, amb: 0.95 }, // S3-S4: cenital duro del patio
  cenital:  { ang: 90,  temp: "bone" as Lum,  amb: 1.00 }, // S4-S5: blanco de laboratorio
  penumbra: { ang: 8,   temp: "sky" as Lum,   amb: 0.22 }, // S5-S6: el hueco del medidor
  alerta:   { ang: 14,  temp: "amber" as Lum, amb: 0.38 }, // S6: el medidor que te cobra
  rasante:  { ang: 6,   temp: "amber" as Lum, amb: 0.70 }, // S7-S8: la tarde dorada (el OESTE)
  noche:    { ang: -12, temp: "torch" as Lum, amb: 0.12 }, // S8-S9: el interior de noche
  tormenta: { ang: -30, temp: "sky" as Lum,   amb: 0.09 }, // S9: el apagon
  papel:    { ang: 70,  temp: "white" as Lum, amb: 0.88 }, // S10-S11: la factura sobre la mesa
  cierre:   { ang: 12,  temp: "amber" as Lum, amb: 0.58 }, // S12: el garaje al atardecer
};

/** LA LEY DE DIRECCION (no negociable, §5 de la vara):
 *  lo que TE COBRAN entra desde ARRIBA y en FRIO · lo que TE QUEDA entra desde ABAJO y en CALIDO.
 *  Devuelve el desplazamiento de entrada y el color de borde que le corresponde a un elemento. */
export const flujo = (tipo: "cobran" | "queda", t: number) => {
  const arriba = tipo === "cobran";
  const e = interpolate(clamp01(t), [0, 1], [0, 1], { easing: Easing.bezier(0.16, 0.84, 0.24, 1) });
  return {
    dy: (1 - e) * (arriba ? -190 : 190),
    color: arriba ? V.sky : V.amber,
    glow: arriba ? rgba(V.sky, 0.34) : rgba(V.amber, 0.4),
    opacity: e,
  };
};

/** KEY LIGHT DIRECCIONAL. Se monta DENTRO de la atmosfera (nunca encima del avatar: en el trailer
 *  el avatar va LIMPIO, sin una sola capa apoyada sobre su cara). `ang` en grados: 0 = rasante desde
 *  la izquierda, 90 = cenital. */
export const SunKey: React.FC<{ ang: number; temp?: Lum; amb?: number; soft?: number }> = ({
  ang, temp = "white", amb = 0.6, soft = 62,
}) => {
  const rad = (ang * Math.PI) / 180;
  const x = 50 - Math.cos(rad) * 58;
  const y = 50 - Math.sin(rad) * 62;
  return (
    <AbsoluteFill style={{
      pointerEvents: "none",
      background:
        `radial-gradient(${soft}% ${soft * 0.82}% at ${x.toFixed(1)}% ${y.toFixed(1)}%, ` +
        // ⛔ ERA 0.5/0.16 y LAVABA EL CUADRO. En `screen` eso es medio blanco encima de todo, y algun
        // movimiento apila TRES SunKey a la vez -> el creador lo vio como "un filtro que aumenta el
        // brillo y se ve horrible", sobre todo sobre el avatar, que va SIN filtros por directiva.
        // Sigue siendo una key direccional, pero ya no es un velo.
        `${rgba(V[temp], 0.17 * amb)} 0%, ${rgba(V[temp], 0.055 * amb)} 38%, transparent 72%)`,
      mixBlendMode: "screen",
    }} />
  );
};

/** LA FORMA MADRE del video: el rectangulo del panel con su grilla de celdas.
 *  Es el objeto que CRUZA las fronteras de todo el video — se vuelve la factura, la tabla, la
 *  pantalla del medidor, la hoja del cuaderno. Toda costura MATCH-SHAPE del video nace de aca.
 *  `cells` 0 = superficie lisa (ya es papel/factura); 1 = grilla de panel a full. */
export const PanelForm: React.FC<{
  w: number; h: number; cells?: number; tint?: string; children?: React.ReactNode; style?: React.CSSProperties;
}> = ({ w, h, cells = 1, tint = "#0E1A2B", children, style }) => {
  const k = clamp01(cells);
  const cols = 6, rows = 4;
  return (
    <div style={{
      width: w, height: h, position: "relative", overflow: "hidden",
      background: `linear-gradient(152deg, ${tint} 0%, #060B14 62%, ${tint} 100%)`,
      borderRadius: lerp(10, 3, k),
      boxShadow: `0 ${(h * 0.06).toFixed(0)}px ${(h * 0.16).toFixed(0)}px ${rgba(V.ink0, 0.72)},
                  inset 0 1px 0 ${rgba(V.white, 0.16)}`,
      border: `${lerp(2, 5, k).toFixed(1)}px solid ${rgba(V.bone, lerp(0.2, 0.5, k))}`,
      ...style,
    }}>
      {/* la grilla de celdas: se DESVANECE cuando la forma se convierte en papel */}
      <div style={{
        position: "absolute", inset: 0, opacity: k,
        backgroundImage:
          `linear-gradient(${rgba(V.bone, 0.22)} 1.5px, transparent 1.5px),` +
          `linear-gradient(90deg, ${rgba(V.bone, 0.22)} 1.5px, transparent 1.5px)`,
        backgroundSize: `${(w / cols).toFixed(2)}px ${(h / rows).toFixed(2)}px`,
      }} />
      {/* barrido especular: el hold VIVO obligatorio, nada quieto mas de 1,5 s */}
      <div style={{
        position: "absolute", inset: 0, opacity: 0.5 * k,
        background: `linear-gradient(104deg, transparent 34%, ${rgba(V.white, 0.2)} 50%, transparent 66%)`,
      }} />
      {children}
    </div>
  );
