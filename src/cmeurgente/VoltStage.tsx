// VoltStage.tsx — ESCENARIO COMPARTIDO del video `cmeurgente`
...
export const V = {
  ...
  blade: "#E9E9E4",
  panel: "#33507E",      // el azul del panel solar: lo que TODAVIA no hay que comprar
  roof:  "#7A5140",      // la teja del tejado de Ernesto: MATERIA de oclusion
      // blanco plástico neutro
};
...
export const MediaCard: React.FC<{
  src: string;                      // ej: broll/cmeurgente/<nombre>.mp4  |  img/cmeurgente/<nombre>.png
  kind?: "video" | "photo";
  w?: number; h?: number;
  ...
  return (
    ...
    >
      ...
      {grade && <AbsoluteFill style={{ background: rgba(litColor, 0.018), mixBlendMode: "soft-light" }} />}
      {/* bisel + reflejo superior del vidrio */}
      <AbsoluteFill style={{ boxShadow: `inset 0 1px 0 ${rgba(V.white, 0.13 * lit)}, inset 0 0 ${Math.round(h * 0.2)}px ${rgba(V.ink0, 0.5)}` }} />
      <AbsoluteFill style={{ background: `linear-gradient(178deg, ${rgba(V.white, 0.035 * lit)} 0%, rgba(255,255,255,0) 22%)` }} />
      {/* barrido especular: la tarjeta se lee como VIDRIO, no como recorte */}
      {sheen >= 0 && sheen <= 1 && (
        <AbsoluteFill style={{
          background: `linear-gradient(104deg, rgba(255,255,255,0) 38%, ${rgba(V.white, 0.11)} 50%, rgba(255,255,255,0) 62%)`,
...
export const Readout: React.FC<{
  value: string; unit?: string; label?: string; at?: number; hold?: number;
  x?: number; y?: number; size?: number; color?: string; align?: "left" | "center" | "right";
}> = ({ value, unit, label, at = 0, hold = 92, x = 50, y = 50, size = 128, color = V.volt, align = "center" }) => {
  const frame = useCurrentFrame();
  const p = clamp01((frame - at) / 9);
  if (frame < at - 1) return null;
  // ⛔⛔ EL READOUT TENIA ENTRADA Y NO TENIA SALIDA: una vez que aparecia se quedaba hasta el final
  // del movimiento, asi que las cifras SE ACUMULABAN (cinco juntas al final de MovCientoVeintiseis).
  // Ese apilamiento es lo que se lee como "sobrecargado". Ahora vive `hold` cuadros (5,6 s por
  // defecto: mas que el tiempo de lectura) y se va. Sale flotando hacia arriba, no en un fundido
  // plano, y como esta posicionado en absoluto no mueve NADA de lo que tiene al lado.
  const out = clamp01((frame - at - hold) / 12);
  ...
  return (
    <div style={{
      position: "absolute", left: `${x}%`, top: `${y}%`,
      transform: `translate(-50%,-50%) translateY(${(-out * 22).toFixed(1)}px) scale(${pop.toFixed(3)})`,
      opacity: 1 - out,
      textAlign: align, whiteSpace: "nowrap",
    }}>
      {label && (
        <div style={{
          fontFamily: F_DISPLAY, fontWeight: 700, fontSize: Math.round(size * 0.2), letterSpacing: 3.6, opacity: 0.5,
...
export const Kick: React.FC<{ children?: React.ReactNode; color?: string }> = () => null;
...
export const Body: React.FC<{ children?: React.ReactNode; size?: number; color?: string }> = () => null;
...
// ── ⭐ SUN FIELD — LA FIRMA VISUAL DE `cmeurgente` ───────────────────────────────────────────
// Veinticuatro celdas = las veinticuatro horas del día. Sólo las de SOL (9 a 16) se encienden en
// volt; las demás quedan en el azul frío de la noche. Es literalmente el NÚMERO DOS del video
// ("cuánto de tu consumo cae mientras hay sol") y funciona como textura de ambiente en cualquier
// movimiento: la barra late despacio y un cabezal recorre la hora actual.
//   sun  = fracción del día con sol (por defecto 7/24)
//   from = hora en la que arranca el sol (9)
//   use  = 0..1 → cuánto de la casa consume DENTRO del sol (pinta esas celdas en ámbar)
export const SunField: React.FC<{
  sun?: number; from?: number; cells?: number; on?: number; use?: number;
  tint?: string; night?: string; y?: number; w?: number; h?: number; cycle?: number;
}> = ({ sun = 7 / 24, from = 9, cells = 24, on = 1, use = 0, tint = V.volt, night = V.sky,
        y = 50, w = 1240, h = 46, cycle = 210 }) => {
  const frame = useCurrentFrame();
  const head = ((frame / cycle) * cells) % cells;
  const nSun = Math.round(cells * clamp01(sun));
  const nUse = Math.round(nSun * clamp01(use));
  return (
    <div style={{
      position: "absolute", left: "50%", top: `${y}%`, width: w, height: h,
      marginLeft: -w / 2, marginTop: -h / 2, display: "flex",
      gap: Math.max(2, Math.round(w / cells / 7)), opacity: clamp01(on),
    }}>
      {Array.from({ length: cells }, (_, i) => {
        const haySol = i >= from && i < from + nSun;
        const usado = haySol && i < from + nUse;
        const paso = clamp01(1 - Math.abs(((head - i + cells) % cells)) / 2.2);
        const a = haySol ? 0.55 + 0.45 * paso : 0.10 + 0.07 * paso;
        const c = usado ? V.amber : haySol ? tint : night;
        return (
          <div key={i} style={{
            flex: 1, borderRadius: 3,
            background: rgba(c, a),
            boxShadow: haySol ? `0 0 ${Math.round(10 + 20 * paso)}px ${rgba(c, 0.42 * a)}` : "none",
          }} />
        );
      })}
    </div>
...
// ── EL TEJADO — el plano donde IRÍAN los paneles (y donde al final hay menos) ────────────────
// Hermano de PadPlane: mismo contrato, pero inclinado y con la trama de tejas. Sirve de SUELO
// para las tarjetas del tramo del tejado y de MATERIA (V.roof) para las oclusiones de esa zona.
//   panels = 0..1 → cuánto del tejado está cubierto de azul (0 = tejado limpio)
export const RoofPlane: React.FC<{
  y?: number; w?: number; h?: number; rx?: number; lit?: number; z?: number; panels?: number;
}> = ({ y = 72, w = 1420, h = 320, rx = 58, lit = 1, z = -140, panels = 0 }) => {
  const frame = useCurrentFrame();
  const breathe = 1 + Math.sin(frame / 103) * 0.0035;
  const cols = 8;
  const nOn = Math.round(cols * clamp01(panels));
  return (
    <div style={{
      position: "absolute", left: "50%", top: `${y}%`, width: w, height: h, marginLeft: -w / 2,
      transform: `translateZ(${z}px) rotateX(${rx}deg) scale(${breathe.toFixed(4)})`,
      transformStyle: "preserve-3d", transformOrigin: "50% 0%",
      background: `linear-gradient(178deg, ${rgba(V.roof, 0.34 * lit)} 0%, ${rgba(V.roof, 0.14 * lit)} 44%, ${rgba(V.ink0, 0.92)} 100%)`,
      boxShadow: `inset 0 1px 0 ${rgba(V.white, 0.14 * lit)}, 0 -18px 60px ${rgba(V.ink0, 0.82)}`,
      borderRadius: 2, overflow: "hidden",
    }}>
      <AbsoluteFill style={{
        opacity: 0.22 * lit, mixBlendMode: "overlay",
        backgroundImage:
          "repeating-linear-gradient(0deg, rgba(255,255,255,.42) 0 1px, rgba(0,0,0,0) 1px 26px)," +
          "repeating-linear-gradient(90deg, rgba(0,0,0,.5) 0 1px, rgba(0,0,0,0) 1px 54px)",
      }} />
      <div style={{ position: "absolute", inset: "12% 6% 26% 6%", display: "flex", gap: 10 }}>
        {Array.from({ length: cols }, (_, i) => {
          const onP = clamp01(nOn - i);
          if (onP <= 0) return <div key={i} style={{ flex: 1 }} />;
          return (
            <div key={i} style={{
              flex: 1, borderRadius: 2,
              background: `linear-gradient(160deg, ${rgba(V.panel, 0.92 * onP)} 0%, ${rgba(V.panel, 0.55 * onP)} 60%, ${rgba(V.ink1, 0.9 * onP)} 100%)`,
              boxShadow: `inset 0 0 0 1px ${rgba(V.steel, 0.5 * onP)}, 0 6px 18px ${rgba(V.ink0, 0.7 * onP)}`,
            }} />
          );
        })}
      </div>
