/**
 * ============================================================================
 * Main_valeriabocaseca — Doctora Valeria Alcázar · "BOCA SECA: 7 señales +60"
 ...
 * ⚠ AVATAR EN BUCLE: el avatar real dura 5:27 y el video ~29 min. El <OffthreadVideo>
 *   va MUTEADO dentro de un <Loop>; el audio sale de un <Audio> aparte con el MASTER
 *   (parte A = audio real del avatar, parte B = Fish clonado). Así el labial calza
 *   perfecto en los primeros 5:27 y después el avatar es fondo vivo.
 * Los beats salen de ./cues_valeriabocaseca.gen (los genera build_valeriabocaseca.mjs,
...
import {BEATS, TOTAL_FRAMES_VBS, type Cue} from './cues_valeriabocaseca.gen';

const AVATAR = staticFile('valeriabocaseca_opt.mp4');
const MASTER = staticFile('valeriabocaseca.wav');
/** frames exactos del avatar real (327.786 s @30fps) — el bucle corta ahí */
const AVATAR_FRAMES = 9833;
...
const AvatarLayer: React.FC<{cuts: number[]}> = ({cuts}) => {
  ...
  const dust = React.useMemo(() => makeMotes(7, 'vbs-dust', 2, 5, 0.008, 0.02, 0.05, 0.12), []);
  return (
    <>
      <AbsoluteFill
        style={{
      ...
      >
        <Loop durationInFrames={AVATAR_FRAMES} layout="none">
...
export const MainValeriaBocaSeca: React.FC = () => {
...
export const TOTAL_FRAMES = TOTAL_FRAMES_VBS;
export default MainValeriaBocaSeca;
