/**
 * ============================================================================
 * Main_alimentos60 — Doctor Bastida · "Come ESTOS 3 ALIMENTOS" (piel/belleza +60)
 ...
 * Los beats salen de ./cues_alimentos60.gen (los genera build_alimentos60.mjs,
...
import {BEATS, TOTAL_FRAMES_AL60, type Cue} from './cues_alimentos60.gen';

const AVATAR = staticFile('alimentos60_opt.mp4');
...
const AvatarLayer: React.FC<{cuts: number[]}> = ({cuts}) => {
  ...
  const dust = React.useMemo(() => makeMotes(7, 'al60-dust', 2, 5, 0.008, 0.02, 0.05, 0.12), []);
  return (
    <>
      <AbsoluteFill
        style={{
...
const QrScene: React.FC<{cue: Cue; totalF: number}> = ({cue, totalF}) => {
  const frame = useCurrentFrame();
  const {fps, width, height} = useVideoConfig();
  const accent = cue.accent || VAL.gold;
  const qr = sf(cue.image || 'qr_drfederer.png')!;
  const inP = spring({frame, fps, config: {damping: 20, stiffness: 90, mass: 0.9}});
  const out = interpolate(frame, [totalF - Math.round(0.5 * fps), totalF - 2], [0, 1], CLAMP);
  const scan = 0.5 + 0.5 * Math.sin(frame * 0.08);
  const qrScale = interpolate(inP, [0, 1], [0.9, 1], CLAMP);
  const dust = React.useMemo(() => makeMotes(10, 'al60-qr', 2, 6, 0.01, 0.03, 0.05, 0.13), []);
  return (
    <AbsoluteFill style={{background: VAL.paper, overflow: 'hidden', opacity: 1 - out}}>
      <AbsoluteFill style={{background: `radial-gradient(80% 70% at 72% 45%, ${rgba(accent, 0.1)}, transparent 70%)`}} />
      <MotesLayer motes={dust} blur={1.3} scale={height / 1080} />
      {/* texto izquierda */}
      <div style={{position: 'absolute', left: '8%', top: '50%', transform: 'translateY(-50%)', width: '44%'}}>
        <Kicker text={cue.kicker || 'Su guía, gratis'} accent={accent} startSec={0.2} />
        <div style={{marginTop: height * 0.022}}>
          <Words
            text={cue.title || 'Apunte su cámara al código'}
            hot={cue.hot || ['código']}
            accent={accent}
            startSec={0.45}
        ...
        <div
          style={{
            ...
            color: VAL.ink2,
            opacity: interpolate(frame, [Math.round(1.2 * fps), Math.round(1.7 * fps)], [0, 1], CLAMP),
          }}
        >
          {cue.sub || 'Abra la cámara del teléfono y apúntela al recuadro. Es gratis.'}
        </div>
      </div>
      {/* QR derecha, sin recortar */}
      <div
        style={{
          position: 'absolute',
          right: '9%',
          top: '50%',
          transform: `translateY(-50%) scale(${qrScale})`,
          width: Math.min(width * 0.3, height * 0.52),
          padding: 20,
          borderRadius: 22,
          background: VAL.card,
          border: `1px solid ${rgba(accent, 0.5)}`,
          boxShadow: `0 ${height * 0.03}px ${height * 0.06}px ${rgba(VAL.ink, 0.26)}, 0 0 0 6px ${rgba(accent, 0.08 + scan * 0.06)}`,
        }}
      >
        <Img src={qr} style={{width: '100%', display: 'block', borderRadius: 12}} />
      </div>
    </AbsoluteFill>
  );
};
...
const CueScene: React.FC<{cue: Cue; totalF: number}> = ({cue, totalF}) => {
  ...
  switch (cue.kind) {
    ...
    case 'qr':
      return <QrScene cue={cue} totalF={totalF} />;
    default:
      return null;
  }
};
...
export const MainAlimentos60: React.FC = () => {
...
export const TOTAL_FRAMES = TOTAL_FRAMES_AL60;
export default MainAlimentos60;
