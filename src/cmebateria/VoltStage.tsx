// VoltStage.tsx — ESCENARIO COMPARTIDO del video `cmebateria`
// (canal Claudio Mendoza Constructor, ES · "Panel Solar de $50 vs Turbina Eólica de $50").
//
// Se escribe UNA vez, ANTES de lanzar los subagentes de movimientos, y TODOS lo consumen. Es lo que
// hace que seis movimientos escritos por seis agentes distintos se lean como UN video: la misma
...
//
// EL ESCENARIO DE ESTE VIDEO (lo comparten los seis movimientos):
//   Un patio de casa al anochecer, visto como un plató negro. El suelo es la losa de hormigón mate
//   del patio; el aire tiene polvo fino y, cuando hay viento, ESTRÍAS (`WindField`). Hay DOS luces
//   que se disputan la escena y definen el arco del video entero:
//     · la KEY VOLTIO (#C8F000), fría, arriba a la izquierda  = la MEDICIÓN, la pinza, el dato duro
//     · el CONTRA ÁMBAR (#FFC83D), cálido, abajo a la derecha = la FACTURA, el dinero, la casa
//   A medida que avanza el video la luz se corre del voltio al ámbar y, en la noche del apagón,
//   la única fuente pasa a ser el haz blanco de la linterna (`torch`). La luz EVOLUCIONA, no salta.
//
// Lo que exporta, y para qué:
//   · `V` / `F_DISPLAY` / `F_BODY` — paleta y tipografías del canal (negro/voltio/ámbar).
//   · `gcam(f, o)`   — LA CÁMARA. Función del frame GLOBAL del movimiento; nunca vuelve a 0.
//   · `VoltAtmos`    — la atmósfera: se monta UNA vez por movimiento y no se remonta entre actos.
//   · `WindField`    — estrías de viento en el aire. La firma visual de ESTE video.
//   · `Layers`/`Plane` — planos de profundidad con parallax propio (`preserve-3d` + `translateZ`).
...
//   · `PhotoPlane`   — foto/clip a sangre como plano de fondo, con parallax y grade del canal.
//   · `IconPng`      — ícono/logo PNG sin fondo como OBJETO de la escena.
//   · `Readout`      — la lectura de la pinza: cifra grande con unidad, que SALTA a su valor.
//   · `WhiteRoom`    — la escena casi blanca, limpia y hermosa (el pedido literal del creador).
//   · `Seam*`        — las costuras: Occlude, WipeMatter, Flash, ZoomThrough. ⛔ no hay fade.
//   · `Kick/Head/Em/Bed/Num` — tipografía con cama oscura (legibilidad +60).
//
...
//   ⛔ el color de un `SeamOcclude` es el de LA MATERIA que cruza (chapa, hormigón, pala blanca),
//      NUNCA el color del fondo: con el color del fondo hace un fundido a negro que se ve.
...
export type Lum = "volt" | "amber" | "torch" | "sky";
export const light = (t: number, from: Lum = "volt", to: Lum = "volt") => {
  const C = { volt: V.volt, amber: V.amber, torch: V.torch, sky: V.sky } as const;
...
export const MediaCard: React.FC<{
  src: string;                      // "broll/cmeb_o_401_palas.mp4" | "img/cmeb_h_002_billetes.jpg"
  kind?: "video" | "photo";
  w?: number; h?: number;
