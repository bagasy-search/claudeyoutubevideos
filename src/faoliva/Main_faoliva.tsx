// faoliva — VLOG CONTINUO (agnes-video-2.5-flash). Base = un mp4 por escena (armado por scripts/agnes_vlog.mjs),
...
import { TL, TOTAL_FRAMES_FAOLIVA, AUDIO, LAM_KEYS } from "./timeline_faoliva.gen";

export { TOTAL_FRAMES_FAOLIVA };
const FPS = 30;
...
const Lamina: React.FC<{ dur: number }> = ({ dur }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#f6f1e6", opacity: fadeIn }}>
      <Img src={staticFile("img/faoliva/lamina.jpg")} style={{ position: "absolute", left: 0, top: 0, width: W, height: H, transformOrigin: "0 0", transform: `translate(${tx}px, ${ty}px) scale(${s})` }} />
    </AbsoluteFill>
  );
};
  ...
  return (
    <AbsoluteFill style={{ opacity: o }}>
      <div style={{ position: "absolute", right: 70, bottom: 70, background: "#ffffff", borderRadius: 22, padding: 22, boxShadow: "0 10px 40px rgba(0,0,0,0.28)", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        <Img src={staticFile("img/faoliva/qr_faoliva.png")} style={{ width: 360, height: 360, display: "block" }} />
