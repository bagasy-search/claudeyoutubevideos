      ...
      <AbsoluteFill style={{ transform: `translateX(${drift.toFixed(2)}px)` }}>
        ...
        >
          ...
          {nodes.map((p) => {
            ...
            const color = isLast ? V.danger : V.brass;
        ...
        {nodes.map((p) => {
          ...
          // ⛔ La etiqueta va CENTRADA sobre el nodo (`translate(-50%)`), asi que la del primer nodo
          //    (y la del ultimo) se sale del cuadro y se lee cortada — cazado con un `remotion still`
          //    en dale9: el paso 1 salia como "...e line out loud to somebody you live with".
          //    Se ancla por el borde cuando el nodo esta cerca de un extremo, en vez de centrar.
          const BORDE = 3.5;                 // % de margen que se respeta contra el borde del cuadro
          const pegaIzq = leftPct < 22;
          const pegaDer = leftPct > 78;
          const anclaX = pegaIzq ? `${BORDE}%` : pegaDer ? undefined : `${leftPct}%`;
          const anclaDer = pegaDer ? `${BORDE}%` : undefined;
          const shiftX = pegaIzq || pegaDer ? "0" : "-50%";
          return (
            <div
              key={p.i}
              style={{
                ...
                maxWidth: "42%",
                top: `${topPct}%`,
                transform: `translate(${shiftX}, ${((1 - a) * (above ? -10 : 10)).toFixed(1)}px)`,
                opacity: a,
                pointerEvents: "none",
              }}
            >
              ...
              >
                ...
                <span
                  style={{
                    ...
                    color: isLast ? V.dangerSoft : V.white,
                  }}
                >
                  {p.label}
                </span>
