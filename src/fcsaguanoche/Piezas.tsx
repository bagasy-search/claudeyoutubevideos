// Piezas.tsx — piezas de escena del video `fcsaguanoche` (canal Federer Consejos Salud).
...
export const Foto: React.FC<{ src: string; seed: number }> = ({ src, seed }) => {
  const kb = useKenBurns(seed, 1.06, 0.06, 0.16, 1.2);
  return (
...
export const Clip: React.FC<{ src: string; seed: number; frames?: number }> = ({ src, seed, frames }) => {
  ...
  const video = <OffthreadVideo src={staticFile(src)} muted playbackRate={1} style={{ width: "100%", height: "100%", objectFit: "cover", ...kb }} />;
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
...
export const AvatarClip: React.FC<{ src: string; seed: number }> = ({ src, seed }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: V.ink0, overflow: "hidden" }}>
      <OffthreadVideo src={staticFile(src)} muted style={{ width: "100%", height: "100%", objectFit: "cover", transform: `scale(${Math.min(1.06, s).toFixed(4)}) translateX(${dx.toFixed(3)}%)` }} />
    </AbsoluteFill>
  );
};
