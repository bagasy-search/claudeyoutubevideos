...
/** SELLO DE NÚMERO — el único gráfico permitido sobre un plano de ítem (vara del canal:
 *  vlog crudo, cero componentes salvo el sello y el CTA). Va en la ESQUINA, como OVERLAY:
 *  ⛔ nunca como cue base — como plano base queda solo sobre el fondo de marca = pantalla negra.
 *  Sin fondo de tarjeta, sin panel: sólo el número y una barra corta, para no tapar el b-roll. */
export const SelloNum: React.FC<{
  num: string;
  pie?: string;
}> = ({ num, pie }) => {
  const frame = useCurrentFrame();
  const { durationInFrames } = useVideoConfig();
  const inP = interpolate(frame, [0, 9], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const out = interpolate(frame, [Math.max(10, durationInFrames - 8), durationInFrames], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const a = Math.min(inP, out);
  const x = (1 - inP) * -22;
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      <div style={{
        ...
        opacity: a, transform: `translateX(${x.toFixed(1)}px)`,
        borderLeft: `6px solid ${V.volt}`, paddingLeft: 20,
      }}>
        <div style={{
          fontFamily: F_DISPLAY, fontSize: 148, lineHeight: 0.9, fontWeight: 800,
          color: V.volt, textShadow: `0 8px 34px ${rgba(V.ink0, 0.95)}`,
        }}>{num}</div>
        {pie ? (
          <div style={{
            fontFamily: F_DISPLAY, fontSize: 34, letterSpacing: "0.10em", marginTop: 6,
            textTransform: "uppercase", color: V.white,
            textShadow: `0 4px 20px ${rgba(V.ink0, 0.95)}`,
          }}>{pie}</div>
        ) : null}
      </div>
    </AbsoluteFill>
...
/** CTA DE CIERRE — este canal NO tiene landing ni infoproducto, así que el CTA es de canal:
 *  suscripción y el video que viene. Va como OVERLAY sobre el último plano real, en la esquina
 *  inferior, sin tarjeta opaca de pantalla completa.
 *  ⛔ Va en `overlays[]`, NUNCA como cue base (mina medida en `dale1`: 13 s de pantalla negra). */
export const CtaFinal: React.FC<{
  ...
  const out = interpolate(frame, [Math.max(16, durationInFrames - 10), durationInFrames], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const a = Math.min(inP, out);
  const y = (1 - inP) * 26;
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      <div style={{
        position: "absolute", left: 64, bottom: 150, maxWidth: 1360,
        opacity: a, transform: `translateY(${y.toFixed(1)}px)`,
      }}>
        <div style={{
          display: "inline-block", padding: "18px 30px 20px",
          background: rgba(V.ink0, 0.74), borderLeft: `6px solid ${V.volt}`,
        }}>
          <div style={{
            fontFamily: F_DISPLAY, fontSize: 72, lineHeight: 1.04, fontWeight: 800,
            textTransform: "uppercase", color: V.white, letterSpacing: "-0.005em",
            textShadow: `0 4px 24px ${rgba(V.ink0, 0.95)}`,
          }}>{head}</div>
          {sub ? (
            <div style={{
              fontFamily: F_DISPLAY, fontSize: 42, lineHeight: 1.15, marginTop: 10,
              color: V.volt, textShadow: `0 4px 22px ${rgba(V.ink0, 0.95)}`,
            }}>{sub}</div>
          ) : null}
        </div>
