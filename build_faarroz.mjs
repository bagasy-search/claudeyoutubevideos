...
const cues = [];
const usables = mm.filter((m) => m.tipo || m.comp);
for (let k = 0; k < usables.length; k++) {
  ...
  if (m.comp) {
    ...
    cues.push({ key: `c${m.i}`, start: f0, dur: Math.min(Math.max(f1 - f0, F(piso)), TOTAL - f0), capa: "over", comp: m.comp });
    continue;
  }
...
const el = (c) => {
  ...
  if (c.comp) return `<${c.comp.pieza} {...(${j(c.comp.props)} as any)} />`;
  if (c.tipo === "clip") return `<Clip src="${c.src}" seed={${c.start}} frames={${c.frames}} />`;
  return `<Foto src="${c.src}" seed={${c.start}} />`;
};
...
console.log(`ventanas de avatar ....... ${ventanas} (deliberadas, en el tramo con lipsync real)`);
