// build_ohfcardboard.mjs — "Unbelievable Cardboard Trick for Termites!" (The Old House Fix, EN).
//
//   node build_ohfcardboard.mjs
//
...
// El avatar es el PISO: real en los primeros 602 s (su propio lipsync) y en bucle mudo después,
...
//
// ⛔ TODO lo que dibuja video usa OffthreadVideo (copias locales en src/ohfcardboard/). Con <Video>
...
const SLUG = "ohfcardboard", COMP = "OhfCardboard";
...
for (const b of beats) {
  ...
  if (b.tipo === "clip") {
    ...
    cues.push({ key, start, dur, el: `(d) => <MdClipOhf durationInFrames={d} src="${src}" />` });
  } else {
    ...
    cues.push({ key, start, dur, el: `(d) => <MdFotoOhf durationInFrames={d} src="${src}" pan={${pan}} />` });
  }
}
...
// Faltó una vez y los 60 chunks murieron con "Failed to fetch audio data ... 404".
...
const assets = [...clipsUsed, ...fotosUsed, ...blurs, VIS].sort();
