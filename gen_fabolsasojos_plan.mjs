// gen_fabolsasojos_plan.mjs — DIRECTOR (spec + assets reales en disco) → _v3/fabolsasojos_plan.json
//   canal Federer Archivos · "Un Médico Explica Cómo Deshacerse de las Bolsas en los Ojos" · Fish + RunPod (1 reel)
//   node gen_fabolsasojos_plan.mjs && node build_fabolsasojos.mjs
...
const SLUG = "fabolsasojos", FPS = 30;
...
const ultimoNoAvatar = (idx) => { for (let k = idx; k >= 0; k--) if (beats[k].tipo !== "avatar" && beats[k].tipo !== "lamina" && beats[k].src) return beats[k]; return null; };
const rellenoDe = (msA, msB, idx) => {
  ...
  for (const m of MOM) {                                  // cualquier asset SIN USAR de la zona (±45 s)
    if (Math.abs(m.t * 1000 - msA) > 45000 || SPEC[m.name]?.t === "avatar") continue;
...
for (let i = 0; i < beats.length; i++) {
  ...
  if (beats[i].tipo === "avatar") {
    ...
    if (i + 1 < beats.length && beats[i + 1].tipo !== "avatar") beats[i + 1].ms_in = beats[i].ms_out;
    else {
