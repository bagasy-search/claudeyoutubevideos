...
execFileSync("node", ["scripts/avatar_assemble.mjs", `${dir}/reel30.mp4`, `${dir}/reel.wav`, `${raw}:0:${reelWav.toFixed(3)}`], { stdio: "inherit" });
...
for (const w of W) {
  ...
  execFileSync("ffmpeg", ["-v", "error", "-y", "-ss", w.reel_off.toFixed(3), "-i", `${dir}/reel30.mp4`, "-t", d.toFixed(3), "-an", "-vf", "fps=30,setsar=1,format=yuv420p", "-c:v", "libx264", "-preset", "veryfast", "-crf", "18", "-r", "30", out]);
