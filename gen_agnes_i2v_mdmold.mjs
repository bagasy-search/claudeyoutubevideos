// gen_agnes_i2v_mdmold.mjs — IMAGEN-A-VIDEO con agnes-video-v2.0: anima las fotos hero del
...
const buildPrompt = (it) => [
  ...
  "no cuts, no new scene.",
  // HARDEN=1 → pasada de REPARACIÓN. Sale de lo que encontró `clipaudit_agnes.mjs` en la 1ª tanda:
  // objetos que se esfuman de la mano, herramientas que mutan, dedos fundidos y etiquetas con texto
  // inventado legible. Se pide explícito lo contrario de cada uno.
  process.env.HARDEN ? "Stability: every object he holds stays in his hands for the whole take and " +
    "never disappears, never changes shape and never changes size. His fingers stay separate and " +
    "countable and his grip does not change. Every label, sticker, dial and printed surface stays " +
    "blank and unreadable, with no letters, words or logos anywhere. The room, the fixtures and the " +
    "objects behind him stay exactly where they are and nothing new appears." : null,
