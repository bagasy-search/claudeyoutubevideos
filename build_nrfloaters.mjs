// build_nrfloaters.mjs — mismo motor que build_nrvaseneck (cadena fcsmanos10: avatar por ventanas + LaminaZoom + camas). Sólo cambian slug/portada/QR/landing.
//   src/_fed6/VideoEdit/Main_nrvaseneck.tsx      (build real, con `startSec:` por cue → lo mide el gate)
//   src/_fed6/VideoEdit/avatar_nrvaseneck.gen.ts (ventanas de avatar → tiempo VISIBLE del gate)
//   src/index_nrvaseneck.tsx
...
const SLUG = "nrfloaters", COMP = "Nrfloaters", UP = "NRFLOATERS", FPS = 30;
const COVER = "img/nrfloaters_cover.jpg", QR = "qr_nrfloaters.png", SITE = "docfederer.com/after-60";
const WAV_FILE = `${SLUG}.m4a`;
const TABLE = `img/${SLUG}/lam_table.jpg`;
...
for (let i = 0; i + 1 < rows.length; i++) {
  ...
  const fuente = rows[i].tipo === "avatar" ? rows[i].clip : (rows[i].tipo === "clip" || rows[i].tipo === "floaters") ? rows[i].src : rows[i].under || null;
  if (!fuente) continue;
  // ⛔ nunca adelantar una VENTANA DE AVATAR: su clip arrancaría antes que su audio (labios corridos).
  //    Si el que sigue es avatar, el clip corto se cubre con <Loop> (ReframedVideo recibe frames reales).
  if (rows[i + 1].tipo === "avatar" && rows[i].tipo !== "avatar") continue;
...
// vuelve negro PURO. Medido: fcsunaclavada entregado = 57 tramos, nrg-oil = 119, TODOS en
...
for (const b of rows) {
  ...
  if (b.tipo === "avatar") {
  ...
  } else if (b.tipo === "floaters") {
    scan(b.src);
    const frames = Math.floor(durDe(b.src) * FPS) - 1;
    if (frames < 15) { console.error(`⛔ clip ${b.src} con ${frames} cuadros`); process.exit(1); }
    cues.push({ key, start, dur, el: `<FloaterClip src={${JSON.stringify(b.src)}} seed={${b.f0}} frames={${frames}} kind={${JSON.stringify(b.kind || "sky")}} />` });
    ...
    scan(b.src); assets.add(COVER); assets.add(QR); assets.add(TABLE);
    cues.push({ key, start, dur, el: `<Lamina3D table={${JSON.stringify(TABLE)}} src={${JSON.stringify(b.src)}} keys={${JSON.stringify(b.keys)}} ${b.cta !== undefined ? `cta={${b.cta}} cover={${JSON.stringify(COVER)}} qr={${JSON.stringify(QR)}} site={${JSON.stringify(SITE)}}` : ""} />` });
...
if (sinCama.length) {
  ...
  console.error("   → corré: el plan trae bed por componente (_v3/nrt_plan.py)");
  process.exit(1);
}
...
{ // fps / aspecto
  ...
  const imgs = [...assets].filter((a) => /\.(png|jpe?g)$/i.test(a) && a !== COVER && a !== QR);
...
fs.writeFileSync(`_v3/${SLUG}_cues.json`, JSON.stringify(
  rows.filter((b) => b.tipo === "clip" || b.tipo === "imagen" || b.tipo === "floaters")
