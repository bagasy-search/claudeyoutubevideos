// gen_facrema81_plan.mjs — DIRECTOR (spec + assets reales en disco) → _v3/facrema81_plan.json
...
const WAV_S = durDe(`broll/${SLUG}/${SLUG}.m4a`);
const TOTAL_S = WAV_S + 0.5;

const QR = `img/${SLUG}/${SLUG}_qrcard.png`, DOM = "drfederer.com";
...
for (const [frag, props] of [
  ["si están en el teléfono, el enlace está abajo", { eyebrow: "LA GUÍA COMPLETA", title: "Esta es una página de la guía",
    sub: "En la tele, escanea el código. En el teléfono, el enlace está abajo.", domain: DOM, qr: QR, showQr: true }],
  ["escanea el código que aparece en la pantalla", { eyebrow: "EN LA DESCRIPCIÓN", title: "La guía completa del Dr. Federer",
    sub: "Con esta receta, los errores y las señales de alarma, ordenados por tema.", domain: DOM, qr: QR, showQr: true }],
]) {
  const k = buscar(frag);
