...
# La línea de tiempo sale de timeline_faperejil.gen.ts (segmentos de escena con startFrom) → cero re-render.
...
W = "D:/Proyectos/video2-wt/faperejil/"; B = W + "vlog/faperejil/"; F = B + "foley/"
FIN = "D:/videosdeclaude/faperejil_entrega.mp4"; SR = 48000; FPS = 30
...
def gate(x):
    ...
    return x * out
def src(cid):
    if cid.startswith("s2_"): return F + (f"out_S2/{cid}.flac" if cid in FULL else f"out_S2m/{cid}.flac")
    return F + (f"out_full/{cid}.flac" if cid in FULL else f"out_gate/{cid}.flac")
def ambient(kind, L):
    base = "out_S2amb/amb" if kind == "kitchen" else f"out_amb/{kind}"
...
EV, AM, n_ev = {}, {}, 0
for s in range(1, 9):
    plan = json.load(open(B + f"plan_S{s}.json", encoding="utf-8")); st = json.load(open(B + f"S{s}/clips/state.json", encoding="utf-8"))
    ...
    for c, t in zip(plan["clips"], T):
        ...
        if cid in FULL or cid in GATE:
            ...
            if cid in ("s2_02", "s2_07", "s3_05", "s3_06", "s8_01"): x *= 0.55     # agua continua: más baja
...
tl = open(W + "src/faperejil/timeline_faperejil.gen.ts", encoding="utf-8").read()
TOTAL = int(re.search(r"TOTAL_FRAMES_FAPEREJIL = (\d+)", tl).group(1))
...
for seg in TL:
    ...
    if seg["kind"] == "vid":
        s = int(re.search(r"S(\d)\.mp4", seg["src"]).group(1)); last_scene = s
        ...
        m = AM[5][: n] if len(AM[5]) >= n else np.resize(AM[5], n)
        am_f[a0:a0 + n] += m
...
out = "D:/videosdeclaude/faperejil_entrega_sfx.mp4"
