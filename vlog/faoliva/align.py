...
lines = open("vlog/faoliva/guion_voz.txt", encoding="utf-8").read().strip().split("\n")
G, L = [], []
for i, ln in enumerate(lines):
    for w in ln.split():
        n = norm(w)
        if n: G.append(n); L.append(i)
caps = json.load(open("public/captions_faoliva.json", encoding="utf-8"))
...
_w = wave.open("out/faoliva/master.wav"); sr = _w.getframerate(); _ch=_w.getnchannels(); x = np.frombuffer(_w.readframes(_w.getnframes()), dtype=np.int16).astype(np.float32)/32768; x = x.reshape(-1,_ch).mean(1)
TOT = len(x)/sr*1000
# boundaries per line
first = {}; last = {}
...
json.dump(tr, open("vlog/faoliva/tramos.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
d = [t["d"] for t in tr]
